...
void main() {
  testWidgets(
    'ChipAnimationStyle.enableAnimation overrides chip enable animation',
    (WidgetTester tester) async {
      ...
      ); // Advance enable animation by 500ms.
      ...
      ); // Advance enable animation by 1500ms.
  ...
  testWidgets(
    'ChipAnimationStyle.selectAnimation overrides chip select animation',
    (WidgetTester tester) async {
      await tester.pumpWidget(const example.ChipAnimationStyleExampleApp());
      ...
      await tester.pump(
        const Duration(milliseconds: 1500),
      ); // Advance select animation by 1500ms.
      ...
      await tester.pump(
        const Duration(milliseconds: 500),
      ); // Advance select animation by 500ms.
  ...
  testWidgets(
    'ChipAnimationStyle.avatarDrawerAnimation overrides chip checkmark animation',
    (WidgetTester tester) async {
      await tester.pumpWidget(const example.ChipAnimationStyleExampleApp());

      expect(
        tester.getSize(find.widgetWithText(RawChip, 'Checked')).width,
        closeTo(152.6, 0.1),
      );
      ...
      await tester.pump(
        const Duration(milliseconds: 500),
      ); // Advance avatar animation by 500ms.

      expect(
        tester.getSize(find.widgetWithText(RawChip, 'Unchecked')).width,
        closeTo(160.9, 0.1),
      );
      ...
      ); // Advance avatar animation by 1sec.
  ...
  testWidgets(
    'ChipAnimationStyle.deleteDrawerAnimation overrides chip delete icon animation',
    (WidgetTester tester) async {
      await tester.pumpWidget(const example.ChipAnimationStyleExampleApp());

      expect(
        tester.getSize(find.widgetWithText(RawChip, 'Deletable')).width,
        closeTo(180.9, 0.1),
      );
      ...
      await tester.pump(
        const Duration(milliseconds: 500),
      ); // Advance delete icon animation by 500ms.

      expect(
        tester.getSize(find.widgetWithText(RawChip, 'Undeletable')).width,
        closeTo(204.6, 0.1),
      );
      ...
      await tester.pump(
        const Duration(seconds: 1),
      ); // Advance delete icon animation by 1sec.
