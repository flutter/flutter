...
void main() {
  testWidgets('Tapping FAB adds to values', (WidgetTester tester) async {
    ...
    final Finder listContent = find.byWidgetPredicate((Widget widget) => widget is example.ListContent);

    expect(find.text('Current values:'), findsOneWidget);
    expect(find.byIcon(Icons.add), findsOneWidget);
    expect(
      (tester.widget(listContent) as example.ListContent).listNotifier.values.isEmpty,
      isTrue,
    );
    ...
    expect(
      (tester.widget(listContent) as example.ListContent).listNotifier.values,
      <int>[1464685455],
    );
