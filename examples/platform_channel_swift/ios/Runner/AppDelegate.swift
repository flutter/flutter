...
@objc class AppDelegate: FlutterAppDelegate {
  ...
  override func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
    ...
    let batteryChannel = FlutterMethodChannel.init(name: "io.flutter.samples/battery",
      ...
      batteryChannel!.setMethodCallHandler({
        ...
        if ("getBatteryLevel" == call!.method) {
          ...
          result!(nil, FlutterError.init(code: "UNKNOWN_METHOD",
...
private func receiveBatteryLevel(result: FlutterResultReceiver) {
  ...
  if (device.batteryState == UIDeviceBatteryState.unknown) {
    result(nil, FlutterError.init(code: "UNAVAILABLE",
                                  message: "Battery info unavailable",
                                  details: nil));

  } else {
    result(Int(device.batteryLevel * 100), nil);
  }
}
