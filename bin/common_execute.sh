...
if [ -z "$ENTRYPOINT" ]; then
  echo 'Error: do not directly invoke this script. Instead, invoke either'
  echo 'flutter or dart from this directory.'
  exit 1
elif [[ "$ENTRYPOINT" != "flutter" && "$ENTRYPOINT" != "dart" ]]; then
  echo "Error: cannot parse the \$ENTRYPOINT variable: \"$ENTRYPOINT\". It"
  echo 'should be either "flutter" or "dart".'
  exit 1
fi
...
if [[ "$ENTRYPOINT" == 'flutter' ]]; then
  "$DART" --packages="$FLUTTER_TOOLS_DIR/.packages" $FLUTTER_TOOL_ARGS "$SNAPSHOT_PATH" "$@"
elif [[ "$ENTRYPOINT" == 'dart' ]]; then
  "$DART" "$@"
fi
