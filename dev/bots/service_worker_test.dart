...
Future<void> _setAppVersion(final int version) async {
...
String _testTypeToIndexFile(final ServiceWorkerTestType type) {
...
Future<void> _rebuildApp({ required final int version, required final ServiceWorkerTestType testType, required final String target }) async {
...
void _expectRequestCounts(
    ...
    final Map<String, int> requestedPathCounts,
) {
...
Future<void> _waitForAppToLoad(
    final Map<String, int> waitForCounts,
    final Map<String, int> requestedPathCounts,
  ...
  await Future.any(<Future<Object?>>[
    () async {
      while (!waitForCounts.entries.every((final MapEntry<String, int> entry) => (requestedPathCounts[entry.key] ?? 0) >= entry.value)) {
        await Future<void>.delayed(const Duration(milliseconds: 100));
      }
    }(),
    server!.onChromeError.then((final String error) {
...
/// test zone.
void expect(final Object? actual, final Object? expected) {
...
Future<void> runWebServiceWorkerTest({
  required final bool headless,
  required final ServiceWorkerTestType testType,
}) async {
  final Map<String, int> requestedPathCounts = <String, int>{};
  void expectRequestCounts(final Map<String, int> expectedCounts) =>
      _expectRequestCounts(expectedCounts, requestedPathCounts);

  AppServer? server;
  Future<void> waitForAppToLoad(final Map<String, int> waitForCounts) async =>
