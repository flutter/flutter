...
void testNoStopwatches(Stopwatch stopwatch) {
  stopwatch
      .runtimeType; // OK for now, but we probably want to catch public APIs that take a Stopwatch?
  final Stopwatch localVariable =
      Stopwatch(); // Bad: introducing Stopwatch from dart:core.
  ...
  Stopwatch anotherStopwatch =
      stopwatch; // OK: not directly introducing Stopwatch.
  StopwatchAtHome Function() constructor =
      StopwatchAtHome.new; // Bad: introducing a Stopwatch constructor.
  assert(() {
    anotherStopwatch = constructor()..runtimeType;
    constructor =
        StopwatchAtHome.create; // Bad: introducing a Stopwatch constructor.
    anotherStopwatch = constructor()..runtimeType;
    return true;
  }());
  anotherStopwatch.runtimeType;

  externallib.MyStopwatch
      .create(); // Bad: introducing an external Stopwatch constructor.
  ExternalStopwatchConstructor? externalConstructor;

  assert(() {
    externalConstructor = externallib
        .MyStopwatch.new; // Bad: introducing an external Stopwatch constructor.
    return true;
  }());
  ...
  externallib
      .createMyStopwatch()
      .runtimeType; // Bad: calling an external function that returns a Stopwatch.
  externallib
      .createStopwatch()
      .runtimeType; // Bad: calling an external function that returns a Stopwatch.
  externalConstructor = externallib
      .createMyStopwatch; // Bad: introducing the tear-off form of an external function that returns a Stopwatch.
...
void testStopwatchIgnore(Stopwatch stopwatch) {
  ...
  Stopwatch()
      .runtimeType; // flutter_ignore: some_other_ignores, stopwatch (see analyze.dart)
}
