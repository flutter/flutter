...
class Context {
  ...
  static const List<String> expectedEntitlements = <String>[
  ...
  /// This list should be kept in sync with the actual contents of Flutter's
  /// cache.
  List<String> get signedXcframeworks {
    return <String>[
  ...
  Future<void> verifyExist() async {
    ...
    printer('All expected binaries present.');
  }
  ...
  Future<void> verifySignatures() async {
    ...
    for (final String pathToCheck in binariesAndXcframeworks) {
      ...
      if (binariesWithEntitlements.contains(pathToCheck)) {
        verifySignature = true;
        verifyEntitlements = true;
      }
      if (binariesWithoutEntitlements.contains(pathToCheck)) {
        verifySignature = true;
      }
      if (signedXcframeworks.contains(pathToCheck)) {
        verifySignature = true;
      }
      if (!verifySignature && !verifyEntitlements) {
        unexpectedFiles.add(pathToCheck);
        printer('Unexpected binary or xcframework $pathToCheck found in cache!');
        continue;
      }
      printer('Verifying the code signature of $pathToCheck');
      ...
      if (verifyEntitlements) {
        printer('Verifying entitlements of $pathToCheck');
        if (!(await hasExpectedEntitlements(pathToCheck))) {
    ...
    if (unsignedFiles.isNotEmpty) {
      printer('Found ${unsignedFiles.length} unsigned files:');
      unsignedFiles.forEach(printer);
    }

    if (wrongEntitlementBinaries.isNotEmpty) {
      printer('Found ${wrongEntitlementBinaries.length} files with unexpected entitlements:');
      wrongEntitlementBinaries.forEach(printer);
    }

    if (unexpectedFiles.isNotEmpty) {
      printer('Found ${unexpectedFiles.length} unexpected files in the cache:');
      unexpectedFiles.forEach(printer);
    }
    ...
    printer('Verified that files are codesigned and have expected entitlements.');
  }
  ...
  Future<List<String>> findXcframeworksPaths(String rootDirectory) async {
    final io.ProcessResult result = await processManager.run(
      <String>[
        'find',
        rootDirectory,
