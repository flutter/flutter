...
String presentState(ConductorState state) {
  ...
  buffer.writeln();
  buffer.writeln(
      'Release started at: ${DateTime.fromMillisecondsSinceEpoch(state.createdDate.millisecondsSinceEpoch)}');
  buffer.writeln(
      'Last updated at: ${DateTime.fromMillisecondsSinceEpoch(state.lastUpdatedDate.millisecondsSinceEpoch)}');
  buffer.writeln();
...
String phaseInstructions(ConductorState state) {
...
void writeStateToFile(File file, ConductorState state, List<String> logs) {
...
ConductorState readStateFromFile(File file) {
  final String stateAsString = file.readAsStringSync();
  final ConductorState state = ConductorState.fromJson(jsonDecode(stateAsString) as Map<String, dynamic>);
...
bool requiresEnginePR(ConductorState state) {
...
bool requiresFrameworkPR(ConductorState state) {
  if (requiresEnginePR(state)) {
    return true;
  }
  return false;
