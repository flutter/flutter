...
class Timeseries {
  ...
  TimeseriesStats computeStats() {
    ...
    if (_warmUpSampleCount == 0) {
      throw StateError('The benchmark did not warm-up. Use at least one sample to warm-up '
          'the benchmark to reduce noise.');
    }
    if (_warmUpSampleCount >= count) {
      throw StateError('The benchmark did not report any measured samples. Add at least one '
...
class TimeseriesStats {
  ...
  String toString() {
    ...
    buffer.writeln('$name: (samples: $cleanSampleCount clean/$outlierSampleCount '
...
class Profile {
  ...
  Profile({required this.name, this.useCustomWarmUp = false}) : assert(name != null);
  ...
  void stopBenchmark() {
    if (_isWarmingUp) {
      throw StateError('Warm-up has not finished yet. Benchmark should only be stopped after '
  ...
  void _autoUpdateBenchmarkPhase() {
    if (useCustomWarmUp) {
      StateError('Must not call _autoUpdateBenchmarkPhase if custom warm-up is used. '
          'Call `stopWarmingUp` and `stopBenchmark` instead.');
    }

    if (_isWarmingUp) {
      final bool doesHaveEnoughWarmUpSamples =
          scoreData.keys.every((String key) => scoreData[key]!.count >= _kDefaultWarmUpSampleCount);
      if (doesHaveEnoughWarmUpSamples) {
        stopWarmingUp();
      }
    } else if (_isRunning) {
      final bool doesHaveEnoughTotalSamples =
          scoreData.keys.every((String key) => scoreData[key]!.count >= kDefaultTotalSampleCount);
