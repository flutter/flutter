  ...
  Future<void> run() async {
    ...
    if (upgrade || isPrintPaths || isPrintTransitiveClosure) {
      ...
      try {
        ...
        final ProcessResult result = await Process.run(path.join(flutterRoot, 'bin', 'cache', 'dart-sdk', 'bin', 'pub'),
          <String>[
            'upgrade',
            if (offline)
              '--offline',
        ...
        if (result.exitCode != 0) {
          ...
          throw Exception('pub upgrade failed');
        }
        ...
        // each package.
        final ProcessResult depsResult = await Process.run(path.join(flutterRoot, 'bin', 'cache', 'dart-sdk', 'bin', 'pub'),
        ...
        if (depsResult.exitCode != 0) {
          stderr.writeln(depsResult.stderr.toString());
          throw Exception('pub deps failed');
        }
        (depsResult.stdout as String).split('\n').forEach(tree.fill);
      } finally {
...
const String kTransitiveNewMagicString = '# THIS LINE IS AUTOGENERATED - TO UPDATE USE "dart dev/tools/lib/update_packages.dart update-packages --force-upgrade"';
...
class PubspecYaml {
  ...
  void apply(PubDependencyTree versions, Set<String> specialDependencies) {
    ...
    String computeTransitiveDependencyLineFor(String package) {
      return '  $package: ${versions.versionFor(package)} $kTransitiveNewMagicString';
    }
...
class PubspecDependency extends PubspecLine {
  ...
  static PubspecDependency parse(String line, { @required String filename }) {
    ...
    if (hashIndex >= 0) {
      ...
      isTransitive = trailingComment == kTransitiveMagicString || trailingComment == kTransitiveNewMagicString;
