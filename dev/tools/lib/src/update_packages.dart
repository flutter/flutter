...
class UpdatePackagesCommand extends Command<void> {
  ...
  Future<void> _downloadCoverageData() async {
    ...
    final List<int> data = (await http.get(coverageUri)).bodyBytes;
    final String coverageDir = path.join(
      flutterRoot,
      'packages/flutter/coverage',
    );
    File(path.join(coverageDir, 'lcov.base.info'))
      ..createSync(recursive: true)
      ..writeAsBytesSync(data, flush: true);
    File(path.join(coverageDir, 'lcov.info'))
      ..createSync(recursive: true)
      ..writeAsBytesSync(data, flush: true);
  }
  ...
  Future<void> run() async {
    ...
    final bool offline = argResults['offline'] as bool;
    ...
    if (isConsumerOnly) {
      ...
      packages.retainWhere((Directory directory) {
        return consumerPackages.any((String package) {
          return directory.path.endsWith('packages${path.separator}$package');
        });
      });
    }
    ...
    if (upgrade || isPrintPaths || isPrintTransitiveClosure) {
      ...
      final Directory tempDir = Directory.systemTemp.createTempSync('flutter_update_packages.');
      try {
        ...
        final ProcessResult upgradeResult = await Process.run(
          'dart',
          <String>[
            'pub',
            'upgrade',
          ...
          environment: <String, String>{
            if (upgrade)
              'FLUTTER_ROOT': temporaryFlutterSdk.path
            else
              'FLUTTER_ROOT': flutterRoot,
          },
        );
        if (upgradeResult.exitCode != 0) {
          stderr.write(upgradeResult.stderr);
          stdout.write(upgradeResult.stdout);
          exit(1);
        }
  ...
  void showDependencyPaths({
    ...
    if (!tree.contains(from)) {
      stderr.writeln('Package $from not found in the dependency tree.');
    }
    if (!tree.contains(to)) {
      stderr.writeln('Package $to not found in the dependency tree.');
    }
...
class PubspecDependency extends PubspecLine {
  ...
  bool get pointsToSdk {
    ...
    if (_kind == DependencyKind.path &&
        !path.isWithin(
          path.join(flutterRoot, 'bin'), _lockTarget) &&
...
File _pubspecFor(Directory directory) {
  return File(path.join(directory.path, 'pubspec.yaml'));
}
...
Directory createTemporaryFlutterSdk(Directory realFlutter, List<PubspecYaml> pubspecs) {
  final Set<String> currentPackages = Directory(path.join(realFlutter.path, 'packages'))
    .listSync()
    .whereType<Directory>()
    .map((Directory directory) => path.basename(directory.path))
    .toSet();
  ...
  File(path.join(realFlutter.path, 'version'))
    .copySync(path.join(directory.path, 'version'));

  // Directory structure should mirror the current Flutter SDK
  final Directory packages = Directory(path.join(directory.path, 'packages'));
  for (final String flutterPackage in currentPackages) {
    final File pubspecFile = File(path.join(packages.path, flutterPackage, 'pubspec.yaml'))
      ..createSync(recursive: true);
...
List<Directory> getRepoPackages() {
  return getRepoRoots()
    ...
    .map<Directory>((String dir) => Directory(dir))
    .toList();
}
...
List<String> _gatherProjectPaths(String rootPath) {
  ...
  final List<String> projectPaths = Directory(rootPath)
    .listSync(followLinks: false)
    .expand((FileSystemEntity entity) {
      if (entity is Directory && !path.split(entity.path).contains('.dart_tool')) {
