  ...
  group('Tokens logger', () {
    ...
    test('detects invalid logs', overridePrint(() {
      ...
      expect(
          printLog,
          contains(
              errorColoredString('Some referenced tokens do not exist: 2')));
      expect(printLog, contains('  baz'));
      expect(printLog, contains('  foobar'));
    }));

    test("color function doesn't log when providing a default",
        overridePrint(() {
    ...
    test('color function logs when not providing a default', overridePrint(() {
      ...
      expect(
          printLog,
          contains(
              errorColoredString('Some referenced tokens do not exist: 1')));
      expect(printLog, contains('  color_foo_req'));
    }));
    ...
    test('border function logs height token when width token not available',
        overridePrint(() {
      allTokens['border_foo.color'] = 'red';
