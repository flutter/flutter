...
void run(
  ...
  if (options['help'] as bool) {
    _usage(parser, stdout, exit, exitCode: 0);
    return;
  }
    ...
    try {
      for (final FileSystemEntity entity in currentDir.listSync(followLinks: false)) {
        if (entity is Directory) {
          ...
          if (name.startsWith('.') ||
              name == 'build' ||
              (name == 'cache' && fileSystem.path.basename(currentDir.path) == 'bin')) {
            continue;
          }
          search(entity);
...
void _usage(ArgParser parser, StringSink out, void Function(int) exit, {int exitCode = 1}) {
  ...
  out.writeln('Example: dart dev/tools/bin/bump_version_constraints.dart ^3.10.0-0 ^3.11.0-0');
