...
class LogicalKeyData {
  ...
  static void _readKeyEntries(Map<String, LogicalKeyEntry> data, String input) {
    ...
    for (final RegExpMatch match in domKeyRegExp.allMatches(input)) {
      ...
      final String? keyLabel =
          (match.namedGroup('kind')! == 'UNI' && !_isControlCharacter(value)) ? String.fromCharCode(value) : null;
      ...
      data.putIfAbsent(name, () {
        ...
        if (source == 'DOM' && !isPrintable) entry.webNames.add(webName);
        return entry;
      });
    }
  }
  ...
  static void _readMacOsKeyCodes(
    ...
    final Map<String, String> physicalToLogical =
        reverseMapOfListOfString(logicalToPhysical, (String logicalKeyName, String physicalKeyName) {
  ...
  static void _readGtkKeyCodes(
      Map<String, LogicalKeyEntry> data, String headerFile, Map<String, List<String>> nameToGtkName) {
    final RegExp definedCodes = RegExp(
      r'#define '
      r'GDK_KEY_(?<name>[a-zA-Z0-9_]+)\s*'
      r'0x(?<value>[0-9a-f]+),?',
    );
    final Map<String, String> gtkNameToFlutterName =
        reverseMapOfListOfString(nameToGtkName, (String flutterName, String gtkName) {
  ...
  static void _readWindowsKeyCodes(
      Map<String, LogicalKeyEntry> data, String headerFile, Map<String, List<String>> nameMap) {
    // The mapping from the Flutter name (e.g. "enter") to the Windows name (e.g.
    // "RETURN").
    final Map<String, String> nameToFlutterName =
        reverseMapOfListOfString(nameMap, (String flutterName, String windowsName) {
  ...
  static void _readAndroidKeyCodes(
      Map<String, LogicalKeyEntry> data, String headerFile, Map<String, List<String>> nameMap) {
    final Map<String, String> nameToFlutterName =
        reverseMapOfListOfString(nameMap, (String flutterName, String androidName) {
  ...
  static void _readFuchsiaKeyCodes(Map<String, LogicalKeyEntry> data, PhysicalKeyData physicalData) {
    for (final LogicalKeyEntry entry in data.values) {
      ...
      if (value != null) entry.fuchsiaValues.add(value);
    }
  }
  ...
  static void _readGlfwKeyCodes(
      Map<String, LogicalKeyEntry> data, String headerFile, Map<String, List<String>> nameMap) {
    final Map<String, String> nameToFlutterName =
        reverseMapOfListOfString(nameMap, (String flutterName, String glfwName) {
  ...
  static int _sourceToPlane(String source, bool isPrintable) {
    if (isPrintable) return kUnicodePlane.value;
    switch (source) {
      case 'DOM':
...
class LogicalKeyEntry {
  ...
  static String computeCommentName(String name) {
    ...
    return replaced
        ...
        // 'ABCDoo' => 'ABC Doo'
        .replaceAllMapped(
            RegExp(r'([A-Z])([A-Z])([a-z])'), (Match match) => '${match.group(1)} ${match.group(2)}${match.group(3)}')
        // 'AB1' => 'AB 1', 'F1' => 'F1'
