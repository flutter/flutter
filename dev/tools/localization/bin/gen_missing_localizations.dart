...
Future<void> main(List<String> rawArgs) async {
  ...
  updateMissingResources(l10nPath, 'material');
  updateMissingResources(l10nPath, 'cupertino');
}
...
void updateMissingResources(String l10nPath, String groupPrefix) {
  final Directory l10nDir = Directory(l10nPath);
  final RegExp filenamePattern = RegExp('${groupPrefix}_(\\w+)\.arb');

  final Set<String> requiredKeys = resourceKeys(loadBundle(File(path.join(l10nPath, '${groupPrefix}_en.arb'))));

  for (final FileSystemEntity entity in l10nDir.listSync().toList()..sort(sortFilesByPath)) {
