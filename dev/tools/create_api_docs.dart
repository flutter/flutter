...
const Map<String, PlatformDocsSection> kPlatformDocs =
    <String, PlatformDocsSection>{
...
Future<void> main(List<String> arguments) async {
  ...
  final Directory publishRoot =
      filesystem.directory(args['output-dir']! as String).absolute;
  ...
  final PlatformDocGenerator platformGenerator =
      PlatformDocGenerator(outputDir: publishRoot, filesystem: filesystem);
...
class Configurator {
  ...
  Iterable<String> _libraryRefs() sync* {
    for (final Directory dir in findPackages(filesystem)) {
      ...
      for (final FileSystemEntity file
          in dir.childDirectory('lib').listSync()) {
  ...
  void _createPageFooter(Directory footerPath, Version version) {
    ...
    footerPath
        .childFile('footer.html')
        .writeAsStringSync('<script src="footer.js"></script>');
...
''');
  ...
  void _copyCustomizations() {
    ...
    for (final String file in files) {
      ...
      if (path.canonicalize(source.absolute.path) !=
          path.canonicalize(destination.absolute.path)) {
    ...
    final Directory assetsDir =
        filesystem.directory(publishRoot.childDirectory('assets'));
  ...
  Future<void> _createDocset() async {
    ...
    final File iconFile =
        packageRoot.childDirectory('flutter.docset').childFile('icon.png');
    faviconFile
      ..createSync(recursive: true)
...
class DartdocGenerator {
  ...
  List<File> get canaries {
    ...
    final Directory widgetsDirectory =
        flutterDirectory.childDirectory('widgets');
    ...
    return <File>[
      ...
      flutterDirectory
          .childDirectory('material')
          .childFile('Material-class.html'),
      flutterDirectory
          .childDirectory('material')
          .childFile('Tooltip-class.html'),
  ...
  void _sanityCheckDocs([Platform platform = const LocalPlatform()]) {
    ...
    final String expectedBranch =
        luciBranch != null && luciBranch.isNotEmpty ? luciBranch : 'master';
  ...
  void _writeSnippetsIndexFile() {
    ...
    if (snippetsDir.existsSync()) {
      ...
      final Iterable<String> fileContents =
          files.map((File file) => file.readAsStringSync());
...
class PlatformDocGenerator {
  ...
  Future<Archive?> _fetchArchive(String url, int maxTries) async {
    ...
    return responseBytes == null
        ? null
        : ZipDecoder().decodeBytes(responseBytes);
  }

  Future<void> _extractDocs(String url, String name,
      PlatformDocsSection platform, Directory outputDir) async {
  ...
  return FlutterInformation.instance
      ...
      .where((FileSystemEntity entity) {
        ...
        if (!pubspec.existsSync()) {
          print(
              "Unexpected package '${entity.path}' found in packages directory");
          return false;
        }
...
class FlutterInformation {
  ...
  String getEngineRevision() =>
      getFlutterInformation()['engineRevision']! as String;
  ...
  String getFlutterRevision() =>
      getFlutterInformation()['flutterGitRevision']! as String;
  ...
  Map<String, Object> getFlutterInformation() {
    ...
    info['engineRealm'] =
        engineRealm.existsSync() ? engineRealm.readAsStringSync().trim() : '';
