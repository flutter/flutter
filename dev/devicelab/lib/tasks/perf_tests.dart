...
void _disablePartialRepaint(String testDirectory) {
  final String manifestPath =
      path.join(testDirectory, 'ios', 'Runner', 'Info.plist');
  ...
  for (final (String key, String value) in keyPairs) {
    applicationNode.children.add(XmlElement(
        XmlName('key'), <XmlAttribute>[], <XmlNode>[XmlText(key)], false));
...
void _addMetadataToManifest(
    String testDirectory, List<(String, String)> keyPairs) {
      ...
      switch (deviceOperatingSystem) {
        ...
        case DeviceOperatingSystem.windows:
          ...
          applicationBinaryPath = path.join(testDirectory, 'build', 'windows',
              arch, 'runner', 'Profile', '$basename.exe');
      }
      ...
      return TaskResult.success(
        ...
        benchmarkScoreKeys: benchmarkScoreKeys ??
            <String>[
              ...
              if (recordGPU) ...<String>[
                // GPU Frame Time.
                if (data['average_gpu_frame_time'] != null)
                ...
                if (data['worst_gpu_frame_time'] != null)
                  'worst_gpu_frame_time',
                // GPU Memory.
                if (data['average_gpu_memory_mb'] != null)
                  'average_gpu_memory_mb',
                if (data['90th_percentile_gpu_memory_mb'] != null)
                  '90th_percentile_gpu_memory_mb',
                if (data['99th_percentile_gpu_memory_mb'] != null)
  ...
  Future<Map<String, dynamic>> _compileApp(
    ...
    switch (deviceOperatingSystem) {
      ...
      case DeviceOperatingSystem.windows:
        ...
        final String arch = Abi.current() == Abi.windowsX64 ? 'x64' : 'arm64';
        final String exePath = path.join(cwd, 'build', 'windows', arch,
