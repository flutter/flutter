...
class PluginTest {
  ...
  Future<void> _testLocalEngineConfiguration(
      _FlutterProject app, String fakeEngineSourcePath) async {
    ...
    final Directory buildDir =
        Directory(path.join(fakeEngineSourcePath, 'out', 'foo'));
  ...
  Future<void> runNativeTests(String buildTarget) async {
    ...
    switch (buildTarget) {
      ...
      case 'windows':
        ...
        if (await exec(
              path.join(rootPath, 'build', 'windows', arch, 'plugins',
    ...
    await inDirectory(Directory(rootPath), () async {
      ...
      if (target == 'ios' || target == 'macos') {
        ...
        if (validateNativeBuildProject) {
          ...
          if (localEngine != null) {
            final RegExp localEngineSearchPath =
                RegExp('FRAMEWORK_SEARCH_PATHS\\s*=[^;]*${localEngine.path}');
