...
Future<String> getDartVersion() async {
  ...
  if (version.contains(':')) version = version.substring(version.indexOf(':') + 1).trim();
...
Future<int> flutter(
  ...
  return exec(path.join(flutterDirectory.path, 'bin', 'flutter'), args, canFail: canFail, environment: environment);
}
...
void cd(dynamic directory) {
  ...
  if (!d.existsSync()) throw FileSystemException('Cannot cd into directory that does not exist', d.toString());
}
...
final RegExp _obsRegExp = RegExp('An Observatory debugger .* is available at: ');
