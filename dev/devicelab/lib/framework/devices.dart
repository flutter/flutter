...
abstract class Device {
  ...
  Future<void> uninstallApp() async {
    await flutter('install',
        options: <String>['--uninstall-only', '-d', deviceId]);
...
class AndroidDevice extends Device {
  ...
  Future<void> awaitDevice() async {
    ...
    await retryOptions.retry(() async {
      final String adbShellOut =
          await adb(<String>['shell', 'getprop sys.boot_completed']);
      if (adbShellOut != '1') {
...
String get adbPath {
  ...
  if (androidHome == null) {
    throw const DeviceException('The ANDROID_HOME environment variable is '
