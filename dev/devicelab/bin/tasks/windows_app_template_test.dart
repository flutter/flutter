...
Future<void> main() async {
  await task(() async {
    final String integrationTestsPath = join(flutterDirectory.path, _kIntegrationTestsRelativePath);
    final String templatePath = join(flutterDirectory.path, _kTemplateRelativePath);
    ...
    for (final Directory testPath in subDirs) {
      ...
      final String runnerPath = join(testPath.path, _kWindowsRunnerSubPath);
      final Directory runner = Directory(runnerPath);
      if (!runner.existsSync()) {
        continue;
      }
      for (final String fileName in _kFilesToMatch) {
        final String templateFilePath = join(templatePath, fileName);
        String templateFile = await _fileContents(templateFilePath);
        String appFilePath = join(runnerPath, fileName);
