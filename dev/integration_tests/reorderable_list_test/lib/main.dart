...
class _ReorderableListTestPageState extends State<ReorderableListTestPage> {
  ...
  final List<double> animationPositions = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // Debug output for animation positions
          Container(
            key: const Key('animation_debug'),
            height: 50,
            padding: const EdgeInsets.all(8),
            child: Text(
              'Animation positions: ${animationPositions.join(', ')}',
          ...
          Expanded(
            child: ReorderableListView(
              ...
              onReorderStart: (int index) {
                // Clear previous animation positions when starting a new drag
                setState(() {
                  animationPositions.clear();
                });
              },
              onReorderEnd: (int index) {
                // Animation happens after this callback
                // We'll need to capture positions in the test
              },
