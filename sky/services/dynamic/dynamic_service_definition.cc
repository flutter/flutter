...
void DynamicServiceDefinition::Setup(const mojo::String& dylib_path) {
  ...
  if (!OpenDylib(dylib_path)) {
    FTL_LOG(ERROR) << "Could not open the service dylib.";
    return;
  }
  ...
  if (!AcquireServiceVersionProc()) {
    FTL_LOG(ERROR) << "Could not acquire proc for service version check.";
    return;
  }

  if (!CheckServiceVersion()) {
    FTL_LOG(ERROR) << "Service and embedder versions mismatch.";
    return;
  }

  if (!AcquireServiceProcs()) {
    FTL_LOG(ERROR) << "Could not acquire procs for loading the service.";
    return;
  }

  if (!InstallSystemThunks()) {
    FTL_LOG(ERROR) << "Could not install system thunks to process the service.";
    return;
  }

  if (!InvokeLibraryOnLoad()) {
    FTL_LOG(ERROR) << "Could not invoke library OnLoad.";
    return;
  }
