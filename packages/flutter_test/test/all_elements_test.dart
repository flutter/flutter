...
void main() {
  testWidgets('collectAllElements goes in LTR DFS', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: Row(
        children: <Widget>[
          RichText(text: TextSpan(text: 'a')),
          RichText(text: TextSpan(text: 'b')),
        ],
      ),
    ));
    ...
    expect((elements[1].widget as RichText).text.text, 'a');
    expect(elements[2].widget, isA<RichText>());
    expect((elements[2].widget as RichText).text.text, 'b');
  });
}
