...
void catchIsolateErrors() {
  ...
  errorPort.listen((dynamic message) {
    ...
    final Trace stackTrace = message[1] == null ? Trace(const <Frame>[]) : Trace.parse(message[1]);
    Zone.current.handleUncaughtError(error, stackTrace);
  });
}

/// Serialize a test suite.
StreamChannel<Map<String, Object>> serializeSuite(Function getMain(), {bool hidePrints = true, Future<void> beforeLoad()}) {
  return _RemoteListener.start(getMain, hidePrints: hidePrints, beforeLoad: beforeLoad);
}

class _RemoteListener {
  _RemoteListener(this._suite, this._printZone);
  ...
  static StreamChannel<void> start(Function getMain(),
    ...
    final MultiChannel<Map<String, Object>> channel = MultiChannel<Map<String, Object>>(controller.local);
    final Zone printZone = hidePrints ? null : Zone.current;
    bool verboseChain = true;
    final ZoneSpecification spec = ZoneSpecification(print: (Zone self, ZoneDelegate parent, Zone zone, String line) {
      if (printZone != null) {
    ...
    // once fixed.
    Stream<Object>.fromIterable(<Object>[]).listen((_) {}).cancel();
     SuiteChannelManager().asCurrent(() {
      StackTraceFormatter().asCurrent(() {
        runZoned(() async {
          ...
          final StreamQueue<Map<String, Object>> queue = StreamQueue<Map<String, Object>>(channel.stream);
          final Map<String, Object> message = await queue.next;
          assert(message['type'] == 'initial');
          queue.rest.listen((Map<String, Object> message) {
            assert(message['type'] == 'suiteChannel');
            SuiteChannelManager.current.connectIn(message['name'], channel.virtualChannel(message['id']));
          });
          if (message['asciiGlyphs'] == true) {
            glyph.ascii = true;
          }
          ...
          StackTraceFormatter.current.configure(
            except: Set<String>.from(message['foldTraceExcept'] ?? <String>[]),
            only: Set<String>.from(message['foldTraceOnly'] ?? <String>[]));
          if (beforeLoad != null) {
            await beforeLoad();
          }
          ...
          final Suite suite = Suite(
            ...
            path: message['path']);
          runZoned(() {
            Invoker.guard(() => _RemoteListener(suite, printZone)._listen(channel));
          },
  ...
  static void _sendLoadException(StreamChannel<Map<String, Object>> channel, String message) {
  ...
  static void _sendError(StreamChannel<Map<String, Object>> channel, dynamic error, StackTrace stackTrace, bool verboseChain) {
    channel.sink.add(<String, dynamic>{
      'type': 'error',
      'error': RemoteException.serialize(error, StackTraceFormatter.current.formatStackTrace(stackTrace, verbose: verboseChain))
    });
  }

  /// Send information about [_suite] across [channel] and start listening for
  /// commands to run the tests.
  void _listen(MultiChannel<Map<String, Object>> channel) {
  ...
  Map<String, dynamic> _serializeGroup(MultiChannel<Object> channel, Group group, Iterable<Group> parents) {
    ...
    return <String, Object>{
      ...
      'entries': group.entries.map((Object entry) {
  ...
  /// is `null`.
  Map<String, Object> _serializeTest(MultiChannel<Map<String, Object>> channel, Test test, Iterable<Group> groups) {
    if (test == null) {
      return null;
    }
    final VirtualChannel<Map<String, Object>> testChannel = channel.virtualChannel();
  ...
  void _runLiveTest(LiveTest liveTest, MultiChannel<Map<String, Object>> channel) {
    ...
    liveTest.onError.listen((AsyncError asyncError) {
      final Chain stackTrace = StackTraceFormatter.current.formatStackTrace(
        asyncError.stackTrace,
        verbose: liveTest.test.metadata.verboseTrace,
      );
      channel.sink.add(<String, Object>{
        'type': 'error',
        'error': RemoteException.serialize(asyncError.error, stackTrace),
      });
    });
...
class SuiteChannelManager {
  ...
  final Map<String, StreamChannel<Object>> _incomingConnections = <String, StreamChannel<Object>>{};

  /// Connections from calls to [suiteChannel] that have yet to connect to
  /// corresponding connections from the test runner.
  final Map<String, StreamChannelCompleter<Object>> _outgoingConnections = <String, StreamChannelCompleter<Object>>{};
