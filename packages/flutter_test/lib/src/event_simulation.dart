...
class KeyEventSimulator {
  ...
  static LogicalKeyboardKey _getConcreteKey(VirtualKeyboardKey key) => key.concreteKeys.first;
  ...
  static Future<bool> simulateKeyDownEvent(
    ...
    final LogicalKeyboardKey concreteKey = _getConcreteKey(key);
    Future<bool> _simulateByRawEvent() {
      return _simulateKeyEventByRawEvent(() {
        platform ??= _defaultPlatform;
        return getRawKeyData(concreteKey, platform: platform!, isDown: true, physicalKey: physicalKey, character: character);
      });
    }
    switch (_transitMode) {
      ...
      case KeyDataTransitMode.keyDataThenRawKeyData:
        final LogicalKeyboardKey logicalKey = _getKeySynonym(concreteKey);
        final bool resultByKeyEvent = ServicesBinding.instance!.keyEventManager.handleKeyData(
          ui.KeyData(
            ...
            character: character ?? _keyLabel(concreteKey),
            synthesized: false,
          ),
        );
  ...
  static Future<bool> simulateKeyUpEvent(
    ...
    Future<bool> _simulateByRawEvent() {
      return _simulateKeyEventByRawEvent(() {
        platform ??= _defaultPlatform;
        return getRawKeyData(concreteKey, platform: platform!, isDown: false, physicalKey: physicalKey);
      });
    }
    switch (_transitMode) {
      case KeyDataTransitMode.rawKeyData:
  ...
  static Future<bool> simulateKeyRepeatEvent(
    ...
    Future<bool> _simulateByRawEvent() {
      return _simulateKeyEventByRawEvent(() {
        platform ??= _defaultPlatform;
        return getRawKeyData(key.concreteKeys.first, platform: platform!, isDown: true, physicalKey: physicalKey, character: character);
      });
    }
    switch (_transitMode) {
      case KeyDataTransitMode.rawKeyData:
