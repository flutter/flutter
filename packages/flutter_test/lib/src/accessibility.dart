  ...
  FutureOr<Evaluation> evaluate(WidgetTester tester) {
    ...
    Evaluation traverse(SemanticsNode node) {
      ...
      if (paintBounds.left <= delta ||
          ...
          (paintBounds.bottom - tester.binding.window.physicalSize.height)
                  .abs() <=
              delta ||
          (paintBounds.right - tester.binding.window.physicalSize.width)
                  .abs() <=
              delta) return result;
      // shrink by device pixel ratio.
      final Size candidateSize =
          paintBounds.size / tester.binding.window.devicePixelRatio;
...
class MinimumTextContrastGuideline extends AccessibilityGuideline {
  ...
  Future<Evaluation> evaluate(WidgetTester tester) async {
    ...
    final ByteData byteData =
        (await tester.binding.runAsync<ByteData?>(() async {
      ...
      image = await layer.toImage(renderView.paintBounds,
          pixelRatio: 1 / tester.binding.window.devicePixelRatio);
    ...
    Future<Evaluation> evaluateNode(SemanticsNode node) async {
      ...
      if (node.isInvisible ||
          ...
          (node.hasFlag(ui.SemanticsFlag.hasEnabledState) &&
              !node.hasFlag(ui.SemanticsFlag.isEnabled))) return result;
      final SemanticsData data = node.getSemanticsData();
      ...
      if (elements.length == 1) {
        ...
        assert(
            element.renderObject != null && element.renderObject is RenderBox);
        final RenderBox renderObject = element.renderObject! as RenderBox;
        paintBounds = Rect.fromPoints(
          renderObject.localToGlobal(
              renderObject.paintBounds.topLeft - const Offset(4.0, 4.0)),
          renderObject.localToGlobal(
              renderObject.paintBounds.bottomRight + const Offset(4.0, 4.0)),
        );
  ...
  bool _shouldSkipNode(SemanticsData data) {
    ...
    if (data.label.trim().isEmpty && data.value.trim().isEmpty) return true;
    return false;
  }
  ...
  bool _isNodeOffScreen(Rect paintBounds, ui.FlutterView window) {
    return paintBounds.top < -50.0 ||
        ...
        paintBounds.bottom >
            (window.physicalSize.height * window.devicePixelRatio) + 50.0 ||
        paintBounds.right >
            (window.physicalSize.width * window.devicePixelRatio) + 50.0;
  }
  ...
  Future<Evaluation> evaluate(WidgetTester tester) async {
    ...
    Evaluation evaluateElement(Element element) {
      ...
      if (report.isEmptyRect || contrastRatio >= minimumRatio - tolerance) {
        ...
        return Evaluation.fail('$element:\nExpected contrast ratio of at least '
