...
class TestAsyncUtils {
    ...
    Future<T> completionHandler(dynamic error, StackTrace stack) {
      ...
      while (_scopeStack.isNotEmpty) {
        ...
        if (closedScope == scope)
          break;
        if (!leaked) {
          message.add(violationMessage('Asynchronous call to guarded function leaked.'));
          message.add(hintMessage('You must use "await" with all Future-returning test APIs.'));
          leaked = true;
        }
        final _StackEntry originalGuarder = _findResponsibleMethod(closedScope.creationStack, 'guard', message);
        if (originalGuarder != null) {
          message.add(violationMessage(
  ...
  static void guardSync() {
    ...
    assert(scope != null);
    final List<DiagnosticsNode> message = <DiagnosticsNode>[];
    message.add(errorMessage('Guarded function conflict.'));
    message.add(hintMessage('You must use "await" with all Future-returning test APIs.'));
    ...
    if (originalGuarder != null && collidingGuarder != null) {
      ...
      if (originalGuarder.className == null) {
        ...
        message.add(descriptionMessage(
      ...
      message.add(errorSeparator());
      message.add(StackTraceProperty(
  ...
  static void verifyAllScopesClosed() {
    if (_scopeStack.isNotEmpty) {
      final List<DiagnosticsNode> message = <DiagnosticsNode>[];
      message.add(errorMessage('Asynchronous call to guarded function leaked.'));
      message.add(hintMessage('You must use "await" with all Future-returning test APIs.'));
  ...
  static _StackEntry _findResponsibleMethod(StackTrace rawStack, String method, List<DiagnosticsNode> errors) {
      ...
      if (guardMatch != null) {
        ...
        if (index < stack.length) {
          ...
          if (callerMatch != null) {
            ...
            errors.add(errorMessage('(Unable to parse the stack frame of the method that called the method that called $_className.$method(). The stack may be incomplete or bogus.)'));
            errors.add(descriptionMessage('${stack[index]}'));
          }
        } else {
          errors.add(errorMessage('(Unable to find the stack frame of the method that called the method that called $_className.$method(). The stack may be incomplete or bogus.)'));
        }
      } else {
        errors.add(errorMessage('(Unable to parse the stack frame of the method that called $_className.$method(). The stack may be incomplete or bogus.)'));
        errors.add(descriptionMessage('${stack[index]}'));
      }
    } else {
      errors.add(errorMessage('(Unable to find the method that called $_className.$method(). The stack may be incomplete or bogus.)'));
    }
    return null;
  }
}
