...
Duration _kShortTimeout(double multiplier) => const Duration(seconds: 5) * multiplier;

/// Default timeout for awaiting an Isolate to become runnable.
Duration _kIsolateLoadRunnableTimeout(double multiplier) => const Duration(minutes: 1) * multiplier;

/// Time to delay before driving a Fuchsia module.
Duration _kFuchsiaDriveDelay(double multiplier) => const Duration(milliseconds: 500) * multiplier;

/// Default timeout for long-running RPCs.
Duration _kLongTimeout(double multiplier) => _kShortTimeout(multiplier) * 6;

/// Additional amount of time we give the command to finish or timeout remotely
/// before timing out locally.
Duration _kRpcGraceTime(double multiplier) => _kShortTimeout(multiplier) ~/ 2;

/// The amount of time we wait prior to making the next attempt to connect to
/// the VM service.
Duration _kPauseBetweenReconnectAttempts(double multiplier) => _kShortTimeout(multiplier) ~/ 5;
...
class FlutterDriver {
  ...
  static Future<FlutterDriver> connect({
    ...
    isolateReadyTimeout ??= _kIsolateLoadRunnableTimeout(timeoutMultiplier);
    ...
    if (Platform.isFuchsia && isolateNumber == null) {
      ...
      await Future<void>.delayed(_kFuchsiaDriveDelay(timeoutMultiplier));
    ...
    if (isolate.pauseEvent is! VMPauseStartEvent &&
      ...
      await Future<void>.delayed(_kShortTimeout(timeoutMultiplier) ~/ 10);
    ...
    if (isolate.pauseEvent is VMPauseStartEvent) {
      ...
      try {
        ...
        await whenServiceExtensionReady.timeout(_kLongTimeout(timeoutMultiplier) * 2);
    ...
    Future<Health> checkHealth() async {
      try {
        ...
        await enableIsolateStreams();
        await waitForServiceExtension().timeout(_kLongTimeout(timeoutMultiplier) * 2);
  ...
  Future<Map<String, dynamic>> _sendCommand(Command command) async {
    ...
    try {
      ...
      response = await _appIsolate
          ...
          .timeout(command.timeout + _kRpcGraceTime(timeoutMultiplier));
  ...
  Future<Health> checkHealth({Duration timeout}) async {
    timeout ??= _kShortTimeout(timeoutMultiplier);
  ...
  Future<List<int>> screenshot({ Duration timeout }) async {
    timeout ??= _kLongTimeout(timeoutMultiplier);
...
Future<VMServiceClientConnection> _waitAndConnect(String url) async {
  ...
  Future<VMServiceClientConnection> attemptConnection() async {
    ...
    WebSocket ws2;
    try {
      ws1 = await WebSocket.connect(uri.toString()).timeout(_kShortTimeout(connectionTimeoutMultiplier));
      ws2 = await WebSocket.connect(uri.toString()).timeout(_kShortTimeout(connectionTimeoutMultiplier));
      ...
      if (timer.elapsed < _kLongTimeout(connectionTimeoutMultiplier) * 2) {
        _log.info('Waiting for application to start');
        await Future<void>.delayed(_kPauseBetweenReconnectAttempts(connectionTimeoutMultiplier));
