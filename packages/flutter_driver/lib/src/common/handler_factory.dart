...
mixin CommandHandlerFactory {
  ...
  Future<RequestDataResult> _requestData(Command command) async {
    ...
    return RequestDataResult(
        dataHandler == null ? 'No requestData Extension registered' : await dataHandler(requestDataCommand.message));
  }
  ...
  Future<Result> _waitUntilNoPendingFrame(Command command) async {
    await _waitUntilFrame(() {
      return SchedulerBinding.instance.transientCallbackCount == 0 && !SchedulerBinding.instance.hasScheduledFrame;
    });
    return Result.empty;
  }
  ...
  Future<Result> _scroll(Command command, WidgetController prober, CreateFinderFactory finderFactory) async {
    ...
    final int totalMoves =
        scrollCommand.duration.inMicroseconds * scrollCommand.frequency ~/ Duration.microsecondsPerSecond;
  ...
  Future<Result> _scrollIntoView(Command command, CreateFinderFactory finderFactory) async {
    ...
    await Scrollable.ensureVisible(target.evaluate().single,
        duration: const Duration(milliseconds: 100), alignment: scrollIntoViewCommand.alignment);
    return Result.empty;
  }
  ...
  Future<Finder> waitForElement(Finder finder) async {
    if (_frameSync) await _waitUntilFrame(() => SchedulerBinding.instance.transientCallbackCount == 0);
