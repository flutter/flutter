...
import '../../flutter/test/widgets/navigator_utils.dart';

// Matches _kTopGapRatio in cupertino/sheet.dart.
const double _kTopGapRatio = 0.08;

void main() {
  testWidgets('Sheet route does not cover the whole screen', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets(
    'Previous route moves slight downward when sheet route is pushed',
  ...
  testWidgets(
    'If a sheet covers another sheet, then the previous sheet moves slightly upwards',
    (WidgetTester tester) async {
      final GlobalKey scaffoldKey = GlobalKey();
  ...
  testWidgets(
    'by default showCupertinoSheet does not enable nested navigation',
    (WidgetTester tester) async {
      final GlobalKey scaffoldKey = GlobalKey();
  ...
  testWidgets('useNestedNavigation handles programmatic pops', (
    WidgetTester tester,
  ) async {
    final GlobalKey scaffoldKey = GlobalKey();
  ...
  testWidgets('useNestedNavigation handles system pop gestures', (
    WidgetTester tester,
  ) async {
    final GlobalKey scaffoldKey = GlobalKey();
            ...
            builder: (BuildContext context) {
              return CupertinoPageScaffold(
                navigationBar: CupertinoNavigationBar(
                  middle: Text('Page: ${settings.name}'),
                ),
                child: Container(),
              );
            },
  ...
  testWidgets('content does not go below the bottom of the screen', (
    WidgetTester tester,
  ) async {
    final GlobalKey scaffoldKey = GlobalKey();
    ...
    expect(
      tester.getSize(find.byType(Container)).height,
      600.0 - (600.0 * _kTopGapRatio),
    );
  });

  testWidgets('nested navbars remove MediaQuery top padding', (
    WidgetTester tester,
  ) async {
    final GlobalKey scaffoldKey = GlobalKey();
    ...
    final double homeNavBardHeight = tester
        .getSize(find.byKey(appBarKey))
        .height;

    await tester.tap(find.text('Push Page 2'));
    await tester.pumpAndSettle();

    final double sheetNavBarHeight = tester
        .getSize(find.byKey(sheetBarKey))
        .height;
  ...
  testWidgets(
    'Previous route corner radius goes to same when sheet route is popped',
    (WidgetTester tester) async {
      final GlobalKey scaffoldKey = GlobalKey();
  ...
  testWidgets('Sheet transition does not interfere after popping', (
    WidgetTester tester,
  ) async {
  ...
  group('drag dismiss gesture', () {
    Widget dragGestureApp(
      GlobalKey homeScaffoldKey,
      GlobalKey sheetScaffoldKey,
    ) {
      return CupertinoApp(
        home: CupertinoPageScaffold(
          key: homeScaffoldKey,
    ...
    testWidgets('partial drag and drop does not pop the sheet', (
      WidgetTester tester,
    ) async {
      final GlobalKey homeKey = GlobalKey();
      final GlobalKey sheetKey = GlobalKey();
    ...
    testWidgets('dismissing with a drag pops all nested routes', (
      WidgetTester tester,
    ) async {
      final GlobalKey homeKey = GlobalKey();
      final GlobalKey sheetKey = GlobalKey();
    ...
    testWidgets('Popping the sheet during drag should not crash', (
      WidgetTester tester,
    ) async {
      final GlobalKey homeKey = GlobalKey();
      final GlobalKey sheetKey = GlobalKey();
    ...
    testWidgets('Sheet should not block nested scroll', (
      WidgetTester tester,
    ) async {
      final GlobalKey homeKey = GlobalKey();
      ...
      final double startPosition = tester
          .getTopLeft(find.text('Middle of Scroll'))
          .dy;
      ...
      final double endPosition = tester
          .getTopLeft(find.text('Middle of Scroll'))
          .dy;
    ...
    await tester.pumpWidget(
      CupertinoApp(
        home: CupertinoPageScaffold(
          key: scaffoldKey,
          navigationBar: const CupertinoNavigationBar(
            middle: Text('SystemUiOverlayStyle'),
          ),
          child: Center(
            child: Column(
