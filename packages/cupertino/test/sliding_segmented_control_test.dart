...
RenderBox getRenderSegmentedControl(WidgetTester tester) {
  return tester.allRenderObjects.firstWhere((RenderObject currentObject) {
        return currentObject.toStringShort().contains(
...
void main() {
  ...
  testWidgets('SegmentedControl dark mode', (WidgetTester tester) async {
    ...
    expect(
      getThumbColor(tester).value,
      CupertinoColors.systemGreen.color.value,
    );
  ...
  testWidgets('Disabled segment can be selected programmatically', (
    WidgetTester tester,
  ) async {
    const Map<int, Widget> children = <int, Widget>{
      0: Text('Child 1'),
  ...
  testWidgets(
    'Width of each segmented control segment is determined by widest widget by default',
    (WidgetTester tester) async {
      final Map<int, Widget> children = <int, Widget>{
  ...
  testWidgets('If proportionalWidth is true, the width of each segmented '
      'control segment is determined by its own content', (
    WidgetTester tester,
  ) async {
    final Map<int, Widget> children = <int, Widget>{
  ...
  testWidgets('If proportionalWidth is true, the width of each segmented '
      'control segment is updated when children change', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets(
    'If proportionalWidth is true and the overall segment control width '
    ...
    (WidgetTester tester) async {
      ...
      expect(
        ...
        (50 + 10.0 * 2) * maxAllowedTotal / originalTotal,
      );

      final Size secondChildSize = getChildSize(1);
      expect(
        secondChildSize.width,
        (100 + 10.0 * 2) * maxAllowedTotal / originalTotal,
      );

      final Size thirdChildSize = getChildSize(2);
      expect(
        thirdChildSize.width,
        (200 + 10.0 * 2) * maxAllowedTotal / originalTotal,
      );
    },
  );
  ...
  testWidgets('Non-centered taps work on proportional segments', (
    WidgetTester tester,
  ) async {
    final Map<int, Widget> children = <int, Widget>{};
    ...
    await tester.tapAt(
      centerOfChild0 +
          Offset(firstChild.width / 2 + 1 + secondChild.width + 1, 0),
    );
    expect(groupValue, 2);
  });
  ...
  testWidgets('Hit-tests report accurate local position in segments', (
    ...
    final Offset segment0GlobalOffset = tester.getTopLeft(
      find.byWidget(children[0]!),
    );
  ...
  testWidgets(
    'Hit-tests report accurate local position in proportional segments',
    (WidgetTester tester) async {
      final Map<int, Widget> children = <int, Widget>{};
      late TapDownDetails tapDownDetails;
      children[0] = GestureDetector(
  ...
  testWidgets('Thumb animation is correct when the selected segment changes', (
    ...
    final Rect initialRect = currentUnscaledThumbRect(
      tester,
      useGlobalCoordinate: true,
    );
    expect(currentThumbScale(tester), 1);
    final TestGesture gesture = await tester.startGesture(
      tester.getCenter(find.text('Child 2')),
    );
    await tester.pump();
    ...
    double centerDelta =
        tester.getSize(find.text('Child 2')).width *
        (1 - currentThumbScale(tester)) /
        2;
    expect(
      currentUnscaledThumbRect(tester, useGlobalCoordinate: true).center.dy,
      moreOrLessEquals(
        tester.getCenter(find.text('Child 2')).dy,
        epsilon: 0.01,
      ),
    );
    expect(
      currentUnscaledThumbRect(tester, useGlobalCoordinate: true).center.dx,
      moreOrLessEquals(
        tester.getCenter(find.text('Child 2')).dx - centerDelta,
        epsilon: 0.01,
      ),
    );
    ...
    expect(
      currentUnscaledThumbRect(tester, useGlobalCoordinate: true).center.dx,
      moreOrLessEquals(
        tester.getCenter(find.text('Child 2')).dx - centerDelta / 2,
        epsilon: 0.01,
      ),
    );
    ...
    expect(currentThumbScale(tester), moreOrLessEquals(0.95, epsilon: 0.01));
    centerDelta =
        tester.getSize(find.text('Child 1')).width *
        (1 - currentThumbScale(tester));
    expect(
      currentUnscaledThumbRect(tester, useGlobalCoordinate: true).center.dy,
      moreOrLessEquals(
        tester.getCenter(find.text('Child 1')).dy,
        epsilon: 0.01,
      ),
    );
    expect(
      currentUnscaledThumbRect(tester, useGlobalCoordinate: true).center.dx,
      moreOrLessEquals(
        tester.getCenter(find.text('Child 1')).dx + centerDelta / 2,
        epsilon: 0.01,
      ),
    );
  ...
  testWidgets('Thumb does not go out of bounds in animation', (
    WidgetTester tester,
  ) async {
    const Map<int, Widget> children = <int, Widget>{
    ...
    final Rect initialThumbRect = currentUnscaledThumbRect(
      tester,
      useGlobalCoordinate: true,
    );
    ...
    final Offset segmentedControlOrigin = renderSegmentedControl.localToGlobal(
      Offset.zero,
    );
    ...
    final Rect thumbRect = currentUnscaledThumbRect(
      tester,
      useGlobalCoordinate: true,
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Rect secondThumbRect = currentUnscaledThumbRect(
        tester,
        useGlobalCoordinate: true,
      );
      ...
      expect(
        secondThumbRect.center.dx,
        greaterThan(initialThumbRect.center.dx),
      );
      expect(
        secondThumbRect.center.dx,
        lessThan(tester.getCenter(find.text('B')).dx),
  ...
  testWidgets('change selection programmatically when dragging', (
    ...
    final double centerDelta =
        tester.getSize(find.text('A')).width *
        (1 - currentThumbScale(tester)) /
        2;
    expect(
      currentUnscaledThumbRect(tester, useGlobalCoordinate: true).center.dx,
      moreOrLessEquals(
        tester.getCenter(find.text('A')).dx + centerDelta,
        epsilon: 0.01,
      ),
    );
  ...
  testWidgets(
    'Disable "highlighted" segment during drag, onValueChanged is still called',
    (WidgetTester tester) async {
      const Map<int, Widget> children = <int, Widget>{
        0: Text('A'),
        1: Text('B'),
  ...
  testWidgets('CupertinoSlidingSegmentedControl defaults - no selection', (
    ...
    // Dark theme
    await tester.pumpWidget(
      buildSlidingSegmentedControl(brightness: Brightness.dark),
    );
  ...
  testWidgets(
    'CupertinoSlidingSegmentedControl defaults - group value is not null',
    (WidgetTester tester) async {
      const Map<int, Widget> children = <int, Widget>{
        0: Text('A'),
        1: Text('BB'),
      ...
      await expectLater(
        find.byType(CupertinoSlidingSegmentedControl<int>),
        matchesGoldenFile(
          'cupertino_sliding_segmented_control.with_selection.light_theme.png',
        ),
      );
      ...
      await expectLater(
        find.byType(CupertinoSlidingSegmentedControl<int>),
        matchesGoldenFile(
          'cupertino_sliding_segmented_control.with_selection.dark_theme.png',
        ),
      );
    },
  );

  testWidgets('CupertinoSlidingSegmentedControl defaults - disabled', (
    WidgetTester tester,
  ) async {
    const Map<int, Widget> children = <int, Widget>{
      0: Text('A'),
    ...
    await expectLater(
      find.byType(CupertinoSlidingSegmentedControl<int>),
      matchesGoldenFile(
        'cupertino_sliding_segmented_control.disabled.light_theme.png',
      ),
    );
    ...
    await expectLater(
      find.byType(CupertinoSlidingSegmentedControl<int>),
      matchesGoldenFile(
        'cupertino_sliding_segmented_control.disabled.dark_theme.png',
      ),
    );
  });
