...
void main() {
  ...
  testWidgets('largeTitle is aligned with asymmetrical actions', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      const CupertinoApp(
  ...
  testWidgets('largeTitle still aligned with back button', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      const CupertinoApp(
        home: CupertinoNavigationBar.large(largeTitle: Text('Title')),
      ),
    );
  ...
  testWidgets(
    "Background doesn't add blur effect when no content is scrolled under",
  ...
  testWidgets(
    'Blur affect is disabled when enableBackgroundFilterBlur is false',
    (WidgetTester test) async {
      await test.pumpWidget(
        CupertinoApp(
          theme: const CupertinoThemeData(brightness: Brightness.light),
          ...
          CupertinoSheetRoute<void>(
            builder: (BuildContext context) {
              return const CupertinoPageScaffold(
                child: CustomScrollView(
                  slivers: <Widget>[
                    CupertinoSliverNavigationBar(largeTitle: Text('Page 3')),
                  ],
                ),
              );
            },
  ...
  testWidgets('Large CupertinoNavigationBar has semantics', (
    WidgetTester tester,
  ) async {
    final SemanticsTester semantics = SemanticsTester(tester);
  ...
  testWidgets('Persistent nav bar text scaling clamps to upper bound', (
    WidgetTester tester,
  ) async {
    setWindowToPortrait(tester);
  ...
  testWidgets('Nav bar text scaling clamps to lower bounds', (
    WidgetTester tester,
  ) async {
    setWindowToPortrait(tester);
    ...
    for (final String text in <String>['leading', 'middle', 'trailing']) {
      expect(
        barItems.any(
          (RichText t) =>
              t.text.toPlainText() == text &&
              t.textScaler == TextScaler.noScaling,
        ),
        isTrue,
      );
    }
  ...
  testWidgets('Active search view cancel button does not text scale', (
    WidgetTester tester,
  ) async {
    setWindowToPortrait(tester);
    const double scaleFactor = 10.0;
  ...
  testWidgets('NavigationBarBottomMode.automatic mode for bottom', (
    WidgetTester tester,
  ) async {
    ...
    expect(
      tester.getBottomLeft(largeTitleFinder).dy,
      persistentHeight + largeTitleHeight,
    );
    ...
    expect(
      tester.getBottomLeft(bottomFinder).dy -
          tester.getTopLeft(bottomFinder).dy,
      0.0,
    );
  ...
  testWidgets('NavigationBarBottomMode.always mode for bottom', (
    WidgetTester tester,
  ) async {
    const double persistentHeight = 44.0;
    const double largeTitleHeight = 44.0;
    ...
    expect(
      tester.getBottomLeft(bottomFinder).dy,
      persistentHeight + bottomHeight,
    );
  });

  testWidgets(
    'Disallow providing a bottomMode without a corresponding bottom',
    (WidgetTester tester) async {
      expect(
        () => const CupertinoSliverNavigationBar(
          bottom: PreferredSize(
  ...
  testWidgets(
    'Overscroll when stretched does not resize bottom in automatic mode',
    (WidgetTester tester) async {
      const double bottomHeight = 10.0;
      ...
      expect(
        tester.getTopLeft(bottomFinder).dy,
        greaterThan(bottomDisplacement),
      );
      expect(
        tester.getBottomLeft(bottomFinder).dy -
            tester.getTopLeft(bottomFinder).dy,
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await scrollGesture.moveBy(
        const Offset(0.0, -(largeTitleHeight / 2) - 1),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await scrollGesture2.moveBy(
        const Offset(0.0, -(bottomHeight + largeTitleHeight / 2)),
      );
      await scrollGesture2.up();
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        ...
        largeTitleHeight + (bottomHeight / 2) + 1,
      );
      expect(
        tester.getBottomLeft(bottomFinder).dy -
            tester.getTopLeft(bottomFinder).dy,
  ...
  testWidgets('CupertinoNavigationBar with bottom widget', (
    ...
    final CupertinoNavigationBar navBar = tester.widget<CupertinoNavigationBar>(
      navBarFinder,
    );
  ...
  testWidgets(
    'CupertinoSliverNavigationBar.search field collapses nav bar on tap',
    (WidgetTester tester) async {
      setWindowToPortrait(tester);
      await tester.pumpWidget(
        const CupertinoApp(
      ...
      await tester.pump(
        const Duration(microseconds: 1) + const Duration(milliseconds: 300),
      );
  ...
  testWidgets('CupertinoSliverNavigationBar.search golden tests', (
    WidgetTester tester,
  ) async {
    setWindowToPortrait(tester);
    await tester.pumpWidget(
