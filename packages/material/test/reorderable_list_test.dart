...
void main() {
    ...
    group('in vertical mode', () {
      testWidgets(
        ...
        (WidgetTester tester) async {
          ...
          final List<String> currentOriginalListItems = originalListItems
              .take(1)
              .toList();
      ...
      testWidgets('properly determines the vertical drop area extents', (
        ...
        drag = await tester.startGesture(
          tester.getCenter(find.text('Tall item')),
        );
        await tester.pump(kLongPressTimeout + kPressTimeout);
        await tester.pumpAndSettle();
        expect(getListHeight(), kDraggingListHeight);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final double item0Height = tester
          .getSize(find.text('0').hitTestable())
          .height;
      final double item1Height = tester
          .getSize(find.text('1').hitTestable())
          .height;
      final double item2Height = tester
          .getSize(find.text('2').hitTestable())
          .height;
  ...
  testWidgets('ReorderableListView auto scrolls speed is configurable', (
    ...
    Future<double> pumpListAndDrag({
      ...
      await pumpFor(
        ...
        interval: Duration(
          milliseconds: (1000 / autoScrollerVelocityScalar).round(),
        ),
      );
      await drag.up();
  ...
  testWidgets(
    'Mouse cursor behavior on the drag handle',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
      ...
      await gesture.addPointer(
        location: tester.getCenter(find.byIcon(Icons.drag_handle).first),
      );
      await tester.pump();
      expect(
