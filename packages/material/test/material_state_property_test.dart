...
void main() {
  test('MaterialStateProperty.resolveWith()', () {
    ...
    expect(
      value.resolve(<MaterialState>{MaterialState.hovered}),
      MaterialState.hovered,
    );
    expect(
      value.resolve(<MaterialState>{MaterialState.focused}),
      MaterialState.focused,
    );
    expect(
      value.resolve(<MaterialState>{MaterialState.pressed}),
      MaterialState.pressed,
    );
    expect(
      value.resolve(<MaterialState>{MaterialState.dragged}),
      MaterialState.dragged,
    );
    expect(
      value.resolve(<MaterialState>{MaterialState.selected}),
      MaterialState.selected,
    );
    expect(
      value.resolve(<MaterialState>{MaterialState.disabled}),
      MaterialState.disabled,
    );
    expect(
      value.resolve(<MaterialState>{MaterialState.error}),
      MaterialState.error,
    );
  });

  test('MaterialStateProperty.all()', () {
    final MaterialStateProperty<int> value = MaterialStateProperty.all<int>(
      123,
    );
  ...
  test('MaterialStatePropertyAll', () {
    const MaterialStatePropertyAll<int> value = MaterialStatePropertyAll<int>(
      123,
    );
  ...
  test('toString formats correctly', () {
    ...
    expect(
      colorProperty.toString(),
      equals('WidgetStatePropertyAll(${const Color(0xffffffff)})'),
    );
  ...
  test("Can interpolate between two MaterialStateProperty's", () {
    const MaterialStateProperty<TextStyle?> textStyle1 =
        MaterialStatePropertyAll<TextStyle?>(TextStyle(fontSize: 14.0));
    const MaterialStateProperty<TextStyle?> textStyle2 =
        MaterialStatePropertyAll<TextStyle?>(TextStyle(fontSize: 20.0));
