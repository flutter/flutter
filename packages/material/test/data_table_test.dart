...
void main() {
  testWidgets('DataTable control test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildTable(sortColumnIndex: 1))),
    );
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Material(
          child: buildTable(sortColumnIndex: 1, sortAscending: false),
        ),
      ),
    );
    await tester.pumpAndSettle(const Duration(milliseconds: 200));
  ...
  testWidgets('DataTable control test - tristate', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Material(
          child: buildTable(<bool>[true, true, false], disabledIndex: 2),
        ),
      ),
    );
    await tester.tap(find.byType(Checkbox).first);
  ...
  testWidgets('DataTable control test - no checkboxes', (
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildTable(checkboxes: true))),
    );
  ...
  testWidgets('DataTable overflow test - header', (WidgetTester tester) async {
    ...
    expect(
      tester.renderObject<RenderBox>(find.byType(Text).first).size.width,
      greaterThan(800.0),
    );
    expect(
      tester.renderObject<RenderBox>(find.byType(Row).first).size.width,
      greaterThan(800.0),
    );
    expect(
          ...
          child: DataTable(
            columns: <DataColumn>[
              DataColumn(
                label: Text(
                  'X ' * 2000,
                ), // has soft wrap points, but they should be ignored
              ),
            ],
            rows: const <DataRow>[
              DataRow(cells: <DataCell>[DataCell(Text('X'))]),
  ...
  testWidgets('DataTable overflow test', (WidgetTester tester) async {
    ...
    expect(
      tester.takeException(),
      isNull,
    ); // cell overflows table, but text doesn't overflow cell
  });
  ...
  testWidgets('DataTable sort indicator orientation', (
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildTable(sortAscending: false))),
    );
  ...
  testWidgets('DataTable custom row height', (WidgetTester tester) async {
    ...
    expect(
      tester.getSize(findFirstContainerFor('Frozen yogurt')).height,
  ...
  testWidgets('DataTable custom row height one row taller than others', (
    ...
    final double singleLineRowHeight = tester
        .getSize(findFirstContainerFor('Data'))
        .height;
    final double multilineRowHeight = tester
        .getSize(findFirstContainerFor(multilineText))
        .height;
  ...
  testWidgets('DataTable custom horizontal padding - checkbox', (
    ...
    // DEFAULT VALUES
    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildDefaultTable())),
    );
  ...
  testWidgets('DataTable column heading cell - with and without sorting', (
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildTable(sortEnabled: false))),
    );

    {
      ...
      expect(
        find.descendant(of: nameCell, matching: find.byType(Icon)),
        findsNothing,
      );
    }
  ...
  testWidgets('Material2 - DataRow renders custom colors when pressed', (
    ...
    final TestGesture gesture = await tester.startGesture(
      tester.getCenter(find.text('Content1')),
    );
    await tester.pump(
  ...
  testWidgets('DataTable clip behavior', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: buildTable(clipBehavior: Clip.none)),
    );
    ...
    await tester.pumpWidget(
      MaterialApp(home: buildTable(clipBehavior: Clip.hardEdge)),
    );
  ...
  testWidgets("DataRow cursor doesn't update checkbox cursor", (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('DataTable text styles are merged with default text style', (
    ...
    final TextStyle? headingTextStyle = _getTextRenderObject(
      tester,
      'Header 1',
    ).text.style;
    expect(headingTextStyle, defaultTextStyle.style);

    final TextStyle? dataTextStyle = _getTextRenderObject(
      tester,
      'Data 1',
    ).text.style;
  ...
  testWidgets('DataColumn label can be centered', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      buildTable(headingRowAlignment: MainAxisAlignment.center),
    );
  ...
  testWidgets('DataTable with custom column widths - checkbox', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('DataTable with custom column widths - no checkbox', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('DataTable has correct roles in semantics', (
    WidgetTester tester,
  ) async {
        ...
        home: Scaffold(
          body: DataTable(
            ...
            rows: const <DataRow>[
              DataRow(
                cells: <DataCell>[
                  DataCell(Text('Data Cell 1')),
                  DataCell(Text('Data Cell 2')),
                ],
              ),
            ],
          ),
