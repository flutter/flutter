...
void main() {
  ...
  for (final PageTransitionsBuilder pageTransitionsBuilder
      in <PageTransitionsBuilder>[
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          _findPredictiveBackPageTransition(pageTransitionsBuilder),
          findsNothing,
        );
        expect(
          _findFallbackPageTransition(pageTransitionsBuilder),
          findsOneWidget,
        );
        ...
        final ByteData startMessage = const StandardMethodCodec()
            .encodeMethodCall(
        ...
        final ByteData updateMessage = const StandardMethodCodec()
            .encodeMethodCall(
        ...
        final ByteData commitMessage = const StandardMethodCodec()
            .encodeMethodCall(const MethodCall('commitBackGesture'));
        await binding.defaultBinaryMessenger.handlePlatformMessage(
          'flutter/backgesture',
          commitMessage,
          (ByteData? _) {},
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final ByteData commitMessage = const StandardMethodCodec()
            .encodeMethodCall(const MethodCall('cancelBackGesture'));
        await binding.defaultBinaryMessenger.handlePlatformMessage(
          'flutter/backgesture',
          commitMessage,
          (ByteData? _) {},
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        // gets it.
        final ByteData updateMessage2 = const StandardMethodCodec()
            .encodeMethodCall(
              const MethodCall('updateBackGestureProgress', <String, dynamic>{
        ...
        final Offset update2PageBOffset = tester.getTopLeft(
          find.text('page b'),
        );
    ...
    testWidgets(
      'two back gestures back to back dismiss two routes',
      (WidgetTester tester) async {
        final Map<String, WidgetBuilder> routes = <String, WidgetBuilder>{
          '/': (BuildContext context) => Material(
            child: TextButton(
        ...
        // animating out.
        final ByteData startMessage2 = const StandardMethodCodec()
            .encodeMethodCall(
              const MethodCall('startBackGesture', <String, dynamic>{
                'touchOffset': <double>[5.0, 300.0],
                'progress': 0.0,
        ...
        // Commit the system back gesture.
        final ByteData commitMessage2 = const StandardMethodCodec()
            .encodeMethodCall(const MethodCall('commitBackGesture'));
        await binding.defaultBinaryMessenger.handlePlatformMessage(
          'flutter/backgesture',
          commitMessage2,
...
String _getTransitionsString(PageTransitionsBuilder pageTransitionsBuilder) {
  return switch (pageTransitionsBuilder) {
    ...
    _ => throw UnsupportedError(
      'Unsupported subclass of PageTransitionsBuilder',
    ),
  };
}

Finder _findPredictiveBackPageTransition(
  ...
  return find.descendant(
    ...
    matching: find.byWidgetPredicate(
      (Widget w) =>
          '${w.runtimeType}' == _getTransitionsString(pageTransitionsBuilder),
    ),
  );
}
...
Finder _findFallbackPageTransition(
  ...
  final String fallback = switch (pageTransitionsBuilder) {
    final PredictiveBackPageTransitionsBuilder _ =>
      '_FadeForwardsPageTransition',
    final PredictiveBackFullscreenPageTransitionsBuilder _ =>
      '_ZoomPageTransition',
    _ => throw TypeError(),
  };
  return find.descendant(
    of: find.byType(MaterialApp),
    matching: find.byWidgetPredicate(
      (Widget w) => '${w.runtimeType}' == fallback,
    ),
  );
}
