...
void main() {
  ...
  testWidgets('Scaffold shows scrim when bottom sheet becomes dominant', (
    ...
    double getExpectedOpacity(double visValue) => math.max(
      ...
      kMaxBottomSheetScrimOpacity - visValue,
    );
  ...
  testWidgets('Scaffold uses bottomSheetScrimBuilder if defined', (
    WidgetTester tester,
  ) async {
    final DraggableScrollableController draggableController =
        DraggableScrollableController();
    ...
    double getOpacity() =>
        tester.firstWidget<ColoredBox>(findScrim()).color.opacity;
    ...
    for (double i = 0, extent = i / 10; i <= 10; i++, extent = i / 10) {
      ...
      if (extentRemaining < kBottomSheetDominatesPercentage) {
        final double animationValue =
            1 - extentRemaining / kBottomSheetDominatesPercentage;
              ...
              SliverList(
                delegate: SliverChildListDelegate(
                  List<Widget>.generate(
                    20,
                    (int index) =>
                        SizedBox(height: 100.0, child: Text('$index')),
                  ),
                ),
              ),
            ],
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(
        buildStatusBarTestApp(debugDefaultTargetPlatformOverride),
      );
  ...
  testWidgets('Bottom sheet cannot overlap app bar', (
    ...
    final Offset sheetTopRight = sheetBox.localToGlobal(
      sheetBox.size.topRight(Offset.zero),
    );
  ...
  testWidgets('Persistent bottom buttons can apply decoration', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      Directionality(
    ...
    final Decoration decoration = tester
        .widget<Container>(persistentFooter)
        .decoration!;
  ...
  group('back arrow', () {
    Future<void> expectBackIcon(
      ...
      final Map<String, WidgetBuilder> routes = <String, WidgetBuilder>{
        ...
        '/scaffold': (_) =>
            Scaffold(appBar: AppBar(), body: const Text('Scaffold')),
      };
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        await expectBackIcon(
          tester,
          kIsWeb ? Icons.arrow_back : Icons.arrow_back_ios_new_rounded,
        );
      },
      variant: const TargetPlatformVariant(<TargetPlatform>{
        TargetPlatform.iOS,
  ...
  testWidgets('Open drawer hides underlying semantics tree', (
    ...
    expect(
      semantics,
      isNot(includesNodeWith(label: persistentFooterButtonLabel)),
    );
    expect(semantics, isNot(includesNodeWith(label: bottomNavigationBarLabel)));
    expect(
      semantics,
      isNot(includesNodeWith(label: floatingActionButtonLabel)),
    );
  ...
  testWidgets('Scaffold and extreme window padding', (
    ...
    expect(
      tester.getRect(find.byKey(appBar)),
    ...
    expect(
      tester.getRect(find.byKey(insideAppBar)),
  ...
  group('ScaffoldGeometry', () {
    testWidgets('bottomNavigationBar', (WidgetTester tester) async {
      ...
      final _GeometryListenerState listenerState = tester.state(
        find.byType(_GeometryListener),
      );
    ...
    testWidgets('floatingActionButton', (WidgetTester tester) async {
      ...
      final RenderBox floatingActionButtonBox = tester.renderObject(
        find.byKey(key),
      );
      final _GeometryListenerState listenerState = tester.state(
        find.byType(_GeometryListener),
      ...
      final Rect fabRect =
          floatingActionButtonBox.localToGlobal(Offset.zero) &
    ...
    testWidgets('change notifications', (WidgetTester tester) async {
      ...
      expect(
        listenerState.numNotifications,
    ...
    testWidgets('Drawer opens correctly with padding from MediaQuery (RTL)', (
      ...
      final double scaffoldWidth = tester
          .renderObject<RenderBox>(find.byType(Scaffold))
          .size
          .width;
      ScaffoldState scaffoldState = tester.state(find.byType(Scaffold));
      expect(scaffoldState.isDrawerOpen, false);
  ...
  testWidgets('Nested scaffold body insets', (WidgetTester tester) async {
    ...
    Widget buildFrame(
      bool? innerResizeToAvoidBottomInset,
  ...
  group('FlutterError control test', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final Offset defaultOffset = tester.getCenter(
        ...
        final Offset extendedBodyOffset = tester.getCenter(
          find.byType(FloatingActionButton),
        );
                    ...
                    onPressed: () {
                      if (controller == null) {
                        controller = showBottomSheet(
                          context: context,
                          builder: (_) =>
                              Container(key: bottomSheetKey, height: 200),
                        );
                      } else {
                        controller!.close();
                        controller = null;
  ...
  testWidgets('Default ScaffoldMessenger can access ambient theme', (
    WidgetTester tester,
  ) async {
    ...
    final ThemeData messengerTheme = Theme.of(
  ...
  testWidgets('ScaffoldMessenger showSnackBar animation can be customized', (
    ...
    ); // Advance the animation by 300ms.
    ...
    ); // Advance the animation by 600ms.
  ...
  testWidgets('Updated snackBarAnimationStyle updates snack bar animation', (
    ...
    ); // Advance the animation by 400ms.
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        buildSnackBar(
          const AnimationStyle(reverseDuration: Duration(milliseconds: 400)),
        ),
      );
      ...
      ); // Advance the animation by 200ms.
  ...
  testWidgets('FAB default entrance and exit animations can be disabled', (
    ...
    await tester.tap(
      find.widgetWithText(ElevatedButton, 'Update FAB Location'),
    );
    await tester.pump();
...
class _GeometryListenerState extends State<_GeometryListener> {
  ...
  void didChangeDependencies() {
    ...
    final ValueListenable<ScaffoldGeometry> newListenable = Scaffold.geometryOf(
      context,
    );
