...
void main() {
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(home: SelectionArea(child: Text('abc'))),
      );
  ...
  testWidgets('Does not crash when long pressing on padding after dragging', (
    ...
    final TestGesture dragging = await tester.startGesture(
      const Offset(10, 10),
    );
    ...
    final TestGesture longpress = await tester.startGesture(
      const Offset(20, 20),
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        find.text(fakeAction1Label),
        areTextActionsSupported ? findsOneWidget : findsNothing,
      );
      expect(
        find.text(fakeAction2Label),
        areTextActionsSupported ? findsOneWidget : findsNothing,
      );
    },
    variant: TargetPlatformVariant.all(),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pump(const Duration(milliseconds: 500));
      await gesture.up();
      final List<TextBox> boxes = paragraph2.getBoxesForSelection(
      ...
      final Offset handlePos = paragraph2.localToGlobal(
        boxes[0].toRect().bottomRight,
      );
      await gesture.down(handlePos);
      await gesture.moveTo(
        textOffsetToPosition(paragraph2, 11) +
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final RenderParagraph paragraph = tester.renderObject<RenderParagraph>(
        find.descendant(
          of: find.text('one two three four five'),
          matching: find.byType(RichText),
        ),
      );
      final TestGesture gesture = await tester.startGesture(
      ...
      // This is the position of the selection handle displayed at the end.
      final Offset endHandlePos = paragraph.localToGlobal(
        boxes[0].toRect().bottomRight,
      );
      await gesture.down(endHandlePos);
      await gesture.moveTo(
        textOffsetToPosition(paragraph, 22) +
      ...
      expect(
        paragraph.selections[0],
        const TextSelection(baseOffset: 8, extentOffset: 22),
      );

      // Attempt to move the start handle while still touching the end handle.
      final Offset startHandlePos = paragraph.localToGlobal(
        boxes[0].toRect().bottomLeft,
      );
      final TestGesture startHandleGesture = await tester.startGesture(
        startHandlePos,
      );
  ...
  testWidgets(
    ...
    skip:
        !kIsWeb, // [intended] on native both selection handles can be dragged at a time.
  );
  ...
  testWidgets(
    ...
    variant: TargetPlatformVariant.only(TargetPlatform.android),
    skip:
        kIsWeb, // [intended] on web only one selection handle can be dragged at a time.
  );
}
