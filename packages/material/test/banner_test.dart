...
void main() {
  testWidgets('MaterialBanner properties are respected', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets(
    'MaterialBanner properties are respected when presented by ScaffoldMessenger',
  ...
  testWidgets('Actions laid out below content if more than one action', (
    ...
    final Offset contentBottomLeft = tester.getBottomLeft(
      find.text(contentText),
    );
  ...
  testWidgets('Actions laid out beside content if only one action', (
    WidgetTester tester,
  ) async {
    const String contentText = 'Content';
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Offset actionsTopRight = tester.getTopRight(
  ...
  testWidgets('material banner content can scale and has maxScaleFactor', (
      ...
      return MaterialApp(
        home: MediaQuery(
          ...
          child: MaterialBanner(
            ...
            actions: <Widget>[
              TextButton(child: const Text('B'), onPressed: () {}),
            ],
          ),
        ),
      );
    ...
    expect(
      _sizeAlmostEqual(tester.getSize(find.text(label)), const Size(21.25, 30)),
      true,
    );
  ...
  group('MaterialBanner elevation', () {
    Widget buildBanner(
      Key tapTarget, {
      double? elevation,
      double? themeElevation,
    }) {
      return MaterialApp(
        theme: ThemeData(
          bannerTheme: MaterialBannerThemeData(elevation: themeElevation),
        ),
        home: Scaffold(
          body: Builder(
            builder: (BuildContext context) {
    ...
    testWidgets('Elevation defaults to 0', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        buildBanner(tapTarget, elevation: 3.0, themeElevation: 6.0),
      );
      await tester.tap(find.byKey(tapTarget));
      await tester.pumpAndSettle();
    ...
    testWidgets('Uses elevation of MaterialBannerTheme by default', (
      WidgetTester tester,
    ) async {
      const Key tapTarget = Key('tap-target');
    ...
    testWidgets('Scaffold body is pushed down if elevation is 0', (
      WidgetTester tester,
    ) async {
      const Key tapTarget = Key('tap-target');
      ...
      final Offset bannerBottomLeft = tester.getBottomLeft(
  ...
  testWidgets('MaterialBanner twice test', (WidgetTester tester) async {
    ...
    expect(find.text('banner2'), findsOneWidget);
    await tester.tap(
      find.byKey(dismissTarget),
    ); // reverse animation is scheduled
    await tester.pump(); // begin animation
    expect(find.text('banner1'), findsNothing);
    expect(find.text('banner2'), findsOneWidget);
    await tester.pump(
  ...
  testWidgets(
    'ScaffoldMessenger does not duplicate a MaterialBanner when presenting a SnackBar.',
    (WidgetTester tester) async {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Offset bannerTopRight = tester.getTopRight(
        find.byType(MaterialBanner),
      );
      expect(
        actionsTopRight.dx + 8,
        bannerTopRight.dx,
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Offset actionsTopLeft = tester.getTopLeft(find.byType(OverflowBar));
      final Offset bannerTopLeft = tester.getTopLeft(
        find.byType(MaterialBanner),
      );
      expect(
        actionsTopLeft.dx - 8,
  ...
  testWidgets('Actions laid out below content if forced override', (
    WidgetTester tester,
  ) async {
    const String contentText = 'Content';
  ...
  testWidgets('Action widgets layout when presented by ScaffoldMessenger', (
                ...
                return GestureDetector(
                  ...
                  onTap: () {
                    ScaffoldMessenger.of(context).showMaterialBanner(
                      MaterialBanner(
                        content: const SizedBox(width: 100, height: 100),
                        actions: List<Widget>.generate(actionCount, (
                          int index,
                        ) {
                          if (index == 0) {
                            return SizedBox(
    ...
    final Finder dismissTarget = find.byKey(
      const ValueKey<String>('dismiss-target'),
    );
    final Finder action0 = find.byKey(const ValueKey<int>(0));
    final Finder action1 = find.byKey(const ValueKey<int>(1));
    final Finder action2 = find.byKey(const ValueKey<int>(2));
  ...
  testWidgets('Action widgets layout with overflow', (
    WidgetTester tester,
  ) async {
    // This regression test ensures that the action widgets layout matches what
    // it was, before ButtonBar was replaced by OverflowBar.
    ...
    for (int index = 0; index < actionCount; index += 1) {
      expect(
        tester.getTopLeft(find.byKey(ValueKey<int>(index))),
        Offset(592, 134.0 + index * 10),
      );
    }
    ...
    for (int index = 0; index < actionCount; index += 1) {
      expect(
        tester.getTopLeft(find.byKey(ValueKey<int>(index))),
        Offset(8, 134.0 + index * 10),
      );
    }
  });

  testWidgets(
    'Action widgets layout with overflow when presented by ScaffoldMessenger',
    (WidgetTester tester) async {
      // This regression test ensures that the action widgets layout matches what
      // it was, before ButtonBar was replaced by OverflowBar.
  ...
  testWidgets('[overflowAlignment] test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      buildFrame(TextDirection.ltr, OverflowBarAlignment.start),
    );
    for (int index = 0; index < actionCount; index += 1) {
      expect(
        tester.getTopLeft(find.byKey(ValueKey<int>(index))),
    ...
    await tester.pumpWidget(
      buildFrame(TextDirection.ltr, OverflowBarAlignment.center),
    );
    for (int index = 0; index < actionCount; index += 1) {
      expect(
        tester.getTopLeft(find.byKey(ValueKey<int>(index))),
        Offset(300, 134.0 + index * 10),
      );
    }

    await tester.pumpWidget(
      buildFrame(TextDirection.ltr, OverflowBarAlignment.end),
    );
    for (int index = 0; index < actionCount; index += 1) {
      expect(
        tester.getTopLeft(find.byKey(ValueKey<int>(index))),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final ScaffoldMessengerState scaffoldMessengerState = tester
  ...
  testWidgets(
    'minimumActionBarHeight is respected when presented by ScaffoldMessenger',
    (WidgetTester tester) async {
      const Key tapTarget = Key('tap-target');
      const double minActionBarHeight = 20.0;
      await tester.pumpWidget(
