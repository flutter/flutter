...
void main() {
  ...
  final List<DropdownMenuEntry<TestMenu>> menuChildrenWithIcons =
      <DropdownMenuEntry<TestMenu>>[];
  ...
  group('Item style', () {
    ...
    final List<DropdownMenuEntry<TestMenu>> styledMenuEntries =
        <DropdownMenuEntry<TestMenu>>[];
    for (final DropdownMenuEntry<TestMenu> entryWithIcons
        in menuChildrenWithIcons) {
        ...
        find
            .descendant(
              ...
              matching: find.byElementPredicate(
                (Element element) =>
                    element.renderObject.runtimeType.toString() ==
                    '_RenderInkFeatures',
              ),
            )
            .last,
    ...
    testWidgets('defaults are correct', (WidgetTester tester) async {
      ...
      final Material selectedButtonMaterial = getButtonMaterial(
        tester,
        selectedItem.label,
      );
      expect(
        selectedButtonMaterial.color,
        themeData.colorScheme.onSurface.withOpacity(0.12),
      );
      expect(
        selectedButtonMaterial.textStyle?.color,
      ...
      final Material nonSelectedButtonMaterial = getButtonMaterial(
        tester,
        nonSelectedItem.label,
      );
      expect(nonSelectedButtonMaterial.color, Colors.transparent);
      expect(
        nonSelectedButtonMaterial.textStyle?.color,
        themeData.colorScheme.onSurface,
      );
      expect(
      ...
      await gesture.moveTo(
        tester.getCenter(findMenuItemButton(selectedItem.label)),
      );
      await tester.pump();
      ...
      expect(
        ...
        paints..rect(
          color: themeData.colorScheme.onSurface.withOpacity(0.1).withAlpha(0),
        ),
      );

      // Hover a non-selected item.
      await gesture.moveTo(
        tester.getCenter(findMenuItemButton(nonSelectedItem.label)),
      );
      await tester.pump();
      ...
      expect(
        ...
        paints..rect(
          color: themeData.colorScheme.onSurface.withOpacity(0.08).withAlpha(0),
        ),
      );
    });

    testWidgets('can be overridden at application theme level', (
      WidgetTester tester,
    ) async {
      const TestMenu selectedItem = TestMenu.mainMenu3;
      const TestMenu nonSelectedItem = TestMenu.mainMenu2;
      ...
      await tester.pumpWidget(
        MaterialApp(
          theme: ThemeData(
            menuButtonTheme: MenuButtonThemeData(style: customButtonStyle),
          ),
          home: Scaffold(
            body: DropdownMenu<TestMenu>(
              initialSelection: selectedItem,
    ...
    testWidgets('can be overridden at menu entry level', (
      WidgetTester tester,
    ) async {
      const TestMenu selectedItem = TestMenu.mainMenu3;
      const TestMenu nonSelectedItem = TestMenu.mainMenu2;
    ...
    testWidgets('defined at menu entry level takes precedence', (
      WidgetTester tester,
    ) async {
      const TestMenu selectedItem = TestMenu.mainMenu3;
      const TestMenu nonSelectedItem = TestMenu.mainMenu2;
      ...
      await tester.pumpWidget(
        MaterialApp(
          theme: ThemeData(
            menuButtonTheme: MenuButtonThemeData(style: singleColorButtonStyle),
          ),
          home: Scaffold(
            body: DropdownMenu<TestMenu>(
              initialSelection: selectedItem,
    ...
    testWidgets(
      'defined at menu entry level and application level are merged',
  ...
  testWidgets('The width is determined by the menu entries', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets(
    'The width is determined by the label when it is longer than menu entries',
    (WidgetTester tester) async {
      ...
      const double labelPadding =
          8.0; // See RenderEditable.floatingCursorAddedMargin.
  ...
  testWidgets('expandedInsets can use EdgeInsets or EdgeInsetsDirectional', (
    ...
    await tester.pumpWidget(
      buildMenuAnchor(
        expandedInsets: const EdgeInsetsDirectional.only(
          start: 35.0,
          top: 50.0,
          end: 20.0,
        ),
  ...
  testWidgets('The text in the menu button should be aligned with the text of '
    ...
    expect(
      updatedLabelTopLeft.dx,
      equals(iconWidth + leadingIconToInputPadding),
    );
    ...
    final Finder largeLeadingIcon = find
        .widgetWithIcon(SizedBox, Icons.search)
        .last;
    ...
    expect(
      updatedLabelTopLeft1.dx,
      equals(largeIconWidth + leadingIconToInputPadding),
    );
  });
  ...
  testWidgets('The text in the menu button should be aligned with the text of '
    ...
    expect(
      ...
      equals(
        updatedDropdownMenuTopRight.dx -
            largeIconWidth -
            leadingIconToInputPadding,
      ),
    );
  });
  ...
  testWidgets('The icon in the menu button should be aligned with the icon of '
    ...
    final Finder dropdownIcon = find
        .descendant(
          of: find.byIcon(Icons.search).first,
    ...
    await tester.tap(find.byType(DropdownMenu<TestMenu>));
    await tester.pumpAndSettle();
    final Finder itemLeadingIcon = find
        .byKey(leadingIconKey(TestMenu.mainMenu0))
        .last;

    expect(
      tester.getRect(dropdownIcon).left,
      tester.getRect(itemLeadingIcon).left,
    );
  });
  ...
  testWidgets('The icon in the menu button should be aligned with the icon of '
    ...
    expect(
      tester.getRect(dropdownIcon).right,
      tester.getRect(itemLeadingIcon).right,
    );
  });
  ...
  testWidgets('DropdownMenu has default trailing icon button', (
    ...
    final Finder iconButton = find
        .widgetWithIcon(IconButton, Icons.arrow_drop_down)
        .first;
  ...
  testWidgets('InputDecorationTheme.isCollapsed reduces height', (
    WidgetTester tester,
  ) async {
    final ThemeData themeData = ThemeData();
  ...
  testWidgets('DropdownMenu can customize trailing icon button', (
    ...
    final Finder iconButton = find
        .widgetWithIcon(IconButton, Icons.ac_unit)
        .first;
    expect(iconButton, findsOneWidget);
  ...
  testWidgets('Down key can highlight the menu item while focused', (
    WidgetTester tester,
  ) async {
    final ThemeData themeData = ThemeData();
    await tester.pumpWidget(
  ...
  testWidgets('Up key can highlight the menu item while focused', (
    WidgetTester tester,
  ) async {
    final ThemeData themeData = ThemeData();
    await tester.pumpWidget(
  ...
  testWidgets('Left and right keys can move text field selection', (
    WidgetTester tester,
  ) async {
      ...
      MaterialApp(
        ...
        home: Scaffold(
          body: DropdownMenu<TestMenu>(
            ...
            enableFilter: true,
            filterCallback:
                (List<DropdownMenuEntry<TestMenu>> entries, String filter) {
                  return entries
                      .where(
                        (DropdownMenuEntry<TestMenu> element) =>
  ...
  testWidgets(
    'Keyboard navigation does not throw when no entries match the filter',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
  ...
  testWidgets('Keyboard navigation only traverses filtered entries', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController();
    addTearDown(controller.dispose);
  ...
  testWidgets(
    ...
    'another label text using customized search algorithm',
    (WidgetTester tester) async {
      final ThemeData themeData = ThemeData();
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Scaffold(
            body: DropdownMenu<int>(
              requestFocusOnTap: true,
              searchCallback:
                  (List<DropdownMenuEntry<int>> entries, String query) {
                    if (query.isEmpty) {
                      return null;
                    }
  ...
  // Regression test for https://github.com/flutter/flutter/issues/152375.
  testWidgets(
    'Searching can highlight entry after keyboard navigation while focused',
    (WidgetTester tester) async {
      final ThemeData themeData = ThemeData();
      await tester.pumpWidget(
        MaterialApp(
  ...
  testWidgets(
    'Disabled button will be skipped while pressing up/down key while focused',
    (WidgetTester tester) async {
      final ThemeData themeData = ThemeData();
  ...
  testWidgets(
    'Searching is enabled by default if initialSelection is non null',
    (WidgetTester tester) async {
      final ThemeData themeData = ThemeData();
      await tester.pumpWidget(
        MaterialApp(
  ...
  testWidgets(
    'Enable filtering with custom filter callback that filter text case sensitive',
    (WidgetTester tester) async {
      final ThemeData themeData = ThemeData();
      final TextEditingController controller = TextEditingController();
      addTearDown(controller.dispose);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      Widget buildDropdownMenu({required bool requestFocusOnTap}) =>
          MaterialApp(
            theme: themeData,
            home: Scaffold(
              body: Column(
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        buildDropdownMenu(enabled: false, requestFocusOnTap: true),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets('Semantics does not include initial menu buttons', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController();
    addTearDown(controller.dispose);
    ...
    for (final String label in TestMenu.values.map(
      (TestMenu menu) => menu.label,
    )) {
  ...
  testWidgets('Material3 - DropdownMenu uses correct text styles', (
    ...
    final Material material = getButtonMaterial(
      tester,
      TestMenu.mainMenu0.label,
    );
  ...
  testWidgets('DropdownMenu can have customized search algorithm', (
    ...
    void checkExpectedHighlight({
      String? searchResult,
      required List<String> otherItems,
    }) {
  ...
  testWidgets(
    'Menu shows scrollbar when height is limited',
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: DropdownMenu<TestMenu>(dropdownMenuEntries: menuItems),
          ),
        ),
      );
  ...
  testWidgets('Focus traversal ignores non visible entries', (
    WidgetTester tester,
  ) async {
    ...
    primaryFocus!.nextFocus();
    await tester.pump();
    final Element iconButton = tester.firstElement(
          ...
          body: DropdownMenu<int>(
            dropdownMenuEntries: <DropdownMenuEntry<int>>[
              DropdownMenuEntry<int>(
                value: 0,
                label:
                    'This is a long text that is multiplied by 4 so it can overflow. ' *
                    4,
              ),
            ],
          ),
  ...
  testWidgets('Text field does not overflow parent', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('Text field with large helper text can be selected', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('DropdownMenu allows customizing text field text align', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(
  ...
  testWidgets('DropdownMenu correctly sets keyboardType on TextField', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('DropdownMenu passes an alignmentOffset to MenuAnchor', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('DropdownMenu filter is disabled until text input', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets(
    'Setting DropdownMenu.requestFocusOnTap to false makes TextField read only',
    (WidgetTester tester) async {
  ...
  testWidgets('scrollToHighlight does not scroll parent', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('DropdownMenu with expandedInsets can be aligned', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('DropdownMenu.closeBehavior controls menu closing behavior', (
    ...
    MenuAnchor dropdownMenuAnchor = tester.widget<MenuAnchor>(
      find.byType(MenuAnchor).last,
    );
    ...
    await tester.pumpWidget(
      buildDropdownMenu(closeBehavior: DropdownMenuCloseBehavior.self),
    );
    ...
    await tester.tap(find.byType(TextField));
    await tester.pumpAndSettle();
    dropdownMenuAnchor = tester.widget<MenuAnchor>(
      find.byType(MenuAnchor).last,
    );
    expect(dropdownMenuAnchor.controller!.isOpen, true);
    ...
    await tester.pumpWidget(
      buildDropdownMenu(closeBehavior: DropdownMenuCloseBehavior.none),
    );
  ...
  testWidgets('Using keyboard navigation to select', (
    WidgetTester tester,
  ) async {
    final FocusNode focusNode = FocusNode();
    addTearDown(focusNode.dispose);
  ...
  testWidgets('DropdownMenu passes textInputAction to TextField', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets(
    'items can be constrainted to be smaller than the text field with menuStyle',
    (WidgetTester tester) async {
      ...
      expect(tester.takeException(), isNull);
      expect(
        tester.getSize(findMenuItemButton(menuChildren.first.label)).width,
        150.0,
      );
    },
  );
  ...
  testWidgets('ensure exclude semantics for trailing button', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('restorationId is passed to inner TextField', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Finder iconButton = find.widgetWithIcon(
        IconButton,
  ...
  testWidgets(
    'Explicitly provided controllers should not be disposed when switched out.',
    (WidgetTester tester) async {
