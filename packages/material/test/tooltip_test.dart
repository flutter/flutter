...
void main() {
  ...
  testWidgets('Material3 - Does tooltip end up in the right place - top left', (
    ...
    expect(
      tip.size.height,
      equals(30.0),
    ); // 20.0 height + 5.0 padding * 2 (top, bottom)
    expect(
      tip.localToGlobal(tip.size.topLeft(Offset.zero)),
      equals(const Offset(10.0, 20.0)),
    );
  ...
  testWidgets('Material2 - Tooltip text scales with textScaler', (
    ...
    await tester.pumpWidget(
      buildApp(tooltipText, textScaler: TextScaler.noScaling),
    );
    await tester.longPress(find.byType(Tooltip));
    expect(find.text(tooltipText), findsOneWidget);
    expect(
  ...
  testWidgets('Hovered text should dismiss when clicked outside', (
    ...
    final TestGesture mouseGesture1 = await tester.createGesture(
      kind: PointerDeviceKind.mouse,
    );
    ...
    final TestGesture mouseGesture2 = await tester.createGesture(
      kind: PointerDeviceKind.mouse,
    );
  ...
  testWidgets('Tooltip is not selectable', (WidgetTester tester) async {
    ...
    await gesture.moveTo(
      Alignment.centerLeft.alongSize(tooltipTextRect.size) +
          tooltipTextRect.topLeft,
    );
    await tester.pump();
    await gesture.moveTo(
      Alignment.centerRight.alongSize(tooltipTextRect.size) +
          tooltipTextRect.topLeft,
    );
    await tester.pump();
  ...
  testWidgets(
    'Tooltip overlay ignores pointer by default when passing simple message',
    (WidgetTester tester) async {
  ...
  testWidgets(
    "Tooltip overlay doesn't ignore pointer by default when passing rich message",
    (WidgetTester tester) async {
  ...
  testWidgets(
    'Tooltip overlay with richMessage ignores pointer when passing ignorePointer: true',
    (WidgetTester tester) async {
      const InlineSpan richMessage = TextSpan(
        children: <InlineSpan>[
          TextSpan(
  ...
  testWidgets(
    'Tooltip respects and prefers the given constraints over theme constraints',
    (WidgetTester tester) async {
      final GlobalKey<TooltipState> tooltipKey = GlobalKey<TooltipState>();
      ...
      const BoxConstraints tooltipConstraints = BoxConstraints.tightFor(
        width: 500,
        height: 250,
      );
      await tester.pumpWidget(
      ...
      expect(
        ...
        equals(tooltipConstraints.biggest),
      );
    },
  );
  ...
  testWidgets(
    'The Tooltip on the ModalBottomSheet can still be displayed after showMenu.',
    (WidgetTester tester) async {
