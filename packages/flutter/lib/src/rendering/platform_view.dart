...
///
/// {@template flutter.rendering.RenderLinuxView}
/// Embedding GtkWidgets is still preview-quality.
/// {@endtemplate}
///
/// [RenderLinuxView] is responsible for sizing and displaying a Linux GtkWidget.
///
...
/// See also:
///
///  * [LinuxView] which is a widget that is used to show a GtkWidget.
///  * [PlatformViewsService] which is a service for controlling platform views.
class RenderLinuxView extends RenderBox {
  /// Creates a render object for a Linux GtkWidget.
  ///
  /// The `viewId`, `hitTestBehavior`, and `gestureRecognizers` parameters must not be null.
  RenderLinuxView({
    required LinuxViewController viewController,
    required this.hitTestBehavior,
    required Set<Factory<OneSequenceGestureRecognizer>> gestureRecognizers,
  }) : _viewController = viewController {
    updateGestureRecognizers(gestureRecognizers);
  ...
  /// The unique identifier of the GtkWidget controlled by this controller.
  ///
  /// Typically generated by [PlatformViewsRegistry.getNextPlatformViewId], the UIView
  /// must have been created by calling [PlatformViewsService.initLinuxView].
  LinuxViewController get viewController => _viewController;
  LinuxViewController _viewController;
  set viewController(LinuxViewController viewController) {
    assert(viewController != null);
    final bool needsSemanticsUpdate = _viewController.id != viewController.id;
    _viewController = viewController;
    markNeedsPaint();
  ...
  void updateGestureRecognizers(
    ...
    _gestureRecognizer?.dispose();
    _gestureRecognizer =
        _LinuxViewGestureRecognizer(viewController, gestureRecognizers);
  }
  ...
  _LinuxViewGestureRecognizer? _gestureRecognizer;
...
// This recognizer constructs gesture recognizers from a set of gesture recognizer factories
// it was give, adds all of them to a gesture arena team with the _LinuxViewGestureRecognizer
// as the team captain.
// When the team wins a gesture the recognizer notifies the engine that it should release
// the touch sequence to the embedded GtkWidget.
class _LinuxViewGestureRecognizer extends OneSequenceGestureRecognizer {
  _LinuxViewGestureRecognizer(
    this.controller,
    this.gestureRecognizerFactories, {
    PointerDeviceKind? kind,
  }) : super(kind: kind) {
