...
class RenderEditable extends RenderBox
  ...
  void debugAssertLayoutUpToDate() {
    assert(
      _textLayoutLastMaxWidth == constraints.maxWidth && _textLayoutLastMinWidth == constraints.minWidth,
    ...
    for (final InlineSpanSemanticsInformation info in _cachedCombinedSemanticsInfos!) {
      ...
      if (info.isPlaceholder) {
        ...
        final SemanticsNode newChild =
            (_cachedChildNodes?.isNotEmpty ?? false) ? _cachedChildNodes!.removeFirst() : SemanticsNode();
        newChild
  ...
  List<TextSelectionPoint> getEndpointsForSelection(TextSelection selection) {
    ...
    if (boxes.isEmpty) {
      ...
      final Offset start =
          Offset(boxes.first.start.clamp(0, _textPainter.size.width), boxes.first.bottom) + paintOffset;
  ...
  Rect getLocalRectForCaret(TextPosition caretPosition) {
    ...
    final Rect rect =
        Rect.fromLTWH(0.0, 0.0, cursorWidth, cursorHeight).shift(caretOffset + _paintOffset + cursorOffset);
  ...
  void selectWordsInRange({required Offset from, Offset? to, required SelectionChangedCause cause}) {
    ...
    final TextSelection lastWord =
        to == null ? firstWord : _getWordAtOffset(_textPainter.getPositionForOffset(globalToLocal(to - _paintOffset)));
  ...
  TextSelection _getWordAtOffset(TextPosition position) {
    ...
    if (obscureText) {
      ...
      // the position.
    } else if (TextLayoutMetrics.isWhitespace(_plainText.codeUnitAt(position.offset)) && position.offset > 0) {
  ...
  List<PlaceholderDimensions> _layoutChildren(BoxConstraints constraints, {bool dry = false}) {
    ...
    final List<PlaceholderDimensions> placeholderDimensions =
        List<PlaceholderDimensions>.filled(childCount, PlaceholderDimensions.empty);
    int childIndex = 0;
  ...
  void _computeCaretPrototype() {
    ...
    switch (defaultTargetPlatform) {
      ...
      case TargetPlatform.windows:
        _caretPrototype =
            Rect.fromLTWH(0.0, _kCaretHeightOffset, cursorWidth, cursorHeight - 2.0 * _kCaretHeightOffset);
        break;
    }
  }
  ...
  Size computeDryLayout(BoxConstraints constraints) {
    ...
    _layoutText(minWidth: constraints.minWidth, maxWidth: constraints.maxWidth);
    final double width =
        forceLine ? constraints.maxWidth : constraints.constrainWidth(_textPainter.size.width + _caretMargin);
...
class _FloatingCursorPainter extends RenderEditablePainter {
  ...
  void paint(Canvas canvas, Size size, RenderEditable renderEditable) {
    ...
    // Floating Cursor.
    if (floatingCursorRect == null || floatingCursorColor == null || !shouldPaint) return;
...
class _CompositeRenderEditablePainter extends RenderEditablePainter {
  ...
  void addListener(VoidCallback listener) {
    for (final RenderEditablePainter painter in painters) painter.addListener(listener);
  }

  @override
  void removeListener(VoidCallback listener) {
    for (final RenderEditablePainter painter in painters) painter.removeListener(listener);
  }

  @override
  void paint(Canvas canvas, Size size, RenderEditable renderEditable) {
    for (final RenderEditablePainter painter in painters) painter.paint(canvas, size, renderEditable);
  }
