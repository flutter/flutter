...
class PipelineOwner {
  ...
  // yet. To set this, call enableMutationsToDirtySubtrees().
  bool _debugAllowMutationsToDirtySubtrees = false;

  // See RenderObject.enableMutationsToDirtySubtrees
...
abstract class RenderObject extends AbstractNode implements HitTestTarget {
  ...
  ///
  /// See also [enableMutationsToDirtySubtrees].
  @protected
  ...
  /// Allows mutations to be made to dirty nodes in the render tree owned by the
  /// same [PipelineOwner] as this object. The `callback` argument is invoked
  /// synchronously, and the mutations are allowed only during that callback's
  /// execution.
  ///
  /// This exists to enable [GlobalKey] reparenting during the processing of
  /// delayed builds as caused by [LayoutBuilder]. Calling this function
  /// disables a number of assertions that are intended to catch likely bugs. As
  /// such, using this function is generally discouraged.
  ///
  /// This function can only be called during layout.
  ///
  /// See also [invokeLayoutCallback].
  @protected
  void enableMutationsToDirtySubtrees(VoidCallback callback) {
    assert(owner != null);
    assert(_debugMutationsLocked);
    assert(_debugDoingThisLayout);
    assert(!_doingThisLayoutWithCallback);
    _doingThisLayoutWithCallback = true;
    try {
      owner._enableMutationsToDirtySubtrees(callback);
    } finally {
      _doingThisLayoutWithCallback = false;
    }
  }
