      ...
      while (child != null) {
        ...
        if (flex > 0) {
          ...
          final BoxConstraints innerConstraints = switch (_direction) {
            Axis.horizontal => constraints.copyWith(
                minHeight: minCrossSize,
                minWidth: minChildExtent,
                maxWidth: maxChildExtent),
            Axis.vertical => constraints.copyWith(
  ...
  void performLayout() {
    ...
    final double betweenSpace = switch (_mainAxisAlignment) {
      ...
      MainAxisAlignment.center =>
        0.0,
      MainAxisAlignment.spaceBetween when childCount > 1 =>
        remainingSpace / (childCount - 1),
      MainAxisAlignment.spaceAround when childCount > 0 =>
        remainingSpace / childCount,
      MainAxisAlignment.spaceEvenly when childCount > 0 =>
        remainingSpace / (childCount + 1),
