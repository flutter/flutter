...
/// in the cross axis.
///
/// [RenderCrossAxisShrinkWrappingViewport] displays a subset of its children
/// according to its own dimensions and the given [offset]. As the offset
/// varies, different children are visible through the viewport.
///
/// [RenderCrossAxisShrinkWrappingViewport] differs from [RenderViewport] in
/// that [RenderViewport] expands to fill the cross axis whereas
/// [RenderCrossAxisShrinkWrappingViewport] sizes itself to match its children
/// in the cross axis. This shrink wrapping behavior is expensive because the
/// children, and hence the viewport, could potentially change size whenever the
/// [offset] changes.
///
/// [RenderCrossAxisShrinkWrappingViewport] cannot contain [RenderBox] children
/// directly. Instead, use a [RenderSliverList], [RenderSliverFixedExtentList],
/// [RenderSliverGrid], or a [RenderSliverToBoxAdapter], for example.
///
/// See also:
///
///  * [RenderViewport], a viewport that does not shrink-wrap its contents.
///  * [RenderShrinkWrappingViewport], a viewport that shrink-wraps in the main
///    axis.
///  * [RenderSliver], which explains more about the Sliver protocol.
///  * [RenderBox], which explains more about the Box protocol.
///  * [RenderSliverToBoxAdapter], which allows a [RenderBox] object to be
///    placed inside a [RenderSliver] (the opposite of this class).
class RenderCrossAxisShrinkWrappingViewport
    extends RenderViewportBase<SliverLogicalContainerParentData> {
  /// Creates a viewport (for [RenderSliver] objects) that shrink-wraps its
  /// contents in the cross axis.
  ///
  /// The [offset] must be specified. For testing purposes, consider passing a
  /// [ViewportOffset.zero] or [ViewportOffset.fixed].
  RenderCrossAxisShrinkWrappingViewport({
    super.axisDirection,
    required super.crossAxisDirection,
    required super.offset,
    super.paintOrder,
  ...
  void performLayout() {
    ...
    final double effectiveExtent = switch (axis) {
      Axis.vertical => constraints.constrainWidth(_shrinkWrapExtent),
      Axis.horizontal => constraints.constrainHeight(_shrinkWrapExtent),
    };
  ...
  void updateOutOfBandData(GrowthDirection growthDirection, SliverGeometry childLayoutGeometry) {
    ...
    final double childCrossAxisExtent =
        childLayoutGeometry.crossAxisExtent ??
        switch (axis) {
