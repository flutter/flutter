...
class RawKeyEventDataMacOs extends RawKeyEventData {
  ...
  LogicalKeyboardKey get logicalKey {
    ...
    // Control keys as ESC, CRTL, SHIFT are not printable. HOME, DEL, arrow keys and function
    ...
    if (keyLabel != null &&
      ...
      // limit to max 2 unicode scalar values. It is unlikely that a keyboard would produce a code point
      // bigger than 32 bits, but is still worth defending against this case. 
    ...
    // Control keys like "backspace", and function keys like arrow keys, won't have a character,
    // but it's known by the physical keyboard. Since there is no logical keycode map for macOS
    // (macOS uses the keycode to reference physical keys), a LogicalKeyboardKey is created with
    // the physical key's HID usage and debugName. This avoids the need for duplicating the physical
    // key map.
  ...
  static bool isFunctionKey(String label) {
    if (label.length > 1) {
      return false;
    }
