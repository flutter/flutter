...
class LogicalKeyboardKey extends Diagnosticable {
  ...
  /// A map of key codes to the pseudo-key synonym for that keycode.
  ///
  /// The map allows finding the pseudo-key that also represents the concrete
  /// key so that a class with a key map can match pseudo-keys as well as the
  /// actual generated keys.
  ///
  /// Keys in this map are used to represent keys which appear in multiple
  /// places on the keyboard, such as the [shift], [alt], [control], and numeric
  /// keypad keys. These key codes will never be generated by the key event
  /// system, but may be used in key maps to represent the union of all the keys
  /// of each type in order to match them.
  static final Map<LogicalKeyboardKey, LogicalKeyboardKey> synonyms = <LogicalKeyboardKey, LogicalKeyboardKey>{
    shiftLeft: shift,
    shiftRight: shift,
    metaLeft: meta,
    metaRight: meta,
