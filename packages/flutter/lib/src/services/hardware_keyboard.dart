...
/// handler with [addHandler]. It is recommended to use [Focus] instead to be
/// notified only when specific part of the app is focused.
///
...
/// possible.
///
/// [HardwareKeyboard] guarantees the key model to be "regularized": The key
...
/// non-empty initial state or loss of focus.  For example, if CapsLock is
...
/// dispatched.
///
/// The resulting event stream does not map one-to-one to native key event
/// stream. Some native events might be skipped, while some events
/// might be fabricated and do not correspond to native events, marked
...
class HardwareKeyboard {
  ...
  ///
  /// If used during a key event handler, the result will have included effect
  /// of the event.
  ///
  /// See also:
  ///
  ...
  ///
  /// All handlers are removed except for the 1st one, which is added by
  /// [ServicesBinding].
  ///
...
///
/// While Flutter's [KeyEvent]s are converted from key messages from the native
/// platform, every native message might result in multiple [KeyEvent]s, for
/// example, in order to synthesize missed modifier key presses or releases.
...
/// and [RawKeyboard] for recording keeping, then dispatches the [KeyMessage]
...
class KeyEventManager {
  ...
  ///
  /// Key messages received from the platform is first sent to [RawKeyboard]'s
  /// listeners and [HardwareKeyboard]'s handlers, then sent to
  /// [keyMessageHandler], regardless of the results of [HardwareKeyboard]'s
  /// handlers. (Handler result is explained below.) The result from the
  /// handlers and [keyMessageHandler] are combined and returned to the plaform.
  ///
  /// This handler is normally set by the [FocusManager] so that it can control
  /// the key event propagation to focused widgets. Applications that uses the
  ...
  /// attribute with a [KeyMessageHandler] that returns true if the propagation
