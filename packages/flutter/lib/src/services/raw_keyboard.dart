    ...
    switch (keymap) {
      case 'android':
        data = RawKeyEventDataAndroid(
          ...
          eventId: message['eventId'] as int ?? 0,
        );
        break;
      case 'fuchsia':
...
///
/// The handler should return true if the key has been handled.
...
/// keyboard, and not those from software keyboards.
///
/// See also:
///
...
class RawKeyboard {
  ...
  ///
  /// Since they have no way to indicate what they did with the event, listeners
  /// are assumed to not handle the key event, and so these events will also be
  /// distributed to other listeners, and to the [keyEventHandler].
  ///
  /// Most applications can use the focus system (see [Focus] and
  /// [FocusManager]) to receive key events instead of this kind of passive
  /// listener.
  ///
  ...
  /// A handler for key events that will stop propagation if it returns true.
  ///
  ...
  /// event, and indicate that the event was handled, set this attribute to a
  /// [RawKeyEventHandler]. Otherwise, key events will be assumed to not have
  /// been handled, and will also be sent to other (possibly non-Flutter)
