...
///  * [showWindow], which renders a window managed by this controller.
@internal
...
/// When mounted with [showWindow] or [NestedWindow], descendants of the
...
///  * [showWindow], which renders a dialog in a separate widget subtree.
...
///  * [showWindow], which renders a satellite in a separate widget subtree.
@internal
...
///  * [showWindow], which adds a window to the nearest manager.
@internal
...
/// registry and inspect [windows]. To add a window, use [showWindow].
...
///
/// Descendants may use [showWindow] to add windows. Each entry is
...
///
/// {@tool sample}
/// This example starts with one window and uses [showWindow] to add a
...
/// See also:
///
///  * [showWindow], a global function to render a window into the tree.
@internal
class WindowManager extends StatefulWidget {
  ...
  /// [showWindow]; to close one, call [BaseWindowController.destroy].
  ///
  /// {@macro flutter.widgets.windowing.experimental}
...
void showWindow({required BuildContext context, required WindowEntry entry}) {
  ...
  if (windowRegistry == null) {
    throw StateError(
      'To use `showWindow`, a `WindowManager` widget must be rendered in your hierarchy and accessible in via the context.',
    );
  }
