...
/// A widget through which a portion of larger content can be viewed, typically
/// in combination with a [Scrollable], that shrink-wraps its contents in the
/// cross axis.
///
/// [CrossAxisShrinkWrappingViewport] differs from [Viewport] in that
/// [Viewport] expands to fill the cross axis whereas
/// [CrossAxisShrinkWrappingViewport] sizes itself to match its children in the
/// cross axis. This shrink wrapping behavior is expensive because the children,
/// and hence the viewport, could potentially change size whenever the [offset]
/// changes.
///
/// [CrossAxisShrinkWrappingViewport] cannot contain box children directly.
/// Instead, use a [SliverList], [SliverFixedExtentList], [SliverGrid], or a
/// [SliverToBoxAdapter], for example.
///
/// See also:
///
///  * [ListView], [PageView], [GridView], and [CustomScrollView], which combine
///    [Scrollable] and a viewport into widgets that are easier to use.
///  * [ShrinkWrappingViewport], a viewport that shrink-wraps in the main axis.
///  * [Viewport], a viewport that does not shrink-wrap its contents.
class CrossAxisShrinkWrappingViewport extends MultiChildRenderObjectWidget {
  /// Creates a widget that is bigger on the inside and shrink-wraps its
  /// children in the cross axis.
  ///
  /// The viewport listens to the [offset], which means you do not need to
  /// rebuild this widget when the [offset] changes.
  const CrossAxisShrinkWrappingViewport({
    super.key,
    this.axisDirection = AxisDirection.down,
    this.crossAxisDirection,
    required this.offset,
  ...
  @override
  MultiChildRenderObjectElement createElement() => _CrossAxisShrinkWrappingViewportElement(this);

  @override
  RenderCrossAxisShrinkWrappingViewport createRenderObject(BuildContext context) {
    return RenderCrossAxisShrinkWrappingViewport(
      axisDirection: axisDirection,
      crossAxisDirection:
          crossAxisDirection ?? Viewport.getDefaultCrossAxisDirection(context, axisDirection),
      offset: offset,
  ...
  void updateRenderObject(
    BuildContext context,
    RenderCrossAxisShrinkWrappingViewport renderObject,
  ) {
    renderObject
      ..axisDirection = axisDirection
      ..crossAxisDirection =
...
class _CrossAxisShrinkWrappingViewportElement extends MultiChildRenderObjectElement
    with NotifiableElementMixin, ViewportElementMixin {
  _CrossAxisShrinkWrappingViewportElement(CrossAxisShrinkWrappingViewport super.widget);
