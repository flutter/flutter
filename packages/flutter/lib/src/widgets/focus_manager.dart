...
class FocusNode with DiagnosticableTreeMixin, ChangeNotifier {
  ...
  bool get canRequestFocus =>
      _canRequestFocus &&
      (_focusabilityListenable?.value ?? _computeAncestorsAllowFocus());
  bool _computeAncestorsAllowFocus() =>
      ancestors.every(_allowDescendantsToBeFocused);
  ...
  set canRequestFocus(bool value) {
    ...
    final _FocusabilityListenable? focusabilityListenable =
        _focusabilityListenable;
    if (focusabilityListenable != null && focusabilityListenable.hasListeners) {
      final bool ancestorsAllowFocus =
          focusabilityListenable._ancestorsAllowFocus =
              _adjustListeningNodeCountForAncestors(value ? 1 : -1);
  ...
  ValueListenable<bool> get focusabilityListenable =>
  ...
  void _onDescendantsAreFocusableChanged(bool newValue) {
    ...
    final int ancestorListenerAdjustment = newValue
  ...
  void _notifyFocusabilityListenersInSubtree(bool ancestorsAllowFocus) {
    final _FocusabilityListenable? focusabilityListenable =
        _focusabilityListenable;
    if (_canRequestFocus &&
        focusabilityListenable != null &&
        focusabilityListenable.hasListeners) {
      assert(ancestorsAllowFocus == _computeAncestorsAllowFocus());
      assert(
          ancestorsAllowFocus != focusabilityListenable._ancestorsAllowFocus);
  ...
  void _reparent(FocusNode child) {
    ...
    final _FocusabilityListenable? childFocusabilityListenable =
        child._focusabilityListenable;
    final int childSubtreeListenerCount = (child.descendantsAreFocusable
            ? child._focusabilityListeningDescendantCount
            : 0) +
        (child._canRequestFocus &&
                childFocusabilityListenable != null &&
                childFocusabilityListenable.hasListeners
            ? 1
            : 0);
    ...
    if (childSubtreeListenerCount > 0) {
      final bool childCanFocus = child
          ._adjustListeningNodeCountForAncestors(childSubtreeListenerCount);
...
class _FocusabilityListenable extends ChangeNotifier
  ...
  bool get value {
    assert(!hasListeners ||
        _ancestorsAllowFocus == node._computeAncestorsAllowFocus());
    return node._canRequestFocus &&
        (hasListeners
            ? _ancestorsAllowFocus
            : node._computeAncestorsAllowFocus());
  }
