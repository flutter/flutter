...
/// [BaseSliverRefreshControl] far enough to demonstrate that they want the app to
/// refresh. The returned [Future] must complete when the refresh operation is
/// finished.
///
/// Used by [BaseSliverRefreshControl.onRefresh].
...
/// the same values passed into the [BaseSliverRefreshControl].
///
/// The `pulledExtent` parameter is the currently available space either from
/// over-scrolling or as held by the sliver during refresh.
///
...
class _SliverRefresh extends SingleChildRenderObjectWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, covariant _RenderSliverRefresh renderObject) {
    renderObject
...
class _RenderSliverRefresh extends RenderSliver with RenderObjectWithChildMixin<RenderBox> {
    ...
    child!.layout(
      constraints.asBoxConstraints(
        maxExtent: layoutExtent
            // Plus only the overscrolled portion immediately preceding this sliver.
            +
            overscrolledExtent,
      ),
      parentUsesSize: true,
...
/// A base [StatefulWidget] class for sliver widget pull to refresh content control.
///
/// Concrete subclasses must override [buildRefreshIndicator].
///
/// See also:
///
...
abstract class BaseSliverRefreshControl extends StatefulWidget {
  /// An abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const BaseSliverRefreshControl({
    super.key,
  ...
  /// Returns a non-nullable [Widget] which is the widget to show
  /// while the subclasses of [BaseSliverRefreshControl] begin overscrolled.
  ///
  /// Concrete [BaseSliverRefreshControl] subclasses should return the refresh indicator
  /// for the nearest subclass-specific based on the [RefreshIndicatorMode] (state).
  ///
  @protected
  ...
  @override
  State<BaseSliverRefreshControl> createState() => _BaseSliverRefreshControlState();
...
class _BaseSliverRefreshControlState extends State<BaseSliverRefreshControl> {
  ...
  static const double inactiveResetOverscrollFraction = 0.1;
  ...
  RefreshIndicatorMode transitionNextState() {
    ...
    switch (refreshState) {
      ...
      // a subsequent pull-to-refresh from starting.
        if (latestIndicatorBoxExtent > widget.refreshTriggerPullDistance * inactiveResetOverscrollFraction) {
