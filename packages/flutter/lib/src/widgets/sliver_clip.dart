...
class RenderSliverClipRect extends _RenderSliverCustomClip<Rect> {
  ...
  @override
  Rect getClip() {
    _clip ??= clipper?.getClip(child!.paintBounds.size) ?? child!.paintBounds;

    return _clip!;
  }

  @override
  bool clipContains(Offset offset) {
    final Rect clipRect = getClip();
    return clipRect.contains(offset);
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (clipBehavior != Clip.none) {
      ...
      if (clipOverlap && constraints.overlap > 0) {
        clipRect = switch (applyGrowthDirectionToAxisDirection(
          constraints.axisDirection,
          constraints.growthDirection,
        )) {
          AxisDirection.down => clipRect.copyWith(top: clipRect.top + constraints.overlap),
          AxisDirection.up => clipRect.copyWith(bottom: clipRect.bottom - constraints.overlap),
          AxisDirection.right => clipRect.copyWith(left: clipRect.left + constraints.overlap),
          AxisDirection.left => clipRect.copyWith(right: clipRect.right - constraints.overlap),
        };
      }
...
class RenderSliverClipRRect extends _RenderSliverCustomClip<RRect> {
  ...
  RRect getClip() {
    _clip ??=
        clipper?.getClip(paintBounds.size) ??
        borderRadius.resolve(textDirection).toRRect(paintBounds);
  ...
  @override
  bool clipContains(Offset offset) {
    final RRect clipRRect = getClip();
    return clipRRect.contains(offset);
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (clipBehavior != Clip.none) {
      final Rect paintBounds = child!.paintBounds;
      RRect clipRRect =
          clipper?.getClip(paintBounds.size) ??
          borderRadius.resolve(textDirection).toRRect(paintBounds);

      if (clipOverlap && constraints.overlap > 0) {
        clipRRect = switch (applyGrowthDirectionToAxisDirection(
          constraints.axisDirection,
          constraints.growthDirection,
        )) {
          AxisDirection.down => clipRRect.copyWith(top: clipRRect.top + constraints.overlap),
          AxisDirection.up => clipRRect.copyWith(bottom: clipRRect.bottom - constraints.overlap),
          AxisDirection.right => clipRRect.copyWith(left: clipRRect.left + constraints.overlap),
          AxisDirection.left => clipRRect.copyWith(right: clipRRect.right - constraints.overlap),
        };
      }

      if (!clipRRect.isEmpty) {
        context.pushClipRRect(needsCompositing, offset, clipRRect.outerRect, clipRRect, (
          PaintingContext context,
          Offset offset,
        ) {
          context.paintChild(child!, offset);
...
abstract class _RenderSliverCustomClip<T> extends RenderProxySliver {
  ...
  bool hitTest(
    ...
    if (clipBehavior != Clip.none) {
      final Offset hitOffset = switch (constraints.axis) {
        Axis.vertical => Offset(crossAxisPosition, mainAxisPosition),
        Axis.horizontal => Offset(mainAxisPosition, crossAxisPosition),
      };

      if (!clipContains(hitOffset)) {
        return false;
      }
    }
