...
class Navigator extends StatefulWidget {
  ...
  /// {@end-tool}
  @optionalTypeArgs
  static String restorablePush<T extends Object?>(BuildContext context, RestorableRouteBuilder<T> routeBuilder,
      {Object? arguments}) {
  ...
  ///    be restored during state restoration.
  @optionalTypeArgs
  static Future<T?> pushReplacement<T extends Object?, TO extends Object?>(BuildContext context, Route<T> newRoute,
      {TO? result}) {
  ...
  static String restorablePushAndRemoveUntil<T extends Object?>(
      BuildContext context, RestorableRouteBuilder<T> newRouteBuilder, RoutePredicate predicate,
      {Object? arguments}) {
  ...
  static String restorableReplace<T extends Object?>(BuildContext context,
      {required Route<dynamic> oldRoute, required RestorableRouteBuilder<T> newRouteBuilder, Object? arguments}) {
    return Navigator.of(context)
        .restorableReplace<T>(oldRoute: oldRoute, newRouteBuilder: newRouteBuilder, arguments: arguments);
  }
  ...
  /// {@macro flutter.widgets.Navigator.restorablePushNamed.returnValue}
  @optionalTypeArgs
  static String restorableReplaceRouteBelow<T extends Object?>(BuildContext context,
      {required Route<dynamic> anchorRoute, required RestorableRouteBuilder<T> newRouteBuilder, Object? arguments}) {
    return Navigator.of(context).restorableReplaceRouteBelow<T>(
        anchorRoute: anchorRoute, newRouteBuilder: newRouteBuilder, arguments: arguments);
  }
  ...
  static List<Route<dynamic>> defaultGenerateInitialRoutes(NavigatorState navigator, String initialRouteName) {
    ...
    if (result.isEmpty) result.add(navigator._routeNamed<dynamic>(Navigator.defaultRouteName, arguments: null));
...
class NavigatorState extends State<Navigator> with TickerProviderStateMixin, RestorationMixin {
  ...
  void didChangeDependencies() {
    ...
    for (final _RouteEntry entry in _history) entry.route.changedExternalState();
  }
  ...
  void _updateHeroController(HeroController? newHeroController) {
    if (_heroControllerFromScope != newHeroController) {
              ...
              if (_heroControllerFromScope == newHeroController) {
                ...
                if (!hasHeroControllerOwnerShip || previousOwner._heroControllerFromScope == newHeroController) {
                  final NavigatorState otherOwner =
                      hasHeroControllerOwnerShip ? previousOwner : _heroControllerFromScope!._navigator!;
                  FlutterError.reportError(
                    FlutterErrorDetails(
                      exception: FlutterError(
      ...
      // this navigator.
      if (_heroControllerFromScope?._navigator == this) _heroControllerFromScope?._navigator = null;
  ...
  void didUpdateWidget(Navigator oldWidget) {
    ...
    if (oldWidget.observers != widget.observers) {
      for (final NavigatorObserver observer in oldWidget.observers) observer._navigator = null;
      for (final NavigatorObserver observer in widget.observers) {
        assert(observer.navigator == null);
        observer._navigator = this;
      }
  ...
  @override
  void deactivate() {
    for (final NavigatorObserver observer in _effectiveObservers) observer._navigator = null;
    super.deactivate();
  }
  ...
  void dispose() {
    ...
    assert(() {
      for (final NavigatorObserver observer in _effectiveObservers) assert(observer._navigator != this);
      return true;
    }());
  ...
  void _updatePages() {
    ...
    while (oldEntriesBottom <= oldEntriesTop) {
      ...
      if (potentialPageToRemove.key == null ||
        ...
        if (potentialEntryToRemove.willBePresent) potentialEntryToRemove.markNeedsExitingDecision();
      }
  ...
  void _flushHistoryUpdates({bool rearrangeOverlay = true}) {
    ...
    while (index >= 0) {
      switch (entry!.currentState) {
        ...
        case _RouteLifecycle.pop:
          if (!entry.handlePop(
              navigator: this, previousPresent: _getRouteBefore(index, _RouteEntry.willBePresentPredicate)?.route)) {
            assert(entry.currentState == _RouteLifecycle.idle);
            continue;
          }
    ...
    for (final _RouteEntry entry in toBeDisposed) {
      for (final OverlayEntry overlayEntry in entry.route.overlayEntries) overlayEntry.remove();
      entry.dispose();
    }
    if (rearrangeOverlay) {
  ...
  /// {@end-tool}
  @optionalTypeArgs
  String restorablePushReplacement<T extends Object?, TO extends Object?>(RestorableRouteBuilder<T> routeBuilder,
      {TO? result, Object? arguments}) {
    assert(routeBuilder != null);
    assert(_debugIsStaticCallback(routeBuilder), 'The provided routeBuilder must be a static function.');
    assert(debugIsSerializableForRestoration(arguments),
  ...
  bool canPop() {
    ...
    if (!iterator.moveNext()) return false; // we have no active routes, so we can't pop
    if (iterator.current.route.willHandlePopInternally)
      return true; // the first route can handle pops itself, so we can pop
    if (!iterator.moveNext()) return false; // there's only one route, so we can't pop
  ...
  Future<bool> maybePop<T extends Object?>([T? result]) async {
    ...
    if (!mounted) return true; // forget about this pop, we were disposed in the meantime
  ...
  void pop<T extends Object?>([T? result]) {
    ...
    if (entry.currentState == _RouteLifecycle.pop) _flushHistoryUpdates(rearrangeOverlay: false);
  ...
  void finalizeRoute(Route<dynamic> route) {
    ...
    if (entry.hasPage && entry.currentState.index < _RouteLifecycle.pop.index) {
      _observedRouteDeletions
          .add(_NavigatorPopObservation(route, _getRouteBefore(index - 1, _RouteEntry.willBePresentPredicate)?.route));
    } else {
      assert(entry.currentState == _RouteLifecycle.popping);
    }
    entry.finalize();
  ...
  void didStartUserGesture() {
    ...
    if (_userGesturesInProgress == 1) {
      ...
      for (final NavigatorObserver observer in _effectiveObservers) observer.didStartUserGesture(route, previousRoute);
    }
  }
  ...
  void didStopUserGesture() {
    ...
    if (_userGesturesInProgress == 0) {
      for (final NavigatorObserver observer in _effectiveObservers) observer.didStopUserGesture();
    }
  }
          ...
          child: FocusScope(
            ...
            child: UnmanagedRestorationScope(
              ...
              child: Overlay(
                key: _overlayKey,
                initialEntries:
                    overlay == null ? _allRouteOverlayEntries.toList(growable: false) : const <OverlayEntry>[],
              ),
            ),
          ),
        ),
...
class _AnonymousRestorationInformation extends _RestorationInformation {
  ...
  factory _AnonymousRestorationInformation.fromSerializableData(List<Object?> data) {
    ...
    final RestorableRouteBuilder<Object?> routeBuilder =
        ui.PluginUtilities.getCallbackFromHandle(ui.CallbackHandle.fromRawHandle(data[1]! as int))!
