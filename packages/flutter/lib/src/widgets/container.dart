...
class _ContainerState extends State<Container> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child == null && (widget.constraints == null || !widget.constraints!.isTight)) {
      current = LimitedBox(
        maxWidth: 0.0,
        maxHeight: 0.0,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<Clip>('clipBehavior', widget.clipBehavior, defaultValue: Clip.none),
    );
    if (widget.color != null) {
      properties.add(DiagnosticsProperty<Color>('bg', widget.color));
    } else {
      properties.add(DiagnosticsProperty<Decoration>('bg', widget.decoration, defaultValue: null));
    }
    properties.add(
      DiagnosticsProperty<Decoration>('fg', widget.foregroundDecoration, defaultValue: null),
    );
    properties.add(
      DiagnosticsProperty<BoxConstraints>('constraints', widget.constraints, defaultValue: null),
    );
    properties.add(
      DiagnosticsProperty<EdgeInsetsGeometry>('margin', widget.margin, defaultValue: null),
    );
    properties.add(ObjectFlagProperty<Matrix4>.has('transform', widget.transform));
  }
}
