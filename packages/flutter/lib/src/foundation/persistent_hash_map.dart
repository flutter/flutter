...
class _CompressedNode extends _TrieNode {
    ...
    if ((occupiedIndices & bit) != 0) {
      ...
      if (identical(keyOrNull, null)) {
        final _TrieNode newNode =
            _unsafeCast<_TrieNode>(valueOrNode).put(bitIndex + _TrieNode.hashBitsPerLevel, key, keyHash, value);
      ...
      final _TrieNode newNode =
          _resolveCollision(bitIndex + _TrieNode.hashBitsPerLevel, keyOrNull, valueOrNode, key, keyHash, value);
      ...
      if (occupiedCount >= 16) {
        ...
        return _inflate(bitIndex)
          ..descendants[_TrieNode.trieIndex(keyHash, bitIndex)] =
              _CompressedNode.empty.put(bitIndex + _TrieNode.hashBitsPerLevel, key, keyHash, value);
      } else {
        ...
        for (int srcIndex = prefixLength, dstIndex = prefixLength + 2; srcIndex < totalLength; srcIndex++, dstIndex++) {
    ...
    for (int dstIndex = 0; dstIndex < _FullNode.numElements; dstIndex++) {
      if (((occupiedIndices >>> dstIndex) & 1) != 0) {
        ...
        if (keyOrNull == null) {
          ...
          nodes[dstIndex] = _CompressedNode.empty.put(bitIndex + _TrieNode.hashBitsPerLevel, keyOrNull,
              keyValuePairs[srcIndex].hashCode, keyValuePairs[srcIndex + 1]);
        }
        srcIndex += 2;
      }
    }
  ...
  static _TrieNode _resolveCollision(
      int bitIndex, Object existingKey, Object? existingValue, Object key, int keyHash, Object? value) {
...
class _HashCollisionNode extends _TrieNode {
  ...
  factory _HashCollisionNode.fromCollision(int keyHash, Object keyA, Object? valueA, Object keyB, Object? valueB) {
