...
class WriteBuffer {
  ...
  /// performance.
  factory WriteBuffer({final int startCapacity = 8}) {
  ...
  void _append(final Uint8List other) {
  ...
  void _addAll(final Uint8List data, [final int start = 0, final int? end]) {
  ...
  void _resize([final int? requiredLength]) {
  ...
  void putUint8(final int byte) {
    assert(!_isDone);
    _add(byte);
  }

  /// Write a Uint16 into the buffer.
  void putUint16(final int value, {final Endian? endian}) {
    assert(!_isDone);
  ...
  void putUint32(final int value, {final Endian? endian}) {
    assert(!_isDone);
  ...
  void putInt32(final int value, {final Endian? endian}) {
    assert(!_isDone);
  ...
  void putInt64(final int value, {final Endian? endian}) {
    assert(!_isDone);
  ...
  void putFloat64(final double value, {final Endian? endian}) {
    assert(!_isDone);
    _alignTo(8);
  ...
  void putUint8List(final Uint8List list) {
    assert(!_isDone);
    _append(list);
  }

  /// Write all the values from an [Int32List] into the buffer.
  void putInt32List(final Int32List list) {
    assert(!_isDone);
    _alignTo(4);
  ...
  void putInt64List(final Int64List list) {
    assert(!_isDone);
    _alignTo(8);
  ...
  void putFloat32List(final Float32List list) {
    assert(!_isDone);
    _alignTo(4);
    _append(list.buffer.asUint8List(list.offsetInBytes, 4 * list.length));
  }
  ...
  void putFloat64List(final Float64List list) {
    assert(!_isDone);
    _alignTo(8);
    _append(list.buffer.asUint8List(list.offsetInBytes, 8 * list.length));
  }

  void _alignTo(final int alignment) {
    assert(!_isDone);
...
class ReadBuffer {
  ...
  int getUint16({final Endian? endian}) {
  ...
  int getUint32({final Endian? endian}) {
  ...
  int getInt32({final Endian? endian}) {
  ...
  int getInt64({final Endian? endian}) {
  ...
  double getFloat64({final Endian? endian}) {
    _alignTo(8);
  ...
  Uint8List getUint8List(final int length) {
  ...
  Int32List getInt32List(final int length) {
    _alignTo(4);
  ...
  Int64List getInt64List(final int length) {
    _alignTo(8);
  ...
  Float32List getFloat32List(final int length) {
    _alignTo(4);
  ...
  Float64List getFloat64List(final int length) {
    _alignTo(8);
