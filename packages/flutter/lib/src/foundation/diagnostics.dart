...
class TextTreeConfiguration {
  ...
  TextTreeConfiguration({
    ...
    this.mandidtoryAfterProperties = '',
  ...
  /// Manditory string to add after the properties of a node regardless of
  /// whether the node has any properties.
  ///
  /// See [headerLineTextConfiguration] for an example of using this field to add
  /// a colon at the end of the header line.
  final String mandidtoryAfterProperties;
...
final TextTreeConfiguration headerLineTextConfiguration = TextTreeConfiguration(
  ...
  beforeProperties: '(',
  afterProperties: ')',
  mandidtoryAfterProperties: ':',  // Colon that makes this style look like a header.
  prefixLineOne: '',
  prefixOtherLines: '  ',
  prefixLastChildLineOne: '',
  lineBreak: '',
...
class _PrefixedStringBuilder {
  ...
  void _finalizeLine(bool addTrailingLineBreak) {
    ...
    for (String line in lines) {
      i++;
      // XXX switch to writeRaw calls.. XXXX
      _writeLine(
        line,
  ...
  /// Write lines assuming the lines obeys the specified prefixes. Ensures that
  ...
  /// Finishes the currentl line with a stretched version of text.
...
class TextRenderer {
  ...
  TextRenderer({
    this.minLevel = DiagnosticLevel.debug,
    int wrapWidth = 100,
  ...
  final DiagnosticLevel minLevel;
  ...
  String render(
    ...
    if (description == null || description.isEmpty) {
      if (node.showName && node.name != null)
        builder.write(node.name, allowWrap: wrapName);
    } else {
    ...
    if (maxDescendentsTruncatableNode >= 0 && node.allowTruncate) {
      if (propertiesIterable.length < maxDescendentsTruncatableNode) {
        properties =
            propertiesIterable.take(maxDescendentsTruncatableNode).toList();
        properties.add(DiagnosticsNode.message('...'));
      } else {
        properties = propertiesIterable.toList();
      }
      if (maxDescendentsTruncatableNode < children.length) {
        children = children.take(maxDescendentsTruncatableNode).toList();
    ...
    builder.write(config.mandidtoryAfterProperties);
      ...
      for (int i = 0; i < children.length; i++) {
        ...
        if (i == children.length - 1) {
          ...
          if (childConfig.footer.isNotEmpty) {
            /// XXX cleanup resetting the prefix.
            ...
            if (childConfig.manditoryFooter.isNotEmpty) {
              // W
              builder.writeStretched(childConfig.manditoryFooter,
                  // XXX make this a helper fn.
...
class UrlProperty extends DiagnosticsProperty<String> {
  ...
  ///
  /// The [url], and [level] arguments must not be null.
  UrlProperty(String name, {
    ...
    DiagnosticLevel level = DiagnosticLevel.info,
       ...
       super(
         ...
         style: style,
         level: level,
         allowWrap: false,  // Wrapping the url would make it unreadable.
         allowNameWrap: true,
       );
