...
mixin SchedulerBinding on BindingBase {
  ...
  void _runTasks() {
    ...
    if (handleEventLoopCallback()) _ensureEventLoopCallback(); // runs next task when there's time
  }
  ...
  static void _debugDescribeTimeStamp(Duration timeStamp, StringBuffer buffer) {
    ...
    if (timeStamp.inHours > 0) buffer.write('${timeStamp.inHours - timeStamp.inDays * Duration.hoursPerDay}h ');
    if (timeStamp.inMinutes > 0) buffer.write('${timeStamp.inMinutes - timeStamp.inHours * Duration.minutesPerHour}m ');
...
bool defaultSchedulingStrategy({required int priority, required SchedulerBinding scheduler}) {
  if (scheduler.transientCallbackCount > 0) return priority >= Priority.animation.value;
  return true;
}
