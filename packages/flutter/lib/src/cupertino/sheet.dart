...
mixin _CupertinoSheetRouteTransitionMixin<T> on PageRoute<T> {
  ...
  bool canTransitionTo(TransitionRoute<dynamic> nextRoute) {
    if (this is CupertinoSheetRoute<dynamic> && hasCustomTopGap) {
      return false;
    }
    return nextRoute is _CupertinoSheetRouteTransitionMixin && !hasCustomTopGap;
  }
...
class _CupertinoDragGestureController<T> {
  ...
  void dragUpdate(double delta, AnimationController upController) {
    if (popDragController.value == 1.0 && delta < 0) {
      ...
      upController.value -=
          delta / (navigator.context.size!.height * (topGap - _kStretchedTopGapRatio));
    } else {
