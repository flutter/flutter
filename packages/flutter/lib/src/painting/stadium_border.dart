...
class HalfStadiumBorder extends OutlinedBorder {
  /// Create a stadium border.
  ///
  /// The [side] argument must not be null.
  const HalfStadiumBorder({
    super.side,
  ...
  bool _resolvedRoundStart(TextDirection? direction) {
    switch (direction) {
      case null:
  ...
  @override
  ShapeBorder scale(double t) => HalfStadiumBorder(side: side.scale(t));

  @override
  ShapeBorder? lerpFrom(ShapeBorder? a, double t) {
    assert(t != null);
    if (a is HalfStadiumBorder) {
      return HalfStadiumBorder(
        side: BorderSide.lerp(a.side, side, t),
        roundStart: t > 0.5 ? roundStart : a.roundStart);
    }
    return super.lerpFrom(a, t);
  }

  @override
  HalfStadiumBorder copyWith({ BorderSide? side, bool? roundStart }) {
    return HalfStadiumBorder(
      side: side ?? this.side,
  ...
  @override
  Path getInnerPath(Rect rect, { TextDirection? textDirection }) {
    return getOuterPath(rect.deflate(side.strokeInset), textDirection: textDirection);
  }

  @override
  Path getOuterPath(Rect rect, { TextDirection? textDirection }) {
    if (_resolvedRoundStart(textDirection)) {
      final double diameter = rect.shortestSide;
      final Rect arcRect = Rect.fromLTWH(rect.left, rect.top, diameter, diameter);
      return Path()
        ..addPolygon(<Offset>[arcRect.topCenter, rect.topRight, rect.bottomRight, arcRect.bottomCenter], false)
        ..addArc(arcRect, math.pi / 2, math.pi);
    } else {
      final double diameter = rect.shortestSide;
      final Rect arcRect = Rect.fromLTWH(rect.right - diameter, rect.top, diameter, diameter);
      return Path()
        ..addPolygon(<Offset>[rect.topLeft, arcRect.topCenter, arcRect.bottomCenter, rect.bottomLeft], false)
        ..addArc(arcRect, -math.pi / 2, math.pi);
    }
  }
    ...
    switch (side.style) {
      ...
      case BorderStyle.solid:
        ...
        if (_resolvedRoundStart(textDirection)) {
          ...
          canvas.drawLine(arcRect.topCenter, rect.topRight, paint);
          canvas.drawLine(arcRect.bottomCenter, rect.bottomRight, paint);
        } else {
          final double diameter = rect.shortestSide;
          final Rect arcRect = Rect.fromLTWH(rect.right - diameter, rect.top, diameter, diameter);
          canvas.drawArc(arcRect, -math.pi / 2, math.pi, false, paint);
          canvas.drawLine(rect.topLeft, arcRect.topCenter, paint);
          canvas.drawLine(rect.bottomLeft, arcRect.bottomCenter, paint);
        }
    }
  }
  ...
  bool operator ==(Object other) {
    ...
    return other is HalfStadiumBorder
      && other.side == side
      && other.roundStart == roundStart;
  }
  ...
  @override
  String toString() {
    return '${objectRuntimeType(this, 'HalfStadiumBorder')}($side)';
  }
}
