...
class Interval extends Curve {
  ...
  @override
  String toString() {
    if (curve is! _Linear) return '${objectRuntimeType(this, 'Interval')}($begin\u22EF$end)\u27A9$curve';
...
abstract class Curve2D extends ParametricCurve<Offset> {
  ...
  Iterable<Curve2DSample> generateSamples({
    ...
    sample(
      ...
      last,
      forceSubdivide:
          (first.value.dx - last.value.dx).abs() < tolerance && (first.value.dy - last.value.dy).abs() < tolerance,
    );
    return samples;
  }
...
class CatmullRomSpline extends Curve2D {
  ...
  static List<List<Offset>> _computeSegments(
    ...
    for (int i = 0; i < allPoints.length - 3; ++i) {
      ...
      final Offset m1 =
          (diffCurve21 + (diffCurve10 / t01 - (curve[2] - curve[0]) / (t01 + t12)) * t12) * reverseTension;
      final Offset m2 =
          (diffCurve21 + (diffCurve32 / t23 - (curve[3] - curve[1]) / (t12 + t23)) * t12) * reverseTension;
