...
class Stepper extends StatefulWidget {
  ...
  const Stepper({
    ...
    this.headerMargin = const EdgeInsets.symmetric(horizontal: 24.0),
  ...
  /// The amount of space by which to inset the content of [steps].
  /// Default padding is 24.0 logical pixels on
  /// all sides:
  ...
  /// The the controls of [onStepContinue] and [onStepCancel]
  /// should build.
  /// Defaults to true.
...
class _StepperState extends State<Stepper> with TickerProviderStateMixin {
  ...
  ScrollController _horizontalStepperController;
  ...
  void initState() {
    ...
    if (widget.type == StepperType.horizontal) {
      _horizontalStepperController = new ScrollController();
    }
  }
  ...
  void didUpdateWidget(Stepper oldWidget) {
    ...
    _horizontalStepperController.animateTo(
      widget.currentStep * 48.0,
      curve: Curves.linear,
          ...
          secondChild: Container(
            ...
            child: Column(
              children: <Widget>[
                ...
                if (widget.includeControls) _buildVerticalControls(),
              ],
            ),
          ),
          firstCurve: const Interval(0.0, 0.6, curve: Curves.fastOutSlowIn),
  ...
  Widget _buildHorizontal() {
          ...
          UnconstrainedBox(
            child: Container(
              constraints: BoxConstraints(
                  minWidth: widget.steps.length > 2 ? 48.0 : 72.0),
              height: 1.0,
              color: Colors.grey.shade400,
            ),
          ),
    ...
    double leftSpacing = (widget.headerMargin as EdgeInsets).left;
    return Column(
      children: <Widget>[
        Material(
          elevation: 2.0,
          child: Container(
            margin: widget.headerMargin.subtract(
                EdgeInsets.only(left: (leftSpacing >= 8.0 ? 8.0 : 0))),
            constraints: BoxConstraints.expand(height: 72.0),
            child: ListView(
              physics: ClampingScrollPhysics(),
              scrollDirection: Axis.horizontal,
              controller: _horizontalStepperController,
              children: children,
            ),
          ),
        ),
