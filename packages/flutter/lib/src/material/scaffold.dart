...
class ScaffoldState extends State<Scaffold> with TickerProviderStateMixin {
  ...
  void _moveFloatingActionButton(_FABStatus status, final FloatingActionButtonLocation newLocation) {
    ...
    setState(() {
      status.previousLocation = previousLocation;
    ...
    // new position.
    status.moveController.forward(from: restartAnimationFrom);
  }
  ...
  void didUpdateWidget(Scaffold oldWidget) {
    ...
    for (final Key key in possiblyChangedFABKeys) {
      ...
      _fabStatuses[key] ??= _FABStatus(
        ...
        previousLocation: oldConfiguration?.location ?? _kDefaultFloatingActionButtonLocation,
        moveController: AnimationController(
          vsync: this,
          lowerBound: 0.0,
          upperBound: 1.0,
