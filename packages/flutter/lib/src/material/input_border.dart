...
class OutlineInputBorder extends InputBorder {
  ...
  Path _gapBorderPath(Canvas canvas, RRect center, double width, double start, double extent) {
    ...
    if (start + extent < width - scaledRRect.trRadiusX) {
      path.moveTo(start + extent, scaledRRect.top);
      path.lineTo(scaledRRect.right - scaledRRect.trRadiusX, scaledRRect.top);
      if (scaledRRect.trRadius != Radius.zero) {
        path.addArc(trCorner, trCornerArcStart, trCornerArcSweep);
      }
    } else if (start + extent < width) {
      final double dx = width - (start + extent);
