// Copyright 2014 The Flutter Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Version: 34.1.18

class TokenTypescaleEmphasized {
  /// md.sys.typescale.emphasized.display-small.font
  static const String displaySmallFont = 'Roboto';

  /// md.sys.typescale.emphasized.body-large.font
  static const String bodyLargeFont = 'Roboto';

  /// md.sys.typescale.emphasized.display-small
  static const double displaySmallFontSize = 36.00;

  /// md.sys.typescale.emphasized.display-small
  static const double displaySmallFontWeight = 500;

  /// md.sys.typescale.emphasized.display-small
  static const double displaySmallLineHeight = 44.00;

  /// md.sys.typescale.emphasized.display-small
  static const double displaySmallLetterSpacing = 0.00;

  /// md.sys.typescale.emphasized.display-small
  static const String displaySmallFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.body-small.font
  static const String bodySmallFont = 'Roboto';

  /// md.sys.typescale.emphasized.body-medium
  static const double bodyMediumFontSize = 14.00;

  /// md.sys.typescale.emphasized.body-medium
  static const double bodyMediumFontWeight = 500;

  /// md.sys.typescale.emphasized.body-medium
  static const double bodyMediumLineHeight = 20.00;

  /// md.sys.typescale.emphasized.body-medium
  static const double bodyMediumLetterSpacing = 0.25;

  /// md.sys.typescale.emphasized.body-medium
  static const String bodyMediumFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.label-small.font
  static const String labelSmallFont = 'Roboto';

  /// md.sys.typescale.emphasized.title-small
  static const double titleSmallFontSize = 14.00;

  /// md.sys.typescale.emphasized.title-small
  static const double titleSmallFontWeight = 700;

  /// md.sys.typescale.emphasized.title-small
  static const double titleSmallLineHeight = 20.00;

  /// md.sys.typescale.emphasized.title-small
  static const double titleSmallLetterSpacing = 0.10;

  /// md.sys.typescale.emphasized.title-small
  static const String titleSmallFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.label-large
  static const double labelLargeFontSize = 14.00;

  /// md.sys.typescale.emphasized.label-large
  static const double labelLargeFontWeight = 700;

  /// md.sys.typescale.emphasized.label-large
  static const double labelLargeLineHeight = 20.00;

  /// md.sys.typescale.emphasized.label-large
  static const double labelLargeLetterSpacing = 0.10;

  /// md.sys.typescale.emphasized.label-large
  static const String labelLargeFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.label-large.font
  static const String labelLargeFont = 'Roboto';

  /// md.sys.typescale.emphasized.body-large
  static const double bodyLargeFontSize = 16.00;

  /// md.sys.typescale.emphasized.body-large
  static const double bodyLargeFontWeight = 500;

  /// md.sys.typescale.emphasized.body-large
  static const double bodyLargeLineHeight = 24.00;

  /// md.sys.typescale.emphasized.body-large
  static const double bodyLargeLetterSpacing = 0.50;

  /// md.sys.typescale.emphasized.body-large
  static const String bodyLargeFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.title-large.font
  static const String titleLargeFont = 'Roboto';

  /// md.sys.typescale.emphasized.label-small
  static const double labelSmallFontSize = 11.00;

  /// md.sys.typescale.emphasized.label-small
  static const double labelSmallFontWeight = 700;

  /// md.sys.typescale.emphasized.label-small
  static const double labelSmallLineHeight = 16.00;

  /// md.sys.typescale.emphasized.label-small
  static const double labelSmallLetterSpacing = 0.50;

  /// md.sys.typescale.emphasized.label-small
  static const String labelSmallFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.display-medium.font
  static const String displayMediumFont = 'Roboto';

  /// md.sys.typescale.emphasized.title-medium
  static const double titleMediumFontSize = 16.00;

  /// md.sys.typescale.emphasized.title-medium
  static const double titleMediumFontWeight = 700;

  /// md.sys.typescale.emphasized.title-medium
  static const double titleMediumLineHeight = 24.00;

  /// md.sys.typescale.emphasized.title-medium
  static const double titleMediumLetterSpacing = 0.15;

  /// md.sys.typescale.emphasized.title-medium
  static const String titleMediumFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.headline-large
  static const double headlineLargeFontSize = 32.00;

  /// md.sys.typescale.emphasized.headline-large
  static const double headlineLargeFontWeight = 500;

  /// md.sys.typescale.emphasized.headline-large
  static const double headlineLargeLineHeight = 40.00;

  /// md.sys.typescale.emphasized.headline-large
  static const double headlineLargeLetterSpacing = 0.00;

  /// md.sys.typescale.emphasized.headline-large
  static const String headlineLargeFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.headline-medium.font
  static const String headlineMediumFont = 'Roboto';

  /// md.sys.typescale.emphasized.body-small
  static const double bodySmallFontSize = 12.00;

  /// md.sys.typescale.emphasized.body-small
  static const double bodySmallFontWeight = 500;

  /// md.sys.typescale.emphasized.body-small
  static const double bodySmallLineHeight = 16.00;

  /// md.sys.typescale.emphasized.body-small
  static const double bodySmallLetterSpacing = 0.40;

  /// md.sys.typescale.emphasized.body-small
  static const String bodySmallFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.title-small.font
  static const String titleSmallFont = 'Roboto';

  /// md.sys.typescale.emphasized.headline-medium
  static const double headlineMediumFontSize = 28.00;

  /// md.sys.typescale.emphasized.headline-medium
  static const double headlineMediumFontWeight = 500;

  /// md.sys.typescale.emphasized.headline-medium
  static const double headlineMediumLineHeight = 36.00;

  /// md.sys.typescale.emphasized.headline-medium
  static const double headlineMediumLetterSpacing = 0.00;

  /// md.sys.typescale.emphasized.headline-medium
  static const String headlineMediumFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.headline-large.font
  static const String headlineLargeFont = 'Roboto';

  /// md.sys.typescale.emphasized.label-medium.font
  static const String labelMediumFont = 'Roboto';

  /// md.sys.typescale.emphasized.display-medium
  static const double displayMediumFontSize = 45.00;

  /// md.sys.typescale.emphasized.display-medium
  static const double displayMediumFontWeight = 500;

  /// md.sys.typescale.emphasized.display-medium
  static const double displayMediumLineHeight = 52.00;

  /// md.sys.typescale.emphasized.display-medium
  static const double displayMediumLetterSpacing = 0.00;

  /// md.sys.typescale.emphasized.display-medium
  static const String displayMediumFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.title-large
  static const double titleLargeFontSize = 22.00;

  /// md.sys.typescale.emphasized.title-large
  static const double titleLargeFontWeight = 500;

  /// md.sys.typescale.emphasized.title-large
  static const double titleLargeLineHeight = 28.00;

  /// md.sys.typescale.emphasized.title-large
  static const double titleLargeLetterSpacing = 0.00;

  /// md.sys.typescale.emphasized.title-large
  static const String titleLargeFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.body-medium.font
  static const String bodyMediumFont = 'Roboto';

  /// md.sys.typescale.emphasized.headline-small
  static const double headlineSmallFontSize = 24.00;

  /// md.sys.typescale.emphasized.headline-small
  static const double headlineSmallFontWeight = 500;

  /// md.sys.typescale.emphasized.headline-small
  static const double headlineSmallLineHeight = 32.00;

  /// md.sys.typescale.emphasized.headline-small
  static const double headlineSmallLetterSpacing = 0.00;

  /// md.sys.typescale.emphasized.headline-small
  static const String headlineSmallFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.label-medium
  static const double labelMediumFontSize = 12.00;

  /// md.sys.typescale.emphasized.label-medium
  static const double labelMediumFontWeight = 700;

  /// md.sys.typescale.emphasized.label-medium
  static const double labelMediumLineHeight = 16.00;

  /// md.sys.typescale.emphasized.label-medium
  static const double labelMediumLetterSpacing = 0.50;

  /// md.sys.typescale.emphasized.label-medium
  static const String labelMediumFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.title-medium.font
  static const String titleMediumFont = 'Roboto';

  /// md.sys.typescale.emphasized.display-large
  static const double displayLargeFontSize = 57.00;

  /// md.sys.typescale.emphasized.display-large
  static const double displayLargeFontWeight = 500;

  /// md.sys.typescale.emphasized.display-large
  static const double displayLargeLineHeight = 64.00;

  /// md.sys.typescale.emphasized.display-large
  static const double displayLargeLetterSpacing = -0.25;

  /// md.sys.typescale.emphasized.display-large
  static const String displayLargeFontFamily = 'Roboto';

  /// md.sys.typescale.emphasized.display-large.font
  static const String displayLargeFont = 'Roboto';

  /// md.sys.typescale.emphasized.headline-small.font
  static const String headlineSmallFont = 'Roboto';
}
