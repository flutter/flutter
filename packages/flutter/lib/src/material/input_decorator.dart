...
class _RenderDecoration extends RenderBox with SlottedContainerRenderObjectMixin<_DecorationSlot> {
  ...
  _RenderDecorationLayout _layout(BoxConstraints layoutConstraints) {
    ...
    final double topInputBaseline =
        contentPadding.top + topHeight + inputInternalBaseline + baselineAdjustment + interactiveAdjustment;
  ...
  void performLayout() {
    ...
    if (helperError != null || counter != null) {
      ...
      switch (textDirection) {
        case TextDirection.rtl:
          if (helperError != null) baselineLayout(helperError!, right - helperError!.size.width - _boxSize(icon).width);
          if (counter != null) baselineLayout(counter!, left);
          break;
        case TextDirection.ltr:
          if (helperError != null) baselineLayout(helperError!, left + _boxSize(icon).width);
          if (counter != null) baselineLayout(counter!, right - counter!.size.width);
          break;
      }
    }
      ...
      switch (textDirection) {
        case TextDirection.rtl:
          decoration.borderGap!.start = lerpDouble(
              labelX + _boxSize(label).width, _boxSize(container).width / 2.0 + floatWidth / 2.0, floatAlign);
...
class _InputDecoratorState extends State<InputDecorator> with TickerProviderStateMixin {
  ...
  Color _getFillColor(ThemeData themeData) {
    ...
    if (decoration!.fillColor != null) return MaterialStateProperty.resolveAs(decoration!.fillColor!, materialState);
  ...
  Color _getIconColor(ThemeData themeData) {
    Color _resolveIconColor(Set<MaterialState> states) {
      ...
      if (states.contains(MaterialState.focused)) return themeData.colorScheme.primary;
  ...
  TextStyle _getInlineLabelStyle(ThemeData themeData) {
    ...
    return themeData.textTheme.subtitle1!.merge(widget.baseStyle).merge(defaultStyle).merge(style).copyWith(height: 1);
  }
  ...
  TextStyle _getInlineHintStyle(ThemeData themeData) {
    ...
    return themeData.textTheme.subtitle1!.merge(widget.baseStyle).merge(defaultStyle).merge(style);
  }
        ...
        : _Shaker(
            ...
            child: AnimatedOpacity(
              ...
              child: AnimatedDefaultTextStyle(
                duration: _kTransitionDuration,
                curve: _kTransitionCurve,
                style: widget._labelShouldWithdraw ? _getFloatingLabelStyle(themeData) : labelStyle,
...
class InputDecorationTheme with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<TextStyle>('floatingLabelStyle', floatingLabelStyle,
    ...
    properties
        .add(DiagnosticsProperty<TextStyle>('counterStyle', counterStyle, defaultValue: defaultTheme.counterStyle));
    ...
    properties
        .add(DiagnosticsProperty<InputBorder>('errorBorder', errorBorder, defaultValue: defaultTheme.errorBorder));
    properties.add(DiagnosticsProperty<InputBorder>('focusedBorder', focusedBorder,
        defaultValue: defaultTheme.focusedErrorBorder));
    properties.add(DiagnosticsProperty<InputBorder>('focusedErrorBorder', focusedErrorBorder,
        defaultValue: defaultTheme.focusedErrorBorder));
    properties.add(
    ...
    properties.add(DiagnosticsProperty<bool>('alignLabelWithHint', alignLabelWithHint,
        defaultValue: defaultTheme.alignLabelWithHint));
    properties
        .add(DiagnosticsProperty<BoxConstraints>('constraints', constraints, defaultValue: defaultTheme.constraints));
  }
}
