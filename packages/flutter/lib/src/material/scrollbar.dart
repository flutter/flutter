...
class _MaterialScrollbarState extends RawScrollbarState<_MaterialScrollbar> {
  ...
  MaterialStateProperty<Color> get _thumbColor {
    ...
    return MaterialStateProperty.resolveWith((Set<MaterialState> states) {
      if (states.contains(MaterialState.dragged)) return _scrollbarTheme.thumbColor?.resolve(states) ?? dragColor;

      // If the track is visible, the thumb color hover animation is ignored and
      // changes immediately.
      if (_trackVisibility.resolve(states)) return _scrollbarTheme.thumbColor?.resolve(states) ?? hoverColor;
