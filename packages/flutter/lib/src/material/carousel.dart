...
class CarouselController extends ScrollController {
  ...
  /// Animates the carousel to a specific item index.
  ///
  ...
  Future<void> animateToItem(
    ...
    // Ensure the state is attached
    if (_carouselState == null) {
      throw StateError('CarouselController is not attached to a CarouselView.');
    }

    // Calculate the target offset using the item's index and extent
    final double targetOffset = index * _carouselState!._itemExtent;

    // Use the existing `animateTo` method from ScrollController
    await animateTo(
      targetOffset,
      duration: duration,
      curve: curve,
