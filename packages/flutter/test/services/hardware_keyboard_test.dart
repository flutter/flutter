...
void main() {
  ...
  testWidgets('KeyEvent can tell which keys are pressed',
      (WidgetTester tester) async {
    ...
    expect(
        HardwareKeyboard.instance
            .isPhysicalKeyPressed(PhysicalKeyboardKey.numLock),
        isTrue);
    expect(
        HardwareKeyboard.instance
            .isLogicalKeyPressed(LogicalKeyboardKey.numLock),
        isTrue);

    await simulateKeyDownEvent(LogicalKeyboardKey.numpad1, platform: 'windows');
    expect(
        HardwareKeyboard.instance
            .isPhysicalKeyPressed(PhysicalKeyboardKey.numpad1),
        isTrue);
    expect(
        HardwareKeyboard.instance
            .isLogicalKeyPressed(LogicalKeyboardKey.numpad1),
        isTrue);
  ...
  testWidgets('Correctly convert down events that are synthesized released',
      (WidgetTester tester) async {
    final FocusNode focusNode = FocusNode();
    addTearDown(focusNode.dispose);
    final List<KeyEvent> events = <KeyEvent>[];
  ...
  testWidgets('Empty keyData yields no event but triggers inference',
      (WidgetTester tester) async {
    final List<KeyEvent> events = <KeyEvent>[];
  ...
  testWidgets('debugPrintKeyboardEvents causes logging of key events',
      (WidgetTester tester) async {
