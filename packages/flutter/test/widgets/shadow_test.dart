...
void main() {
  testWidgets('Shadows on BoxDecoration', (WidgetTester tester) async {
    ...
    await expectLater(
      ...
      matchesGoldenFile('shadow.BoxDecoration.disabled.png', version: null),
    );
    ...
    await expectLater(
      find.byType(Container),
      matchesGoldenFile('shadow.BoxDecoration.enabled.png', version: null),
      skip: !isLinux
  ...
  testWidgets('Shadows on ShapeDecoration', (WidgetTester tester) async {
    ...
    for (int elevation in kElevationToShadow.keys) {
      ...
      await expectLater(
        find.byType(Container),
        matchesGoldenFile('shadow.ShapeDecoration.$elevation.png', version: null),
      );
    }
    debugDisableShadows = true;
  ...
  testWidgets('Shadows with PhysicalLayer', (WidgetTester tester) async {
    ...
    await expectLater(
      find.byType(Container),
      matchesGoldenFile('shadow.PhysicalModel.disabled.png', version: null),
    );
    debugDisableShadows = false;
    tester.binding.reassembleApplication();
    await tester.pump();
    await expectLater(
      find.byType(Container),
      matchesGoldenFile('shadow.PhysicalModel.enabled.png', version: null),
      skip: !isLinux,
    ); // shadows render differently on different platforms
    debugDisableShadows = true;
  });
  ...
  testWidgets('Shadows with PhysicalShape', (WidgetTester tester) async {
    ...
    for (int elevation in kElevationToShadow.keys) {
      ...
      await expectLater(
        find.byType(Container),
        matchesGoldenFile('shadow.PhysicalShape.$elevation.png', version: '1'),
      );
    }
    debugDisableShadows = true;
  }, skip: !isLinux); // shadows render differently on different platforms
