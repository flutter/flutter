...
class LinkedScrollPosition extends ScrollIndependentPosition {
  ...
  DragScrollActivity beginDrag(DragStartDetails details, VoidCallback dragCancelCallback) {
    final DragScrollActivity activity = new LinkedDragScrollActivity(this, details, dragCancelCallback);
    beginActivity(activity);
    return activity;
  }

  @override
  double setPixels(double value, { bool pushToNeighbors: false }) {
    if (value == pixels)
      return 0.0;
    if (!pushToNeighbors)
      return super.setPixels(value);
    ...
    final double localOverscroll = super.setPixels(value.clamp(
    ...
    return beforeOverscroll + afterOverscroll + localOverscroll;
  }
...
class LinkedScrollActivity extends ScrollActivity {
  LinkedScrollActivity(
    LinkedScrollPosition position,
  ) : super(position);

  @override
  LinkedScrollPosition get position => super.position;
  ...
  double moveBy(double delta) {
    ...
    position.updateUserScrollDirection(commonDirection);
    return position.setPixels(position.pixels + delta);
  }
...
class LinkedDragScrollActivity extends DragScrollActivity {
  LinkedDragScrollActivity(
    ScrollIndependentPosition position,
    DragStartDetails details,
    VoidCallback onDragCanceled,
  ) : super(position, details, onDragCanceled);
  ...
  void applyDragTo(double value) {
    position.setPixels(value, pushToNeighbors: true);
  }
}
