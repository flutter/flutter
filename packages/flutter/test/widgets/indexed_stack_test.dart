...
void main() {
  ...
  testWidgets('IndexedStack supports Positioned children', (WidgetTester tester) async {
    ...
    // Wrapping children with Visibility previously inserted a _Visibility
    // render object between the RenderStack and the user's Positioned, which
    // broke the ParentDataWidget chain. IndexedStack now wraps each child
    // only with inherited widgets (no render objects in between), so
    // Positioned children attach their StackParentData directly to the
    ...
    // Non-Positioned siblings are still wrapped with _VisibilityScope, so
    // Visibility.of(context) continues to work for them.
