...
void main() {
  ...
  testWidgets(
    'State restoration (Restorable Form and TextFormField) '
    ...
    (WidgetTester tester) async {
      ...
      // Error text and most recent text value should be present after
      // restart and restore.
      expect(find.text(errorText('bar')!), findsOneWidget);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
                ...
                child: StatefulBuilder(
                  builder: (BuildContext context, StateSetter state) {
                    ...
                    return Form(
                      ...
                      child: Material(
                        child: TextFormField(
                          // Intentionally leave out restoration ID here.
                          // Verifying that the user interaction is registered
      ...
      final TestRestorationData data = await tester.getRestorationData();
      await tester.restartAndRestore();
      // Error text should be present after restart and restore, but the
      // initial text is shown since TextFormField is not set to be state
      // restorable.
  ...
  testWidgets(
    'State Restoration - Form validator sets the error text only when validate '
    'is called, even on unrestorable FormField descendants',
    (WidgetTester tester) async {
      String? errorText(String? value) => '$value/error';
      late GlobalKey<FormState> formState;
      final GlobalKey<FormFieldState<String>> restorableField = GlobalKey<FormFieldState<String>>();
      final GlobalKey<FormFieldState<String>> unrestoredField = GlobalKey<FormFieldState<String>>();
                      ...
                      child: Material(
                        child: Column(
                          children: <TextFormField>[
                            ...
                            TextFormField(
                              // Intentionally set to not be state restorable.
                              // This is mainly to test validation by [Form].
                              key: unrestoredField,
                              initialValue: 'unrestored',
                              validator: errorText,
                            ),
                          ],
                        ),
      ...
      Future<void> checkErrorText(String testValue) async {
        ...
        // We have to manually validate if we're not autovalidating.
        expect(find.text(errorText(testValue)!), findsNothing);
        expect(find.text(errorText('unrestored')!), findsNothing);

        formState.currentState!.validate();
        await tester.pump();
        expect(find.text(errorText(testValue)!), findsOneWidget);
        expect(find.text(errorText('unrestored')!), findsOneWidget);
