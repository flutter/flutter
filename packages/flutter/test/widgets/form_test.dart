...
void main() {
  ...
  // Regression test for https://github.com/flutter/flutter/issues/173912.
  // Returning a sliver from a [FormField.builder] used to crash because the
  // [Semantics] wrapper added in https://github.com/flutter/flutter/pull/165935
  // is render-box only. [SliverFormField] wraps with [SliverSemantics] instead.
  testWidgets('SliverFormField builder may return a sliver', (WidgetTester tester) async {
    const sliverKey = Key('sliver-list');
    ...
    expect(find.byKey(sliverKey), findsOneWidget);
    expect(find.text('item-0'), findsOneWidget);
    expect(find.text('item-1'), findsOneWidget);
  });

  testWidgets(
    'SliverFormField publishes a SemanticsValidationResult that toggles with validation',
    (WidgetTester tester) async {
              ...
              child: CustomScrollView(
                slivers: <Widget>[
                  SliverFormField<String>(
                    ...
                    builder: (FormFieldState<String> state) {
                      return const SliverToBoxAdapter(child: Text('field'));
                    },
                  ),
                ],
              ),
      ...
      fieldKey.currentState!.didChange('non-empty');
      fieldKey.currentState!.validate();
      await tester.pump();
      expect(
        tester.getSemantics(find.text('field')),
  ...
  testWidgets(
    'SliverFormField with autovalidateMode.onUnfocus does not crash on a sliver child',
    (WidgetTester tester) async {
      final fieldKey = GlobalKey<FormFieldState<String>>();
      await tester.pumpWidget(
        MaterialApp(
