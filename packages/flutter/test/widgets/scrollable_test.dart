...
void main() {
  ...
  testWidgets('Holding scroll', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      await pumpTest(tester, platform);
      await tester.drag(find.byType(Viewport), const Offset(0.0, 200.0), touchSlopY: 0.0);
      expect(getScrollOffset(tester), -200.0, reason: 'on ${describeEnum(platform)}');
      await tester.pump(); // trigger ballistic
      await tester.pump(const Duration(milliseconds: 10));
      expect(getScrollOffset(tester), greaterThan(-200.0), reason: 'on ${describeEnum(platform)}');
      expect(getScrollOffset(tester), lessThan(0.0), reason: 'on ${describeEnum(platform)}');
      ...
      expect(await tester.pumpAndSettle(), 1, reason: 'on ${describeEnum(platform)}');
      expect(getScrollOffset(tester), heldPosition, reason: 'on ${describeEnum(platform)}');
      await gesture.up();
      // Once the hold is let go, it should still snap back to origin.
      expect(await tester.pumpAndSettle(const Duration(minutes: 1)), 2);
      expect(getScrollOffset(tester), 0.0, reason: 'on ${describeEnum(platform)}');
    }
  });

  testWidgets('Repeated flings builds momentum on iOS/macOS and not on Android', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      await pumpTest(tester, platform);
      await tester.fling(find.byType(Viewport), const Offset(0.0, -dragOffset), 1000.0);
      await tester.pump(); // trigger fling
      ...
      // non-trivial amount.
      expect(getScrollVelocity(tester), greaterThan(1100.0), reason: 'on ${describeEnum(platform)}');
    }
  ...
  testWidgets('No iOS/macOS momentum build with flings in opposite directions', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      ...
      // opposite direction.
      expect(getScrollVelocity(tester), greaterThan(-1000.0), reason: 'on ${describeEnum(platform)}');
      expect(getScrollVelocity(tester), lessThan(0.0), reason: 'on ${describeEnum(platform)}');
    }
  });
  ...
  testWidgets('No iOS/macOS momentum kept on hold gestures', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      ...
      expect(getScrollVelocity(tester), 0.0, reason: 'on ${describeEnum(platform)}');
    }
  });
  ...
  testWidgets('Drags creeping must break threshold on iOS/macOS', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      ...
      expect(getScrollOffset(tester), 0.5, reason: 'on ${describeEnum(platform)}');
      await gesture.up();
      resetScrollOffset(tester);
    }
  });
  ...
  testWidgets('Big drag over threshold magnitude preserved on iOS/macOS', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      ...
      expect(getScrollOffset(tester), 30.0, reason: 'on ${describeEnum(platform)}');
      await gesture.up();
      resetScrollOffset(tester);
    }
  });
  ...
  testWidgets('Slow threshold breaks are attenuated on iOS/macOS', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      ...
      expect(getScrollOffset(tester), moreOrLessEquals(1.1666666666666667), reason: 'on ${describeEnum(platform)}');
      await gesture.moveBy(const Offset(0.0, -10.0), timeStamp: const Duration(milliseconds: 20));
      // Subsequent motions unaffected.
      expect(getScrollOffset(tester), moreOrLessEquals(11.16666666666666673), reason: 'on ${describeEnum(platform)}');
      await gesture.up();
      resetScrollOffset(tester);
    }
  });
  ...
  testWidgets('Small continuing motion preserved on iOS/macOS', (WidgetTester tester) async {
    for (TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      ...
      expect(getScrollOffset(tester), 30.0, reason: 'on ${describeEnum(platform)}');
      await gesture.moveBy(const Offset(0.0, -0.5), timeStamp: const Duration(milliseconds: 20));
      expect(getScrollOffset(tester), 30.5, reason: 'on ${describeEnum(platform)}');
      await gesture.moveBy(const Offset(0.0, -0.5), timeStamp: const Duration(milliseconds: 40));
      expect(getScrollOffset(tester), 31.0, reason: 'on ${describeEnum(platform)}');
      await gesture.moveBy(const Offset(0.0, -0.5), timeStamp: const Duration(milliseconds: 60));
      expect(getScrollOffset(tester), 31.5, reason: 'on ${describeEnum(platform)}');
      await gesture.up();
      resetScrollOffset(tester);
    }
  });
