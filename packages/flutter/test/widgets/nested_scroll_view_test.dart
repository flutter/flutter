...
MaterialApp buildNestedScrollViewWithTab({
  required List<String> tabs,
  void Function()? onBuildHeaderSliver,
  bool usePageStorageKey = true,
}) {
            ...
            child: Builder(
              ...
              builder: (BuildContext context) {
                return CustomScrollView(
                  ...
                  // screen.
                  key: usePageStorageKey ? PageStorageKey<String>(name) : null,
                  dragStartBehavior: DragStartBehavior.down,
                  slivers: <Widget>[
...
void main() {
  ...
  testWidgets('When NestedScrollView overscroll, _NestedScrollPosition child correct restore scroll offset', (WidgetTester tester) async {
    Future<void> moveBy (Offset offset) async {
    ...
    Future<void> testWithPageStorageKey(bool usePageStorageKey) async {
      await tester.pumpWidget(
        buildNestedScrollViewWithTab(
          tabs: tabs,
          usePageStorageKey: usePageStorageKey,
        ),
      );

      final ui.Size size = MediaQuery.of(tester.element(find.byType(TabBarView))).size;

      expect(find.text('Item 0'), findsOneWidget);
      expect(find.text('Item 18'), findsNothing);

      await moveBy(Offset(0.0, -size.height));
      await tester.pumpAndSettle();
      expect(find.text('Item 2'), findsNothing);
      expect(find.text('Item 18'), findsOneWidget);

      await moveBy(Offset(-size.width, 0.0));
      await tester.pumpAndSettle();
      expect(find.text('Item 0'), findsOneWidget);
      expect(find.text('Item 18'), findsNothing);

      await moveBy(Offset(0.0, size.height * 2));
      await tester.pumpAndSettle();
      expect(find.text('Item 2'), findsOneWidget);
      expect(find.text('Item 18'), findsNothing);
    ...
    await testWithPageStorageKey(false);
    expect(find.text('Item 0'), findsOneWidget);
    expect(find.text('Item 18'), findsNothing);

    await testWithPageStorageKey(true);
