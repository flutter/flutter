...
class TestTextField extends StatelessWidget {
  const TestTextField({super.key, required this.focusNode, required this.style});
  ...
  Widget build(BuildContext context) {
    return EditableText(
      ...
      backgroundCursorColor: const Color(0x00ffffff),
    );
  }
}

Widget _testApp({required Widget home, Color color = const Color(0xFF2196F3)}) {
  return WidgetsApp(
    home: home,
    color: color,
    pageRouteBuilder: <T>(RouteSettings settings, WidgetBuilder builder) {
...
void main() {
  ...
  testWidgets('Fallback ScrollActions handle too many positions with error message', (
    ...
    await tester.pumpWidget(
      _testApp(home: Row(children: <Widget>[getScrollView(), getScrollView()])),
    );
    await tester.pumpAndSettle();
    expect(
