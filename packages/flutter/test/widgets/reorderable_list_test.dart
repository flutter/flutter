...
void main() {
  ...
  testWidgets('SliverReorderableList with semanticIndexCallback handles separator gap calculations correctly', (
    WidgetTester tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CustomScrollView(
          slivers: <Widget>[
            SliverReorderableList(
              itemCount: 5, // 3 items + 2 separators
              itemBuilder: (BuildContext context, int index) {
                if (index.isEven) {
                  // Items at even indices
                  final int itemIndex = index ~/ 2;
                  return Container(
                    key: ValueKey<int>(items[itemIndex]),
                    height: 60,
                    color: Colors.blue,
                    child: ReorderableDragStartListener(
                      index: index,
                      child: Center(child: Text('Item ${items[itemIndex]}')),
                    ),
                  );
                } else {
                  // Separators at odd indices
                  return Container(
                    key: ValueKey<String>('separator_${index ~/ 2}'),
                    height: 40,
                    color: Colors.red,
                    child: const Center(child: Text('Separator')),
                  );
                }
              },
              semanticIndexCallback: (Widget widget, int index) {
                // Return semantic index only for items (even indices), null for separators (odd indices)
    ...
    // Test that separators exist in the widget tree
    expect(find.text('Separator'), findsNWidgets(2));
    expect(find.text('Item 0'), findsOneWidget);
    expect(find.text('Item 1'), findsOneWidget);
    expect(find.text('Item 2'), findsOneWidget);

    // Verify that the SliverReorderableList has the semanticIndexCallback set
  ...
  testWidgets('SliverReorderableList with semanticIndexCallback skips separators in drag operations', (
    WidgetTester tester,
  ) async {
    final List<int> items = <int>[0, 1, 2];
        ...
        home: CustomScrollView(
          slivers: <Widget>[
            SliverReorderableList(
              ...
              onReorder: (int oldIndex, int newIndex) {
                // This should be called only for item reorders, not separators
              },
            ),
          ],
        ),
    ...
    // Attempt to drag Item 0 downwards. The drag logic should skip separators.
    final TestGesture drag = await tester.startGesture(tester.getCenter(find.text('Item 0')));
    await tester.pump(kLongPressTimeout + kPressTimeout);
    
    // Move past the first separator (should be skipped) to the second item position
    await drag.moveBy(const Offset(0, 140)); // Past separator to Item 1
    await tester.pump();
    
    await drag.up();
    await tester.pumpAndSettle();

    // The drag should complete without errors, indicating separators were properly skipped
    expect(find.text('Item 0'), findsOneWidget);
    expect(find.text('Item 1'), findsOneWidget);
    expect(find.text('Item 2'), findsOneWidget);
  });
