...
Future<void> main() async {
  ...
  testWidgets('Heroes animation curve is customizable', (WidgetTester tester) async {
    ...
    final List<Curve> testCurvesList = <Curve>[
      Curves.linear,
      Curves.ease,
    ...
    for (final Curve curve in testCurvesList) {
      await tester.pumpWidget(MaterialApp(
          key: UniqueKey(),
  ...
  testWidgets('popped hero curve is the flipped version of [curve] if reverseCurve is not specified', (WidgetTester tester) async {
    final List<Curve> testCurvesList = <Curve>[
      Curves.linear,
      Curves.ease,
      Curves.linearToEaseOut,
    ...
    for (final Curve curve in testCurvesList) {
      ...
      expect(
        heroSize.width,
        moreOrLessEquals(tween.transform(0.25)!.width, epsilon: epsilon),
      );
      expect(
        heroSize.height,
        moreOrLessEquals(tween.transform(0.25)!.height, epsilon: epsilon),
      );
      ...
      expect(
        heroSize.width,
        moreOrLessEquals(tween.transform(0.50)!.width, epsilon: epsilon),
      );
      expect(
        heroSize.height,
        moreOrLessEquals(tween.transform(0.50)!.height, epsilon: epsilon),
      );
      ...
      expect(
        heroSize.width,
        moreOrLessEquals(tween.transform(0.75)!.width, epsilon: epsilon),
      );
      expect(
        heroSize.height,
        moreOrLessEquals(tween.transform(0.75)!.height, epsilon: epsilon),
      );
      ...
      expect(
        heroSize.width,
        moreOrLessEquals(tween.transform(1.0)!.width, epsilon: epsilon),
      );
      expect(
        heroSize.height,
        moreOrLessEquals(tween.transform(1.0)!.height, epsilon: epsilon),
      );
    }
  });

  testWidgets('popped hero curve is customizable', (WidgetTester tester) async {
    final List<Curve> testReverseCurvesList = <Curve>[
      Curves.linear,
      Curves.ease,
      Curves.linearToEaseOut,
      Curves.easeInOutCubicEmphasized,
    ...
    for (final Curve reverseCurve in testReverseCurvesList) {
      final Key container1 = UniqueKey();
      final Key container2 = UniqueKey();
      final GlobalKey<NavigatorState> navigator = GlobalKey<NavigatorState>();
