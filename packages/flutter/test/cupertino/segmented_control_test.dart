...
void main() {
  ...
  testWidgets('Animation is correct when the selected segment changes', (WidgetTester tester) async {
    ...
    await tester.pump();
    expect(getBackgroundColor(tester, 0), matchesColor(CupertinoColors.activeBlue));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0x33007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xff3d9aff)));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0x64007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xff7bbaff)));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0x95007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xffb9daff)));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0xc7007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xfff7faff)));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0xf8007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    expect(getBackgroundColor(tester, 0), isSameColorAs(CupertinoColors.white));
    expect(getBackgroundColor(tester, 1), matchesColor(CupertinoColors.activeBlue));
  });
  ...
  testWidgets('Transition is triggered while a transition is already occurring', (WidgetTester tester) async {
    ...
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xff3d9aff)));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0xffc1deff)));
    expect(getBackgroundColor(tester, 2), matchesColor(const Color(0x33007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    // B background color has reached unselected state.
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xff7bbaff)));
    expect(getBackgroundColor(tester, 1), isSameColorAs(CupertinoColors.white));
    expect(getBackgroundColor(tester, 2), matchesColor(const Color(0x64007aff)));

    await tester.pump(const Duration(milliseconds: 100));
    // A background color has reached unselected state.
    expect(getBackgroundColor(tester, 0), isSameColorAs(CupertinoColors.white));
    expect(getBackgroundColor(tester, 2), matchesColor(const Color(0xe0007aff)));

    await tester.pump(const Duration(milliseconds: 40));
    // C background color has reached selected state.
    expect(getBackgroundColor(tester, 2), matchesColor(CupertinoColors.activeBlue));
  });
  ...
  testWidgets('Segment is selected while it is transitioning to unselected state', (WidgetTester tester) async {
    ...
    expect(getBackgroundColor(tester, 0), matchesColor(const Color(0xcd007aff)));
    expect(getBackgroundColor(tester, 1), matchesColor(const Color(0xffc1deff)));
