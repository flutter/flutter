  ...
  group('custom painters', () {
    ...
    group('hit testing', () {
      ...
      test('Basic TextSpan Hit testing', () {
        ...
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(5.0, 5.0)), isTrue);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[textSpanA]);
        // The last A.
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(95.0, 5.0)), isTrue);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[textSpanA]);
        // Far away from the line.
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(200.0, 5.0)), isFalse);
        ...
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(18.0, 15.0)), isTrue);
        ...
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(31.0, 15.0)), isTrue);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[textSpanBC]);

        // On C.
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(54.0, 15.0)), isTrue);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[textSpanBC]);

        // After C.
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(100.0, 15.0)), isFalse);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[]);

        // Not even remotely close.
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(9999.0, 9999.0)), isFalse);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[]);
      });
      ...
      test('TextSpan Hit testing with text justification', () {
        ...
        // Between A and B.
        // ignore: invalid_use_of_protected_member
        expect(renderEditable.hitTestChildren(result = BoxHitTestResult(), position: const Offset(50.0, 5.0)), isTrue);
        expect(result.path.map((HitTestEntry<HitTestTarget> entry) => entry.target as TextSpan), <TextSpan>[textSpanA]);
