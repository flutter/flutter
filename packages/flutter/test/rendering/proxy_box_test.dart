...
void main() {
  ...
  test('RenderFittedBox computeDryBaseline returns null when child is null', () {
    final RenderFittedBox fittedBox = RenderFittedBox();

    final double? baseline = fittedBox.computeDryBaseline(
  ...
  test('RenderFittedBox computeDryBaseline returns null when child has no baseline', () {
    final MockRenderBoxNoBaseline child = MockRenderBoxNoBaseline();

    final RenderFittedBox fittedBox = RenderFittedBox(child: child);
  ...
  test('RenderFittedBox computeDryBaseline forwards to child with unconstrained layout', () {
    final MockRenderBoxWithBaseline child = MockRenderBoxWithBaseline();
    ...
    // Should return the child's baseline value since RenderFittedBox
    // forwards to child with unconstrained layout (matching current behavior)
    expect(baseline, equals(40.0));
  });

  test('RenderFittedBox computeDryBaseline works with different TextBaseline types', () {
    final MockRenderBoxWithBaseline child1 = MockRenderBoxWithBaseline();

    final RenderFittedBox fittedBox1 = RenderFittedBox(child: child1);

    final double? alphabeticBaseline = fittedBox1.computeDryBaseline(
      const BoxConstraints(maxWidth: 100.0, maxHeight: 100.0),
      TextBaseline.alphabetic,
    );

    final MockRenderBoxWithBaseline child2 = MockRenderBoxWithBaseline();
    final RenderFittedBox fittedBox2 = RenderFittedBox(child: child2);

    final double? ideographicBaseline = fittedBox2.computeDryBaseline(
      const BoxConstraints(maxWidth: 100.0, maxHeight: 100.0),
      TextBaseline.ideographic,
    );

    expect(alphabeticBaseline, equals(40.0));
    expect(ideographicBaseline, equals(40.0));
  });
  ...
  test('RenderFittedBox computeDryBaseline works with different BoxFit values', () {
    ...
    expect(containBaseline, equals(40.0));
    expect(fillBaseline, equals(40.0));
  });
...
// Mock render box with baseline for testing
class MockRenderBoxWithBaseline extends RenderBox {
  @override
...
// Mock render box without baseline for testing
