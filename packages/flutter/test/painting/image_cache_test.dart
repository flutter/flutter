...
void main() {
  ...
  test('maintains cache size', () async {
    ...
    final TestImageInfo a = await extractOneFrame(TestImageProvider(1, 1, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(a.value, equals(1));
    final TestImageInfo b = await extractOneFrame(TestImageProvider(1, 2, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(b.value, equals(1));
    final TestImageInfo c = await extractOneFrame(TestImageProvider(1, 3, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(c.value, equals(1));
    final TestImageInfo d = await extractOneFrame(TestImageProvider(1, 4, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(d.value, equals(1));
    final TestImageInfo e = await extractOneFrame(TestImageProvider(1, 5, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(e.value, equals(1));
    final TestImageInfo f = await extractOneFrame(TestImageProvider(1, 6, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    ...
    final TestImageInfo g = await extractOneFrame(TestImageProvider(2, 7, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(g.value, equals(7));

    // cache has two entries in it: 1(1), 2(7)

    final TestImageInfo h = await extractOneFrame(TestImageProvider(1, 8, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(h.value, equals(1));

    // cache still has two entries in it: 2(7), 1(1)

    final TestImageInfo i = await extractOneFrame(TestImageProvider(3, 9, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(i.value, equals(9));

    // cache has three entries in it: 2(7), 1(1), 3(9)

    final TestImageInfo j = await extractOneFrame(TestImageProvider(1, 10, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(j.value, equals(1));

    // cache still has three entries in it: 2(7), 3(9), 1(1)

    final TestImageInfo k = await extractOneFrame(TestImageProvider(4, 11, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(k.value, equals(11));

    // cache has three entries: 3(9), 1(1), 4(11)

    final TestImageInfo l = await extractOneFrame(TestImageProvider(1, 12, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(l.value, equals(1));

    // cache has three entries: 3(9), 4(11), 1(1)

    final TestImageInfo m = await extractOneFrame(TestImageProvider(2, 13, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(m.value, equals(13));

    // cache has three entries: 4(11), 1(1), 2(13)

    final TestImageInfo n = await extractOneFrame(TestImageProvider(3, 14, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(n.value, equals(14));

    // cache has three entries: 1(1), 2(13), 3(14)

    final TestImageInfo o = await extractOneFrame(TestImageProvider(4, 15, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
    expect(o.value, equals(15));

    // cache has three entries: 2(13), 3(14), 4(15)

    final TestImageInfo p = await extractOneFrame(TestImageProvider(1, 16, image: createTestImage()).resolve(ImageConfiguration.empty)) as TestImageInfo;
  ...
  test('putIfAbsent updates LRU properties of a live image', () async {
    ...
    final ui.Image testImage = createTestImage(width: 8, height: 8);
    final ui.Image testImage2 = createTestImage(width: 10, height: 10);
  ...
  test('Live image cache avoids leaks of unlistened streams', () async {
    ...
    TestImageProvider(1, 1, image: createTestImage()).resolve(ImageConfiguration.empty);
    TestImageProvider(2, 2, image: createTestImage()).resolve(ImageConfiguration.empty);
    TestImageProvider(3, 3, image: createTestImage()).resolve(ImageConfiguration.empty);
    TestImageProvider(4, 4, image: createTestImage()).resolve(ImageConfiguration.empty);
    TestImageProvider(5, 5, image: createTestImage()).resolve(ImageConfiguration.empty);
    TestImageProvider(6, 6, image: createTestImage()).resolve(ImageConfiguration.empty);
  ...
  test('Clearing image cache does not leak live images', () {
    imageCache.maximumSize = 1;

    final ui.Image testImage1 = createTestImage(width: 8, height: 8);
    final ui.Image testImage2 = createTestImage(width: 10, height: 10);
  ...
  test('Evicting a pending image clears the live image by default', () {
    final ui.Image testImage = createTestImage(width: 8, height: 8);
  ...
  test('Image is obtained and disposed of properly for cache', () async {
    const int key = 1;
    final ui.Image testImage = createTestImage(width: 8, height: 8, cache: false);
