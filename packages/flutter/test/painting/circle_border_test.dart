...
void main() {
  ...
  test('CircleBorder getInnerPath and getOuterPath', () {
    const circleRect = Rect.fromLTWH(50, 0, 100, 100);
    ...
    const oval = CircleBorder(eccentricity: 1.0);
    expect(oval.getOuterPath(rect).getBounds(), rect);
    expect(oval.getInnerPath(rect).getBounds(), rect);

    const o10 = CircleBorder(side: BorderSide(width: 10.0), eccentricity: 1.0);
  ...
  test('CircleBorder copyWith, ==, hashCode', () {
    ...
    expect(
      const CircleBorder(eccentricity: 0.5).hashCode,
  ...
  test('CircleBorder', () {
    const c10 = CircleBorder(side: BorderSide(width: 10.0));
    const c15 = CircleBorder(side: BorderSide(width: 15.0));
    const c20 = CircleBorder(side: BorderSide(width: 20.0));
    ...
    expect(
      c10.getInnerPath(Rect.fromCircle(center: Offset.zero, radius: 1.0).inflate(10.0)),
      isUnitCircle,
    );
