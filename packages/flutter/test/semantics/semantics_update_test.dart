...
void main() {
  ...
  testWidgets('Semantics update sends traversal parent when grafted child changes', (
    WidgetTester tester,
  ) async {
    final SemanticsHandle handle = tester.ensureSemantics();
    ...
    final childLabel = ValueNotifier<String>('child');
    addTearDown(childLabel.dispose);
        ...
        child: Column(
          children: <Widget>[
            ...
            ValueListenableBuilder<String>(
              valueListenable: childLabel,
              builder: (BuildContext context, String label, _) {
                return Semantics(
                  label: label,
    ...
    SemanticsUpdateBuilderSpy.observations.clear();
    childLabel.value = 'child updated';
    await tester.pump();

    expect(SemanticsUpdateBuilderSpy.observations.containsKey(1), isTrue);
    expect(SemanticsUpdateBuilderSpy.observations.containsKey(2), isTrue);
    expect(SemanticsUpdateBuilderSpy.observations[1]!.label, 'parent');
  ...
  testWidgets('Semantics update sends grafted traversal children with traversal parents', (
    ...
    const identifier = 'identifier';
    final parentLabel = ValueNotifier<String>('parent');
    addTearDown(parentLabel.dispose);

    await tester.pumpWidget(
      Directionality(
        textDirection: TextDirection.ltr,
        child: ValueListenableBuilder<String>(
          valueListenable: parentLabel,
    ...
    SemanticsUpdateBuilderSpy.observations.clear();
    parentLabel.value = 'parent updated';
    await tester.pump();

    expect(SemanticsUpdateBuilderSpy.observations.length, 2);
    expect(SemanticsUpdateBuilderSpy.observations[1]!.label, 'parent updated');
