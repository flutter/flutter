...
void main() {
  ...
  testWidgets('Panel header has semantics, canTapOnHeader = false ',
      (WidgetTester tester) async {
  ...
  testWidgets('ExpansionPanel respects expandIconBuilder',
      (WidgetTester tester) async {
    final List<bool> isExpandedList = <bool>[true, false];
    await tester.pumpWidget(
      MaterialApp(
        home: SingleChildScrollView(
              ...
              return ExpansionPanelList(
                ...
                children: <ExpansionPanel>[
                  ExpansionPanel(
                    ...
                    body: isExpandedList[0] ? const Text('A') : const Text('B'),
                  ),
                  ExpansionPanel(
                    headerBuilder: (BuildContext context, bool isExpanded) {
                      return const Text('ExpansionPanel Header');
                    ...
                    body: isExpandedList[1] ? const Text('C') : const Text('D'),
                  ),
                ],
              );
            },
    ...
    // Tap on the first Checkbox to collapse the first ExpansionPanel.
    await tester.tap(find.byType(Checkbox).first);
    // Tap on the second Checkbox to expand the second ExpansionPanel.
  ...
  testWidgets('ExpansionPanelRadio respects expandIconBuilder',
      (WidgetTester tester) async {
    final List<bool> isExpandedList = <bool>[true, false];
    await tester.pumpWidget(
      MaterialApp(
            ...
            builder: (BuildContext context, StateSetter setState) {
              return ExpansionPanelList.radio(
                ...
                initialOpenPanelValue:
                    isExpandedList.indexWhere((bool val) => val),
                children: <ExpansionPanel>[
                  ExpansionPanelRadio(
                    headerBuilder: (BuildContext context, bool isExpanded) {
                      return const Text('ExpansionPanel Header');
  ...
  testWidgets(
      'ExpansionPanel ignores custom indicator hit test when canTapOnHeader is true',
      (WidgetTester tester) async {
    final List<bool> isExpandedList = <bool>[true, false];
    const Key firstPanelKey = Key('firstPanelKey');
    const Key secondPanelKey = Key('secondPanelKey');
    ...
    // Tap on the second panel header to expand the ExpansionPanel.
    await tester.tap(find.byKey(secondPanelKey));
    await tester.pumpAndSettle();
  ...
  testWidgets(
      'ExpansionPanelRadio ignores custom indicator hit test when canTapOnHeader is true',
      (WidgetTester tester) async {
    final List<bool> isExpandedList = <bool>[true, false];
    const Key firstPanelKey = Key('firstPanelKey');
    const Key secondPanelKey = Key('secondPanelKey');
  ...
  testWidgets('Custom expansion indicator semantics test',
      (WidgetTester tester) async {
    ...
    Widget buildExpansionPanelList([bool canTapOnHeader = false]) {
      return MaterialApp(
        home: SingleChildScrollView(
          child: ExpansionPanelList(
            children: <ExpansionPanel>[
    ...
    await tester.pumpWidget(buildExpansionPanelList());
    ...
    expect(tester.getSemantics(find.byKey(expandedPanelKey)),
        matchesSemantics(label: 'Header Text'));
    ...
    expect(tester.getSemantics(find.byKey(collapsedPanelKey)),
        matchesSemantics(label: 'Header Text'));

    // Check semantics when canTapOnHeader is set to true.
    await tester.pumpWidget(buildExpansionPanelList(true));

    // Check semantics for header widget for expanded panel.
    expect(
        tester.getSemantics(find.byKey(expandedPanelKey)),
        matchesSemantics(
          label: 'Header Text',
          isButton: true,
          isEnabled: true,
    ...
    // Check semantics for header widget for collapsed panel.
    expect(
        tester.getSemantics(find.byKey(collapsedPanelKey)),
        matchesSemantics(
          label: 'Header Text',
          isButton: true,
          isEnabled: true,
