...
class ExpansionPanelListSemanticsTest extends StatefulWidget {
  const ExpansionPanelListSemanticsTest({Key? key, required this.headerKey})
      : super(key: key);
...
void main() {
  ...
  testWidgets('Open/close animations', (WidgetTester tester) async {
    ...
    expect(tester.getRect(find.byType(AnimatedSize).at(0)),
        const Rect.fromLTWH(0.0, 56.0, 800.0, 0.0));
    expect(tester.getRect(find.byType(AnimatedSize).at(1)),
        const Rect.fromLTWH(0.0, 113.0, 800.0, 0.0));
    expect(tester.getRect(find.byType(AnimatedSize).at(2)),
        const Rect.fromLTWH(0.0, 170.0, 800.0, 0.0));
    ...
    expect(
        ...
        inExclusiveRange(
            ...
            113.0 +
                16.0 +
                32.0)); // 16.0 material gap, plus 16.0 top and bottom margins added to the header
    ...
    expect(tester.getRect(find.byType(AnimatedSize).at(0)),
        const Rect.fromLTWH(0.0, 56.0, 800.0, 0.0));
    expect(tester.getRect(find.byType(AnimatedSize).at(1)),
        const Rect.fromLTWH(0.0, 56.0 + 1.0 + 56.0, 800.0, 0.0));
    expect(
        tester.getRect(find.byType(AnimatedSize).at(2)),
        const Rect.fromLTWH(
            0.0, 56.0 + 1.0 + 56.0 + 16.0 + 16.0 + 48.0 + 16.0, 800.0, 100.0));
  });
  ...
  testWidgets('Correct default header padding', (WidgetTester tester) async {
    ...
    expect(
        box.size.height,
        equals(
            80.0)); // _kPanelHeaderCollapsedHeight + 32.0 (double default padding)
    expect(box.size.width, equals(736.0));
  });
