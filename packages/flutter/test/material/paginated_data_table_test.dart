...
class ForceRefreshDataSourceWithData extends TestDataSource {
  bool _forceProgress = false;

  void waitForData(bool value) {
    _forceProgress = value;
  }
...
void main() {
  ...
  testWidgets('PaginatedDataTable paging', (WidgetTester tester) async {
    final ForceRefreshDataSourceWithData source =
        ForceRefreshDataSourceWithData();

    Widget dataTable() => MaterialApp(
      home: PaginatedDataTable(
        key: UniqueKey(),
        header: const Text('Test table'),
        source: source,
    ...
    await tester.pumpWidget(dataTable());

    source.waitForData(true);
    await tester.tap(find.byTooltip('Next page'));
    await tester.pump();
