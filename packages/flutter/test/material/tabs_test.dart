...
Widget buildLeftRightApp(
    {required List<String> tabs, required String value, bool automaticIndicatorColorAdjustment = true}) {
  return MaterialApp(
...
class TabIndicatorRecordingCanvas extends TestRecordingCanvas {
  ...
  void drawLine(Offset p1, Offset p2, Paint paint) {
    ...
    if (paint.color == indicatorColor) indicatorRect = Rect.fromPoints(p1, p2).inflate(indicatorWeight / 2.0);
  }
}
...
void main() {
  ...
  testWidgets('Tab color - normal', (WidgetTester tester) async {
    final Widget tabBar =
        TabBar(tabs: const <Widget>[SizedBox.shrink()], controller: TabController(length: 1, vsync: tester));
    await tester.pumpWidget(
  ...
  testWidgets('TabController listener resets index', (WidgetTester tester) async {
    ...
    tabController.animation!.addListener(() {
      if (tabController.animation!.status == AnimationStatus.forward) tabController.index = 2;
  ...
  testWidgets('TabBar tap skips indicator animation when disabled in controller', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
        buildFrame(tabs: tabs, value: 'A', indicatorColor: indicatorColor, animationDuration: Duration.zero));
  ...
  testWidgets('Tab indicator color should not be adjusted when disable [automaticIndicatorColorAdjustment]',
      (WidgetTester tester) async {
  ...
  testWidgets('tabbar does not scroll when viewport dimensions initially change from zero to non-zero',
      (WidgetTester tester) async {
  ...
  testWidgets('Setting TabController index should make TabBar indicator immediately pop into the position',
      (WidgetTester tester) async {
  ...
  testWidgets('Setting BouncingScrollPhysics on TabBarView does not include ClampingScrollPhysics',
      (WidgetTester tester) async {
  ...
  testWidgets("TabController's animation value should be updated when TabController's index >= tabs's length",
      (WidgetTester tester) async {
  ...
  testWidgets('TabBar preferredSize gives correct value when there are both icon and text in tabs',
      (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      home: DefaultTabController(
        length: 5,
        ...
        TestSemantics.rootChild(
          ...
          children: <TestSemantics>[
            ...
            TestSemantics(
              ...
              rect: TestSemantics.fullScreen,
              children: <TestSemantics>[
                TestSemantics(id: 6, rect: TestSemantics.fullScreen, actions: <SemanticsAction>[
  ...
  testWidgets('Change the TabController should make both TabBar and TabBarView return to the initial index.',
      (WidgetTester tester) async {
  ...
  testWidgets('splashBorderRadius is passed to InkWell.borderRadius', (WidgetTester tester) async {
    ...
    await gesture.moveTo(tester.getCenter(find.byType(Tab)));
    await tester.pumpAndSettle();
    final RenderObject object = tester.allRenderObjects
        .firstWhere((RenderObject element) => element.runtimeType.toString() == '_RenderInkFeatures');
    expect(
      object,
      paints
        ..rrect(
