...
void main() {
  testWidgets('Wraps proper widgets when hovered, pressed, and focused are null.', (WidgetTester tester) async {
    ...
    await tester.pump(); // Register hover.
    await tester.pump(); // Update Based on hover.
    expect(find.text(defaultStateText), findsNothing);
    ...
    await tester.pump(); // Register focus.
    await tester.pump(); // Update Based on focus.
    expect(find.text(defaultStateText), findsNothing);
      ...
      return MaterialApp(
        home: Scaffold(
          body: MaterialStateBuilder(
            ...
            selected: selected,
            error: error,
            builder: (_, Set<MaterialState> states) {
              if (states.contains(MaterialState.disabled)) {
                return const Text(disabledText);
              }
  ...
  testWidgets('Updates all states when passed in as parameters', (WidgetTester tester) async {
    const String defaultStateText = 'default';
    const String disabledText = 'disabled';
    const String selectedText = 'selected';
    const String errorText = 'error';
    ...
    // Hover state.
    await tester.pumpWidget(entireWidget(hovered: true));
    expect(find.text(defaultStateText), findsNothing);
    expect(find.text(hoverText), findsOneWidget);

    // Un-hover.
    await tester.pumpWidget(entireWidget(hovered: false));
    expect(find.text(defaultStateText), findsOneWidget);
    expect(find.text(hoverText), findsNothing);

    // Focus.
    await tester.pumpWidget(entireWidget(focused: true));
    expect(find.text(defaultStateText), findsNothing);
    expect(find.text(focusText), findsOneWidget);

    // Un-focus.
    await tester.pumpWidget(entireWidget(focused: false));
    expect(find.text(defaultStateText), findsOneWidget);
    expect(find.text(focusText), findsNothing);

    // Press.
    await tester.pumpWidget(entireWidget(pressed: true));
    expect(find.text(defaultStateText), findsNothing);
    expect(find.text(pressText), findsOneWidget);

    // Un-press.
    await tester.pumpWidget(entireWidget(pressed: false));
    expect(find.text(defaultStateText), findsOneWidget);
    expect(find.text(pressText), findsNothing);
  });
}
