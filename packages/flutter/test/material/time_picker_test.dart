...
void main() {
  testWidgets('tap-select an hour', (WidgetTester tester) async {
    ...
    await tester.tapAt(new Offset(center.x, center.y - 50.0)); // 12:00 AM
    await finishPicker(tester);
    expect(result, equals(const TimeOfDay(hour: 0, minute: 0)));

    center = await startPicker(tester, (TimeOfDay time) { result = time; });
    await tester.tapAt(new Offset(center.x + 50.0, center.y));
    await finishPicker(tester);
    expect(result, equals(const TimeOfDay(hour: 3, minute: 0)));

    center = await startPicker(tester, (TimeOfDay time) { result = time; });
    await tester.tapAt(new Offset(center.x, center.y + 50.0));
    await finishPicker(tester);
    expect(result, equals(const TimeOfDay(hour: 6, minute: 0)));

    center = await startPicker(tester, (TimeOfDay time) { result = time; });
    await tester.tapAt(new Offset(center.x, center.y + 50.0));
    await tester.tapAt(new Offset(center.x - 50, center.y));
    await finishPicker(tester);
  ...
  testWidgets('drag-select an hour', (WidgetTester tester) async {
    ...
    final Offset hour0 = new Offset(center.x, center.y - 50.0); // 12:00 AM
    final Offset hour3 = new Offset(center.x + 50.0, center.y);
    final Offset hour6 = new Offset(center.x, center.y + 50.0);
    final Offset hour9 = new Offset(center.x - 50.0, center.y);
  ...
  group('haptic feedback', () {
    ...
    testWidgets('drag-select vibrates once', (WidgetTester tester) async {
      final Offset center = await startPicker(tester, (TimeOfDay time) { });
      final Offset hour0 = new Offset(center.x, center.y - 50.0);
      final Offset hour3 = new Offset(center.x + 50.0, center.y);
