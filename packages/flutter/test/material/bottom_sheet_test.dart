...
void main() {
  ...
  group('showModalBottomSheet modalBarrierDismissLabel', () {
    ...
    testWidgetsWithLeakTracking('Swiping up an almost contracted sheet should expand it', (WidgetTester tester) async {
      ...
      Future<void> flingFrom(
        ...
        return TestAsyncUtils.guard<void>(() async {
          final TestPointer testPointer = TestPointer(2);
          const int kMoveCount = 50; // Needs to be >= kHistorySize, see _LeastSquaresVelocityTrackerStrategy
          final double timeStampDelta = 1000000.0 * offset.distance / (kMoveCount * speed);
          double timeStamp = 0.0;
          double lastTimeStamp = timeStamp;
          await tester
              .sendEventToBinding(testPointer.down(startLocation, timeStamp: Duration(microseconds: timeStamp.round())));
          if (initialOffset.distance > 0.0) {
            // TODO This initial move event is a hack to work around the fact that
            // the drag is not performed on the first kDragSlopDefault pixels
            // to get an actual drag, we need to create and exit the exit arena on the first move
            // and then perform the actual drag on the second move
            await tester.sendEventToBinding(testPointer.move(startLocation + const Offset(0, kDragSlopDefault),
                timeStamp: Duration(microseconds: timeStamp.round())));
            await tester.sendEventToBinding(testPointer.move(
                startLocation - const Offset(0, kDragSlopDefault) + initialOffset,
                timeStamp: Duration(microseconds: timeStamp.round())));
            timeStamp += initialOffsetDelay.inMicroseconds;
            await tester.pump(initialOffsetDelay);
          }
          for (int i = 0; i <= kMoveCount; i += 1) {
            ...
            await tester
                .sendEventToBinding(testPointer.move(location, timeStamp: Duration(microseconds: timeStamp.round())));
            timeStamp += timeStampDelta;
            if (timeStamp - lastTimeStamp > frameInterval.inMicroseconds) {
              await tester.pump(Duration(microseconds: (timeStamp - lastTimeStamp).truncate()));
              lastTimeStamp = timeStamp;
            }
          }
          await tester.sendEventToBinding(testPointer.up(timeStamp: Duration(microseconds: timeStamp.round())));
        });
      }
      ...
      await tester.pumpAndSettle();
      await tester.ensureVisible(find.text('BottomSheet'));
      expect(showBottomSheetThenCalled, isFalse);
      expect(find.text('BottomSheet'), findsOneWidget);

      await flingFrom(
        tester.getCenter(find.text('BottomSheet')),
        const Offset(0, -sheetHeight * 0.1),
