...
void main() {
  ...
  // Regression test for https://github.com/flutter/flutter/issues/165561
  testWidgets(
      'showMenu registers `registerCloseCallback` and pops the menu programatically', (
      WidgetTester tester) async {
            ...
            return ElevatedButton(
              onPressed: () {
                showMenu<int>(
                  ...
                  registerCloseCallback: (VoidCallback close) {
                    closeCallback = close;
                  },
                );
              },
    ...
    expect(find.text('One'), findsOneWidget);
    expect(closeCallback, isNotNull);

    closeCallback?.call(); // use callback to close the menu programmatically
    await tester.pumpAndSettle();
  ...
  testWidgets(
      'calling closeCallback multiple times is no-op after menu closed and UI still works', (
      WidgetTester tester) async {
    VoidCallback? closeCallback;
    bool pressed = false;
    ...
    closeCallback?.call(); // Second call should be no-op
    await tester.pumpAndSettle();

    // Interact with UI to confirm app still responds
    await tester.tap(find.text('Another Action'));
    await tester.pumpAndSettle();
  ...
  testWidgets(
      'menu behaves normally if registerCloseCallback provided but closeCallback never called', (
      WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: Builder(
            ...
            return ElevatedButton(
              onPressed: () {
                showMenu<int>(
                  ...
                  registerCloseCallback: (_) {
                    // Intentionally do not save or call the close callback
                  },
                );
              },
              child: const Text('Open Menu'),
    ...
    await tester.tapAt(Offset.zero); // tap outside
    await tester.pumpAndSettle();
