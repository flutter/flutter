...
void main() {
  ...
  void uiTestGroup() {
    ...
    testWidgets('calls the indicator builder when starting to overscroll', (WidgetTester tester) async {
        ...
        // The function is referenced once while passing into SliverRefreshControl()
        // and is called.
        expect(
    ...
    testWidgets("don't call the builder if overscroll doesn't move slivers like on Android default scroll physics", (WidgetTester tester) async {
        await tester.pumpWidget(
          Directionality(
    ...
    testWidgets('refreshing task keeps the sliver expanded forever until done',
    ...
    testWidgets('expanded refreshing sliver scrolls normally', (WidgetTester tester) async {
        ...
        expect(tester.getTopLeft(find.widgetWithText(Center, '-1', skipOffstage: false)).dy,
          moreOrLessEquals(-175.38461538461536),
        );
        expect(tester.getBottomLeft(find.widgetWithText(Center, '-1', skipOffstage: false)).dy,
    ...
    testWidgets("don't do anything unless it can be overscrolled at the start of the list", (WidgetTester tester) async {
        mockHelper.refreshIndicator = const Center(child: Text('-1'));
    ...
    testWidgets('without an onRefresh, builder is called with arm for one frame then sliver goes away', (WidgetTester tester) async {
        mockHelper.refreshIndicator = const Center(child: Text('-1'));
    ...
    testWidgets('properly displays when the refresh sliver is behind the title nav bar sliver', (WidgetTester tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: CustomScrollView(
              physics: const BouncingScrollPhysics(
  ...
  void stateMachineTestGroup() {
    ...
    testWidgets('buildRefreshIndicator progress', (WidgetTester tester) async {
      ...
      expect(tester.widget<RefreshProgressIndicator>(find.byType(RefreshProgressIndicator)).progress, 10.0 / 100.0);
      ...
      expect(tester.widget<RefreshProgressIndicator>(find.byType(RefreshProgressIndicator)).progress, 26.0 / 100.0);
      ...
      expect(tester.widget<RefreshProgressIndicator>(find.byType(RefreshProgressIndicator)).progress, 100.0 / 100.0);
    });
  }
