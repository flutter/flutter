...
void main() {
  ...
  testWidgets('button theme with stateful borders changes border in states', (WidgetTester tester) async {
    final FocusNode focusNode = FocusNode();

    const BorderSide pressedBorder = BorderSide(width: 1.0, color: Colors.red);
    const BorderSide hoverBorder = BorderSide(width: 2.0, color: Colors.orange);
    const BorderSide focusedBorder = BorderSide(width: 3.0, color: Colors.yellow);
    const BorderSide defaultBorder = BorderSide(width: 4.0, color: Colors.green);
    ...
    Color? buttonBorderColor() {
      ...
      final Material widg = tester.widget<Material>(buttonMaterial);
      return (widg.shape! as RoundedRectangleBorder).side.color;
    }

    // Default, not disabled.
    expect(buttonBorderColor(), defaultBorder.color);

    // Focused.
    focusNode.requestFocus();
    await tester.pumpAndSettle();
    expect(buttonBorderColor(), focusedBorder.color);
    ...
    await gesture.moveTo(center);
    await tester.pumpAndSettle();
    expect(buttonBorderColor(), hoverBorder.color);

    // Highlighted (pressed).
    await gesture.down(center);
    await tester.pump(); // Start the splash and highlight animations.
    await tester.pump(const Duration(milliseconds: 800)); // Wait for splash and highlight to be well under way.
    expect(buttonBorderColor(), pressedBorder.color);
  },
    semanticsEnabled: true,
  );
}
