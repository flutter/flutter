...
void main() {
  ...
  testWidgets('CheckboxThemeData configures Checkbox.adaptive colors (cupertino)', (WidgetTester tester) async {
    const Color themeFillColor = Color(0x0000000A);
    const Color themeCheckColor = Color(0x0000000B);
    ...
    for (final TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
        ...
        MaterialApp(
          theme: theme.copyWith(
            ...
            checkboxTheme: const CheckboxThemeData(
              fillColor: WidgetStatePropertyAll<Color>(themeFillColor),
              checkColor: WidgetStatePropertyAll<Color>(themeCheckColor),
            ),
          ),
          home: Material(
            child: Center(
      ...
      expect(
        getCheckboxRenderer(),
        paints
          ..path(color: themeFillColor)
          ..rrect()
          ..path(color: themeCheckColor)
          ..path(color: themeCheckColor),
      );
    }
  });

  testWidgets('CheckboxThemeData configures Checkbox.adaptive shape and side (cupertino)', (WidgetTester tester) async {
    const RoundedRectangleBorder themeShape = RoundedRectangleBorder(
      borderRadius: BorderRadius.all(Radius.circular(10.0))
    );
    const BorderSide themeSide = BorderSide(
      width: 2.0,
      color: Color(0x0000000A),
    );

    for (final TargetPlatform platform in <TargetPlatform>[ TargetPlatform.iOS, TargetPlatform.macOS ]) {
      // The default shape and side.
      await tester.pumpWidget(
        MaterialApp(
      ...
      expect(
        ...
        paints
          ..drrect(
            ...
            outer: RRect.fromLTRBR(13.0, 13.0, 31.0, 31.0, const Radius.circular(4.0)),
            inner: RRect.fromLTRBR(14.0, 14.0, 30.0, 30.0, const Radius.circular(3.0)),
          ),
      );

      // The shape and side changes to those set in the theme.
      await tester.pumpWidget(
        MaterialApp(
          theme: theme.copyWith(
            platform: platform,
            checkboxTheme: const CheckboxThemeData(side: themeSide, shape: themeShape)),
          home: Material(
            child: Center(
              child: StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
                return Checkbox.adaptive(
      ...
      expect(
        ...
        paints
          ..drrect(
            color: const Color(0x0000000A),
            outer: RRect.fromLTRBR(13.0, 13.0, 31.0, 31.0, const Radius.circular(10.0)),
            inner: RRect.fromLTRBR(15.0, 15.0, 29.0, 29.0, const Radius.circular(8.0)),
          ),
      );
    }
  });

  testWidgets('CheckboxThemeData configures Checkbox.adaptive mouse cursor (cupertino)', (WidgetTester tester) async {
    ...
    // The mouse cursor changes to the cursor set in the theme.
    await tester.pumpWidget(Container());
    await tester.pumpWidget(
      MaterialApp(
        theme: theme.copyWith(
