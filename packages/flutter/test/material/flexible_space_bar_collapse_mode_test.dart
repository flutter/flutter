...
void main() {
  testWidgets('FlexibleSpaceBar collapse mode nonePin and nonePinBoundary is zero', (WidgetTester tester) async {
  ...
  testWidgets('FlexibleSpaceBar collapse mode nonePin', (WidgetTester tester) async {
    ...
    final Finder appbarContainer = find.byKey(appbarContainerKey);
    final Offset topBeforeScroll = tester.getTopLeft(appbarContainer);
    await slowDrag(tester, blockKey, const Offset(0.0, -200.0));
    Offset topAfterScroll = tester.getTopLeft(appbarContainer);
    expect(topBeforeScroll.dy, equals(0.0));
    expect(topAfterScroll.dy, equals(0.0));
    await slowDrag(tester, blockKey, const Offset(0.0, -400.0));
    topAfterScroll = tester.getTopLeft(appbarContainer);
