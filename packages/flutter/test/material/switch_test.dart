...
void main() {
  ...
  testWidgets('Switch.adaptive', (WidgetTester tester) async {
    ...
    for (final TargetPlatform platform in <TargetPlatform>[
      ...
      expect(adaptiveSwitch.activeColor, activeTrackColor,
          reason: 'on ${platform.name}');
      expect(adaptiveSwitch.inactiveTrackColor, inactiveTrackColor,
          reason: 'on ${platform.name}');
      expect(adaptiveSwitch.thumbColor?.resolve(<MaterialState>{}), thumbColor,
          reason: 'on ${platform.name}');
  ...
  testWidgets('Switch.adaptive default mouse cursor(Cupertino)',
    ...
    for (final TargetPlatform platform in <TargetPlatform>[
      ...
      await tester.pumpWidget(buildAdaptiveSwitch(
          platform: platform, enabled: false, value: false));
      expect(RendererBinding.instance.mouseTracker.debugDeviceActiveCursor(1),
  ...
  testWidgets('Switch.adaptive default thumb/track color and size(Cupertino)',
    ...
    const Color inactiveTrackColor =
        Color.fromARGB(40, 120, 120, 128); // Default inactive track color.
    const Color activeTrackColor =
        Color.fromARGB(255, 52, 199, 89); // Default active track color.
    for (final TargetPlatform platform in <TargetPlatform>[
      TargetPlatform.iOS,
      TargetPlatform.macOS
    ]) {
      ...
      await tester
          .pumpWidget(buildAdaptiveSwitch(platform: platform, value: false));
      expect(
        Material.of(tester.element(find.byType(Switch))),
        paints
          ..rrect(
          ...
          ..rrect(
              color:
                  const Color(0x0a000000)) // Thumb border color(only cupertino)
          ..rrect(
            color: thumbColor,
  ...
  testWidgets(
      'Default Switch.adaptive are not affected by '
    ...
    for (final TargetPlatform platform in <TargetPlatform>[
      ...
      await tester.pumpWidget(Container());
      await tester.pumpWidget(buildAdaptiveSwitch(
          platform: platform, overallSwitchThemeData: overallSwitchTheme));
      expect(
        Material.of(tester.element(find.byType(Switch))),
        paints
          ..rrect(
    ...
    expect(
      ...
      paints
        ...
        ..rrect(
            ...
            strokeWidth: 3.5,
            style: PaintingStyle.stroke) // Focused outline
        ..rrect()
        ..rrect()
        ..rrect()
  ...
  group('Switch M3 only tests', () {
    ...
    testWidgets('Switch thumb shows correct pressed color - M3',
        (WidgetTester tester) async {
      final ThemeData themeData = ThemeData(useMaterial3: true);
      final ColorScheme colors = themeData.colorScheme;
      ...
      expect(
        ...
        paints
          ..rrect(
            color:
                colors.surfaceContainerHighest.withOpacity(0.12), // track color
            style: PaintingStyle.fill,
          )
          ..rrect(
