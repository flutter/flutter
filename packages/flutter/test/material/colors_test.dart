...
void main() {
  ...
  test('Colors swatches do not contain duplicates', () {
    for (final MaterialColor color in Colors.primaries) {
      expect(primaryKeys.map<Color>((final int key) => color[key]!).toSet().length, primaryKeys.length);
    }

    expect(primaryKeys.map<Color>((final int key) => Colors.grey[key]!).toSet().length, primaryKeys.length);

    for (final MaterialAccentColor color in Colors.accents) {
      expect(accentKeys.map<Color>((final int key) => color[key]!).toSet().length, accentKeys.length);
    }
  });
