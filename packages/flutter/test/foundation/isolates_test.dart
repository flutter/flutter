...
void main() {
  ...
  test('compute closes all ports', () async {
    ...
    final Future<ProcessResult> resultFuture = Process.run(dartPath, <String>[scriptPath]);
    final ProcessResult? result = await Future.any(<Future<ProcessResult?>>[
      resultFuture,
      Future<ProcessResult?>.delayed(const Duration(seconds: 5), null),
    ]);
