...
class HttpHostValidator extends DoctorValidator {
  ...
  List<String> get _requiredHosts => <String>[
        ...
        if (_featureFlags.isAndroidEnabled)
          ...androidRequiredHttpHosts(_platform),
  ...
  Future<_HostValidationResult> _checkHostAvailability(String host) async {
    ...
    try {
      ...
      return _HostValidationResult.fail(host,
          'Failed to connect to host in $timeout second${timeout == 1 ? '' : 's'}');
      ...
      if (exceptionMessage.contains('No host specified')) {
        ...
        if (pubHostedUrl != null && host == pubHostedUrl) {
          return _HostValidationResult.fail(host,
              'The value of $kEnvPubHostedUrl(${_platform.environment[kEnvPubHostedUrl]}) is not a valid host');
        }
        return _HostValidationResult.fail(host,
            'The value of $kEnvCloudUrl(${_platform.environment[kEnvCloudUrl]}) is not a valid host');
      }
      return _HostValidationResult.fail(host,
          'An error occurred while checking the HTTP host: $exceptionMessage');
    }
  }
  ...
  Future<ValidationResult> validate() async {
    ...
    final List<_HostValidationResult> availabilityResults =
        await Future.wait(availabilityResultFutures);
    ...
    if (availabilityResults
      ...
      return ValidationResult(
        ...
        messages
          ..add(
              const ValidationMessage('All required HTTP hosts are available')),
      );
    }
