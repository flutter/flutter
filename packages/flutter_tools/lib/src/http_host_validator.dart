...
List<String> androidRequiredHttpHosts(Platform platform) {
  return <String>[
    ...
    if (!platform.environment.containsKey(kEnvCloudUrl)) 'https://maven.google.com/',
  ];
}
...
class HttpHostValidator extends DoctorValidator {
    ...
    try {
      ...
      if (e.message.contains('Invalid radix-10 number')) {
        return _HostValidationResult.fail(host,
            'The value of $kDoctorHostTimeout(${_platform.environment[kDoctorHostTimeout]}) is not a valid duration in seconds');
        ...
        if (pubHostedUrl != null && host == pubHostedUrl) {
          return _HostValidationResult.fail(host,
              'The value of $kEnvPubHostedUrl(${_platform.environment[kEnvPubHostedUrl]}) could not be parsed as a valid url');
        }
        return _HostValidationResult.fail(host,
            'The value of $kEnvCloudUrl(${_platform.environment[kEnvCloudUrl]}) could not be parsed as a valid url');
      }
      return _HostValidationResult.fail(host, 'An error occurred while checking the HTTP host: ${e.message}');
      ...
      if (exceptionMessage.contains('No host specified')) {
        ...
        if (pubHostedUrl != null && host == pubHostedUrl) {
          return _HostValidationResult.fail(
              host, 'The value of $kEnvPubHostedUrl(${_platform.environment[kEnvPubHostedUrl]}) is not a valid host');
        }
        return _HostValidationResult.fail(
            host, 'The value of $kEnvCloudUrl(${_platform.environment[kEnvCloudUrl]}) is not a valid host');
      }
  ...
  Future<ValidationResult> validate() async {
    ...
    for (final _HostValidationResult result in availabilityResults) {
      messages.add(
          ValidationMessage.error('HTTP host "${result.host}" is not reachable. Reason: ${result.failResultInfo}'));
    }

    return ValidationResult(
      availabilityResults.length == _requiredHosts.length ? ValidationType.notAvailable : ValidationType.partial,
      messages,
    );
  }
}
