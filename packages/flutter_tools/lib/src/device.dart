...
abstract class DeviceManager {
  ...
  Future<List<Device>> findTargetDevices(FlutterProject? flutterProject,
    ...
    if (devices.length > 1 && !hasSpecifiedAllDevices) {
      ...
      if (devices.any((Device device) => device.ephemeral == true)) {
        ...
        final List<Device> ephemeralDevices =
            devices.where((Device device) => device.ephemeral == true).toList();
  ...
  void _displayDeviceOptions(List<Device> devices) {
    ...
    for (final Device device in devices) {
      _logger.printStatus(
          _userMessages.flutterChooseDevice(count, device.name, device.id));
      count++;
    }
  }
  ...
  static DebuggingOptions fromJson(
      ...
      DebuggingOptions._(
        ...
        skiaDeterministicRendering:
            (json['skiaDeterministicRendering'] as bool?)!,
        ...
        dumpSkpOnShaderCompilation:
            (json['dumpSkpOnShaderCompilation'] as bool?)!,
        ...
        webUseSseForInjectedClient:
            (json['webUseSseForInjectedClient'] as bool?)!,
