...
class ProjectFileInvalidator {
  ProjectFileInvalidator(this._packagesPath, this._flutterProject) {
    ...
    _hasBuilders = _flutterProject?.hasBuilders ?? false;
    _computePackageMap(_packageMap, _flutterProject, _hasBuilders);
  }
  ...
  static void _computePackageMap(Map<String, Uri> packageMap, FlutterProject flutterProject, bool hasBuilders) {
  ...
  List<String> findInvalidated() {
    ...
    if (packagesFile.existsSync()) {
      ...
      final bool hadBuilders =_hasBuilders;
      _hasBuilders = _flutterProject?.hasBuilders ?? false;
      if (hadBuilders != _hasBuilders) {
        printError('Warning: Added builders detected. The Flutter runner will require a restart to use them.');
      }
    }
    final List<String> invalidatedFiles = <String>[];
    // To support codegen for arbitrary packages, this logic will need to be
    // expanded for each package.
    for (String packageName in _packageMap.keys) {
      final Uri packageUri =_packageMap[packageName];
      if (_flutterProject != null && _flutterProject.manifest.appName == packageName && _hasBuilders) {
        final Uri generatedUri = _flutterProject
          .generated
          .childDirectory('lib')
          .uri;
        _scanDirectory(packageUri, invalidatedFiles);
        _scanDirectory(generatedUri, invalidatedFiles);
      } else {
        _scanDirectory(packageUri, invalidatedFiles);
      }
    }
