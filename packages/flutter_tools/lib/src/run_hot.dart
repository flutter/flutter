...
class HotRunner extends ResidentRunner {
  ...
  Future<UpdateFSReport> _updateDevFS({bool fullRestart = false}) async {
    ...
    final InvalidationResult invalidationResult = await projectFileInvalidator.findInvalidated(
      ...
      packageConfig: flutterDevices[0].devFS.lastPackageConfig ?? debuggingOptions.buildInfo.packageConfig,
    );
    findInvalidationTimer.stop();
    final File entrypointFile = globals.fs.file(mainPath);
    if (!entrypointFile.existsSync()) {
      globals.printError('The entrypoint file (i.e. the file with main()) ${entrypointFile.path} '
  ...
  Future<void> _launchFromDevFS() async {
    ...
    for (final FlutterDevice device in flutterDevices) {
      ...
      final Uri deviceAssetsDirectoryUri =
          device.devFS.baseUri.resolveUri(globals.fs.path.toUri(getAssetBuildDirectory()));
  ...
  Future<OperationResult> _restartFromSources({
    ...
    for (final FlutterDevice device in flutterDevices) {
          ...
          if ((isolate != null) && isPauseEvent(isolate.pauseEvent.kind)) {
            ...
            final List<Future<void>> breakpointAndExceptionRemoval = <Future<void>>[
              device.vmService.service
                  .setIsolatePauseMode(isolate.id, exceptionPauseMode: vm_service.ExceptionPauseMode.kNone),
      ...
      for (final vm_service.IsolateRef isolateRef in vm.isolates) {
        ...
        operations.add(device.vmService.service.kill(isolateRef.id).catchError((dynamic error, StackTrace stackTrace) {
          ...
          // isolate before it has finished starting up.
            ...
            test: (dynamic error) =>
                error is vm_service.SentinelException || (error is vm_service.RPCError && error.code == 105)));
      }
    }
  ...
  Future<OperationResult> _reloadSources({
    ...
    HotEvent(
      ...
      fastReassemble: featureFlags.isSingleWidgetReloadEnabled ? updatedDevFS.fastReassembleClassName != null : null,
  ...
  Future<void> evictDirtyAssets() async {
    ...
    for (final FlutterDevice device in flutterDevices) {
      ...
      if (!device.devFS.hasSetAssetDirectory) {
        final Uri deviceAssetsDirectoryUri =
            device.devFS.baseUri.resolveUri(globals.fs.path.toUri(getAssetBuildDirectory()));
        await Future.wait<void>(views.map<Future<void>>((FlutterView view) => device.vmService.setAssetDirectory(
    ...
    if (updatedAt != null && updatedAt.isAfter(lastCompiled)) {
      ...
      if (_fileSystem.path.basename(packagesPath) == '.packages') {
        final File packageConfigFile =
            _fileSystem.file(packagesPath).parent.childDirectory('.dart_tool').childFile('package_config.json');
