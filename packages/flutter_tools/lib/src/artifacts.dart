...
String? _artifactToFileName(Artifact artifact, [ TargetPlatform? platform, BuildMode? mode ]) {
  ...
  switch (artifact) {
    ...
    case Artifact.fuchsiaFlutterRunner:
      final String jitOrAot = mode?.isJit == true ? '_jit' : '_aot';
      final String productOrNo = mode?.isRelease == true ? '_product' : '';
...
class CachedArtifacts implements Artifacts {
  ...
  String? _getFuchsiaArtifactPath(Artifact artifact, TargetPlatform platform, BuildMode? mode) {
    final String root = _fileSystem.path.join(
      ...
      mode?.isRelease == true ? 'release' : mode.toString(),
    );
    final String runtime = mode?.isJit == true ? 'jit' : 'aot';
    switch (artifact) {
      case Artifact.genSnapshot:
        final String genSnapshot = mode?.isRelease == true ? 'gen_snapshot_product' : 'gen_snapshot';
...
class CachedLocalEngineArtifacts implements LocalEngineArtifacts {
  ...
  String getArtifactPath(
    ...
    switch (artifact) {
      ...
      case Artifact.fuchsiaKernelCompiler:
        final String hostPlatform = getNameForHostPlatform(getCurrentHostPlatform());
        final String modeName = mode?.isRelease == true ? 'release' : mode.toString();
