...
abstract class Artifacts {
  ...
  static LocalEngineArtifacts getLocalEngine(
      String engineSrcPath, EngineBuildPaths engineBuildPaths) {
    return LocalEngineArtifacts(engineSrcPath, engineBuildPaths.targetEngine,
...
class CachedArtifacts extends Artifacts {
  ...
  String _getAndroidArtifactPath(
    ...
    switch (artifact) {
      ...
      default:
        assert(
            false, 'Artifact $artifact not available for platform $platform.');
        return null;
    }
  }
  ...
  String _getFlutterPatchedSdkPath(BuildMode mode) {
    final String engineArtifactsPath =
        cache.getArtifactDirectory('engine').path;
    return fs.path.join(
        engineArtifactsPath,
        'common',
  ...
  String _getHostArtifactPath(
    ...
    switch (artifact) {
      ...
      case Artifact.frontendServerSnapshotForEngineDartSdk:
        ...
        return fs.path.join(engineArtifactsPath, platformDirName,
      ...
      case Artifact.platformLibrariesJson:
        return fs.path.join(_getFlutterPatchedSdkPath(mode), 'lib',
      ...
      case Artifact.dart2jsSnapshot:
        return fs.path.join(
            dartSdkPath, 'bin', 'snapshots', _artifactToFileName(artifact));
      case Artifact.kernelWorkerSnapshot:
        return fs.path.join(
            dartSdkPath, 'bin', 'snapshots', _artifactToFileName(artifact));
      case Artifact.flutterMacOSFramework:
  ...
  TargetPlatform get _currentHostPlatform {
    if (platform.isMacOS) return TargetPlatform.darwin_x64;
    if (platform.isLinux) return TargetPlatform.linux_x64;
    if (platform.isWindows) return TargetPlatform.windows_x64;
...
class LocalEngineArtifacts extends Artifacts {
  ...
  String getArtifactPath(Artifact artifact,
    ...
    switch (artifact) {
      case Artifact.snapshotDart:
        return fs.path.join(_engineSrcPath, 'flutter', 'lib', 'snapshot',
            _artifactToFileName(artifact));
      case Artifact.genSnapshot:
      ...
      case Artifact.frontendServerSnapshotForEngineDartSdk:
        return fs.path
            .join(_hostEngineOutPath, 'gen', _artifactToFileName(artifact));
      case Artifact.engineDartSdkPath:
        return fs.path.join(_hostEngineOutPath, 'dart-sdk');
      case Artifact.engineDartBinary:
        return fs.path.join(_hostEngineOutPath, 'dart-sdk', 'bin',
            _artifactToFileName(artifact));
      case Artifact.dart2jsSnapshot:
        return fs.path.join(_hostEngineOutPath, 'dart-sdk', 'bin', 'snapshots',
            _artifactToFileName(artifact));
      case Artifact.kernelWorkerSnapshot:
        return fs.path.join(_hostEngineOutPath, 'dart-sdk', 'bin', 'snapshots',
            _artifactToFileName(artifact));
  ...
  String _genSnapshotPath() {
    ...
    for (String clangDir in clangDirs) {
      ...
      if (processManager.canRun(genSnapshotPath)) return genSnapshotPath;
    }
