...
Future<ParsedProjectInfo> valdateXcodeBuild({
...
Future<int> buildXcodeArchive({
  @required ParsedProjectInfo parsedProjectInfo,
  @required BuildableIOSApp app,
  @required BuildInfo buildInfo,
  ...
  final List<String> buildCommands = <String>[
    ...
    'xcrun',
    'xcodebuild',
    '-archivePath', globals.fs.path.join(getBuildDirectory(), 'ios-release', 'App.xcarchive'),
  ...
  return processUtils.stream(buildCommands, workingDirectory: app.project.hostAppRoot.path);
}

Future<int> buildXcodeIpa({
  @required ParsedProjectInfo parsedProjectInfo,
  @required BuildableIOSApp app,
  @required BuildInfo buildInfo,
  @required String targetOverride,
...
''';
  globals.fs.file(globals.fs.path.join(getBuildDirectory(), 'ios-release', 'ExportOptions.plist'))
    ..createSync(recursive: true)
    ..writeAsStringSync(exportTemplate);
  ...
  final List<String> buildCommands = <String>[
    ...
    'xcodebuild',
    '-exportArchive',
    '-exportPath', globals.fs.path.join(getBuildDirectory(), 'ipa'),
    '-exportOptionsPlist', globals.fs.path.absolute(getBuildDirectory(), 'ios-release',  'ExportOptions.plist'),
    '-archivePath', globals.fs.path.join(getBuildDirectory(), 'ios-release', 'App.xcarchive'),
  ];
  return processUtils.stream(buildCommands, workingDirectory: app.project.hostAppRoot.path,);
}
