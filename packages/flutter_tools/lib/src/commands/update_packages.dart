...
class UpdatePackagesCommand extends FlutterCommand {
  ...
  final String _keyCherryPick = 'cherry-pick';
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (forceUpgrade || cherryPicks.isNotEmpty) {
      ...
      _updateTemplates(deps, rootProject.directory);
    ...
    await _pubGet(rootProject, !forceUpgrade && cherryPicks.isEmpty);
    await _pubGet(toolProject, !forceUpgrade && cherryPicks.isEmpty);
    await _pubGet(widgetPreviewScaffoldProject, !forceUpgrade && cherryPicks.isEmpty);
  ...
  Future<ResolvedDependencies> _upgrade(
    ...
    if (forceUpgrade) {
      ...
      if (pinnedDeps.entries.any(
        (MapEntry<String, String> cherryPick) =>
            kManuallyPinnedDependencies[cherryPick.key] != null &&
            kManuallyPinnedDependencies[cherryPick.key] != cherryPick.value,
      )) {
        throw ArgumentError(
          'Cannot cherry-pick a version that does not match the pinned versions: $kManuallyPinnedDependencies',
        );
      }
    } else {
  ...
  void _updateTemplates(ResolvedDependencies deps, Directory directory) {
    final List<String> ignoreTemplates = <String>['widget_preview_scaffold/pubspec.yaml.tmpl'];

    final Iterable<File> templates = directory
        .listSync(recursive: true)
        .whereType<File>()
        .where((File file) => file.basename == '$_pubspecName.tmpl')
        .where(
          (File file) => !ignoreTemplates.any((String template) => file.path.endsWith(template)),
        );
    final RegExp indentedLine = RegExp(r'  [^\s]');
    for (final File template in templates) {
      final List<String> inputLines = template.readAsLinesSync();
      final List<String> outputLines = <String>[];
      bool inDeps = false;
      bool inDevDeps = false;
      for (String line in inputLines) {
        final bool indented = line.startsWith(indentedLine);
        if (indented && inDeps) {
          line = _updateVersionInLine(line, deps, 'dependencies');
        } else if (indented && inDevDeps) {
          line = _updateVersionInLine(line, deps, 'dev_dependencies');
      ...
      if (!outputLines.last.endsWith('\n') && outputLines.last.isNotEmpty) {
        outputLines.add('\n');
      }
      template.writeAsStringSync(outputLines.join('\n'));
    }
  }

  String _updateVersionInLine(String line, ResolvedDependencies deps, String key) {
    final List<String> split = line.trim().split(':');
    if (split.length == 2) {
      final String packageName = split[0];
      final String version = split[1].trim();
      if (version.isNotEmpty) {
        final String? newVersion = deps.data[key]?[packageName];
        line = '$packageName: ${_versionWithCaret(newVersion ?? version)}';
      }
    }
    return line;
  }
