...
class UpdatePackagesCommand extends FlutterCommand {
  UpdatePackagesCommand() {
    argParser
      ...
      ..addFlag(
        'diagnose-downgrades',
        help: 'Determine which pub packages in the repo are causing pub to '
            'version solve to an earlier version.',
      );
  }
  ...
  bool _upgradePubspecs({
    ...
    for (final PubspecYaml pubspec in pubspecs) {
      ...
      // TODO(fujino): actually enforce this
...
String _generateFakePubspec(
  ...
  if (kManuallyPinnedDependencies.isNotEmpty) {
    ...
    kManuallyPinnedDependencies.forEach((String package, String version) {
      ...
      if (!directDependencies.contains(package)) {
        if (verbose) {
          globals.printStatus('Skipping $package because it is transitive');
        }
        return;
      }
