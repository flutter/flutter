...
class UpdatePackagesCommand extends FlutterCommand {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final PubspecYaml oldPubspec = PubspecYaml(tempDir);
    ...
    if (doUpgrade) {
      ...
      final PubspecYaml newPubspec = PubspecYaml(tempDir);
  ...
  bool _verifyUpgrade(PubspecYaml oldPubspec, PubspecYaml newPubspec) {
    ...
    for (final PubspecDependency dep in oldDeps) {
      ...
      try {
        newDep = newDeps.firstWhere((PubspecDependency currentDep) => dep.name == currentDep.name);
      } on StateError {
  ...
  Future<PackageConfig> _generateFakePackage({
    ...
    try {
      ...
      final String packageConfigString = await tempDir.childDirectory('.dart_tool').childFile('package_config.json').readAsString();
  ...
  bool _upgradePubspecs({
    ...
    for (final PubspecYaml pubspec in pubspecs) {
      ...
      if (tree._dependencyTree.containsKey(package)) {
        throw StateError('Dependency tree already contains $package!');
      }
...
class PubspecYaml {
  ...
  PubspecYaml._(this.file, this.name, this.version, this.inputData, this.checksum, this.lockFile);
  ...
  static PubspecYaml _parse(File file, File lockFile) {
    ...
    return PubspecYaml._(file, packageName!, packageVersion, result, checksum ?? PubspecChecksum(null, ''), PubspecLock.parseFile(lockFile));
  }
...
File _pubspecLockFor(Directory directory) {
  return directory.fileSystem.file(
    directory.fileSystem.path.join(directory.path, 'pubspec.lock'));
}
