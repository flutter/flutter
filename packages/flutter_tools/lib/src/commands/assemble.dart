...
class AssembleCommand extends FlutterCommand {
  ...
  Future<CustomDimensions> get usageValues async => CustomDimensions(
    commandBuildBundleTargetPlatform: (await _environment).defines[kTargetPlatform],
  ...
  Future<Event> unifiedAnalyticsUsageValues(String commandPath) async => Event.commandUsageValues(
    ...
    buildBundleTargetPlatform: (await _environment).defines[kTargetPlatform],
    buildBundleIsModule: _flutterProject.isModule,
  );

  @override
  Future<Set<DevelopmentArtifact>> get requiredArtifacts async {
    final String? platform = (await _environment).defines[kTargetPlatform];
    if (platform == null) {
  ...
  late final Future<Environment> _environment = _createEnvironment();

  /// The environmental configuration for a build invocation.
  Future<Environment> _createEnvironment() async {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    try {
      decodedDefines = decodeDartDefines((await _environment).defines, kDartDefines);
    } on FormatException {
      throwToolExit(
    ...
    if (argumentResults.wasParsed('depfile')) {
      ...
      (await _environment).depFileService.writeToFile(depfile, globals.fs.file(depfileFile));
    }
