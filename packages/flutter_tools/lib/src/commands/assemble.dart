...
class AssembleCommand extends FlutterCommand {
  ...
  List<Target> createTargets() {
    ...
    final List<Target> results = <Target>[
      for (final String targetName in rest)
  ...
  bool isDeferredComponentsTargets() {
    for (final String targetName in argResults?.rest ?? <String>[]) {
  ...
  Map<String, String> _parseDefines(List<String> values) {
    ...
    if (argResults?.wasParsed(FlutterOptions.kExtraGenSnapshotOptions) == true) {
      results[kExtraGenSnapshotOptions] = (argResults![FlutterOptions.kExtraGenSnapshotOptions] as List<String>).join(',');
    }
    if (argResults?.wasParsed(FlutterOptions.kDartDefinesOption) == true) {
    ...
    if (argResults?.wasParsed(FlutterOptions.kExtraFrontEndOptions) == true) {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final BuildResult result = await _buildSystem.build(
      ...
      buildSystemConfig: BuildSystemConfig(
        resourcePoolSize: argResults?.wasParsed('resource-pool-size') == true
    ...
    if (argResults?.wasParsed('build-inputs') == true) {
      writeListIfChanged(result.inputFiles, stringArg('build-inputs')!);
    }
    if (argResults?.wasParsed('build-outputs') == true) {
      writeListIfChanged(result.outputFiles, stringArg('build-outputs')!);
    }
    if (argResults?.wasParsed('performance-measurement-file') == true) {
    ...
    if (argResults?.wasParsed('depfile') == true) {
