...
class TestCommand extends FlutterCommand with DeviceBasedDevelopmentArtifacts {
  ...
  Future<FlutterCommandResult> verifyThenRunCommand(String commandPath) {
    ...
    if (_testFiles.isEmpty) {
      ...
      if (_testFiles.isEmpty) {
        throwToolExit('Test directory "${testDir.path}" does not appear to contain any test files.\n'
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (shardIndex != null && (shardIndex < 0 || !shardIndex.isFinite)) {
      throwToolExit('Could not parse --shard-index=$shardIndex argument. It must be an integer greater than -1.');
    }

    final int totalShards = int.tryParse(stringArg('total-shards') ?? '');
    if (totalShards != null && (totalShards <= 0 || !totalShards.isFinite)) {
      throwToolExit('Could not parse --total-shards=$totalShards argument. It must be an integer greater than zero.');
    }
    ...
    if (boolArg('coverage') || boolArg('merge-coverage')) {
      ...
      collector = CoverageCollector(
          ...
          packagesPath: globals.fs.file(buildInfo.packagesPath).parent.parent.childFile('.packages').path);
    }
  ...
  Future<void> _buildTestAsset() async {
    ...
    if (_needRebuild(assetBundle.entries)) {
      await writeBundle(globals.fs.directory(globals.fs.path.join('build', 'unit_test_assets')), assetBundle.entries);
    }
  }
...
Iterable<String> _findTests(Directory directory) {
  return directory
      ...
      .where((FileSystemEntity entity) => entity.path.endsWith('_test.dart') && globals.fs.isFileSync(entity.path))
...
bool _shouldRunAsIntegrationTests(String currentDirectory, List<String> testFiles) {
  ...
  throwToolExit('Integration tests and unit tests cannot be run in a single invocation.'
