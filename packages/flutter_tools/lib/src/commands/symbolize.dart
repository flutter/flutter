...
class SymbolizeCommand extends FlutterCommand {
  SymbolizeCommand({
    ...
    final DwarfSymbolizationService dwarfSymbolizationService = const DwarfSymbolizationService(),
  }) : _stdio = stdio,
...
StreamTransformer<String, String> _defaultTransformer(final Uint8List symbols) {
...
StreamTransformer<String, String> _testTransformer(final Uint8List buffer) {
  return StreamTransformer<String, String>.fromHandlers(
    handleData: (final String data, final EventSink<String> sink) {
      sink.add(data);
    },
    handleDone: (final EventSink<String> sink) {
      sink.close();
    },
    handleError: (final Object error, final StackTrace stackTrace, final EventSink<String> sink) {
...
class DwarfSymbolizationService {
  const DwarfSymbolizationService({
    final SymbolsTransformer symbolsTransformer = _defaultTransformer,
