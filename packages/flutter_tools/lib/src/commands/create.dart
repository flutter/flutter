...
class CreateCommand extends FlutterCommand {
  ...
  Future<FlutterCommandResult> runCommand() async {
    if (argResults['list-samples'] != null) {
      await _writeSamplesJson(argResults['list-samples'] as String);
      return null;
    }
    ...
    if (argResults['sample'] != null) {
      if (argResults['template'] != null &&
        _stringToProjectType(argResults['template'] as String ?? 'app') != _ProjectType.app) {
        throwToolExit('Cannot specify --sample with a project type other than '
          '"${getEnumName(_ProjectType.app)}"');
      }
      // Fetch the sample from the server.
      sampleCode = await _fetchSampleFromServer(argResults['sample'] as String);
    }
    ...
    if (argResults['template'] != null) {
      template = _stringToProjectType(argResults['template'] as String);
    } else {
    ...
    String organization = argResults['org'] as String;
    ...
    String error = _validateProjectDir(projectDirPath, flutterRoot: flutterRoot, overwrite: argResults['overwrite'] as bool);
    if (error != null)
      throwToolExit(error);

    final String projectName = argResults['project-name'] as String ?? fs.path.basename(projectDirPath);
    ...
    final Map<String, dynamic> templateContext = _templateContext(
      ...
      projectDescription: argResults['description'] as String,
      flutterRoot: flutterRoot,
      renderDriverTest: argResults['with-driver-test'] as bool,
      withPluginHook: generatePlugin,
      androidLanguage: argResults['android-language'] as String,
      iosLanguage: argResults['ios-language'] as String,
    );
    ...
    if (!projectDir.existsSync() || projectDir.listSync().isEmpty) {
      ...
      if (sampleCode != null && !(argResults['overwrite'] as bool)) {
    ...
    switch (template) {
      case _ProjectType.app:
        generatedFileCount += await _generateApp(relativeDir, templateContext, overwrite: argResults['overwrite'] as bool);
        break;
      case _ProjectType.module:
        generatedFileCount += await _generateModule(relativeDir, templateContext, overwrite: argResults['overwrite'] as bool);
        break;
      case _ProjectType.package:
        generatedFileCount += await _generatePackage(relativeDir, templateContext, overwrite: argResults['overwrite'] as bool);
        break;
      case _ProjectType.plugin:
        generatedFileCount += await _generatePlugin(relativeDir, templateContext, overwrite: argResults['overwrite'] as bool);
        break;
    }
    if (sampleCode != null) {
...
''');
  ...
  Future<int> _generateApp(Directory directory, Map<String, dynamic> templateContext, { bool overwrite = false }) async {
    ...
    if (argResults['with-driver-test'] as bool) {
    ...
    if (argResults['pub'] as bool) {
      await pubGet(context: PubContext.create, directory: directory.path, offline: argResults['offline'] as bool);
      await project.ensureReadyForPlatformSpecificTooling();
    }
