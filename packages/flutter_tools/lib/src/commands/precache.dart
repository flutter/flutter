...
class PrecacheCommand extends FlutterCommand {
  ...
  Set<String> _explicitArtifactSelections() {
    ...
    bool explicitlySelected(String name) => boolArg(name) && argResults?.wasParsed(name) == true;
  ...
  Future<void> validateCommand() {
    final List<String>? arguments = argResults?.arguments;
    _expandedArtifacts.forEach((String umbrellaName, List<String> childArtifactNames) {
      if (arguments == null || !arguments.contains('--no-$umbrellaName')) {
        return;
      }
      for (final String childArtifactName in childArtifactNames) {
        if (arguments.contains('--$childArtifactName')) {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    for (final DevelopmentArtifact artifact in DevelopmentArtifact.values) {
      final Feature? feature = artifact.feature;
      if (feature != null && !_featureFlags.isEnabled(feature)) {
        continue;
      }
