    ...
    return SilentStatus(
      ...
      onFinish: () {
        ...
        if (timeout > fastOperation) {
          time = getElapsedAsSeconds(stopwatch.elapsed);
        } else {
          time = getElapsedAsMilliseconds(stopwatch.elapsed);
        }
...
class AnsiSpinner extends Status {
  ...
  int get _currentLength => _animation[0].length + (_showingSlowWarning ? _slowWarning.length : 0);
...
class AnsiStatus extends AnsiSpinner {
  ...
  void writeSummaryInformation() {
    if (multilineOutput) {
      ...
      stdout.write('\b' * (_margin.length - 1)); // back over _margin; super.cancel backs up one already
    }
...
class SummaryStatus extends Status {
  ...
  /// Prints a (minimum) 5 character padded time.
  ///
  ...
  /// appends "(!)" to the time.
  ///
  /// Examples: ' 0.5s', '150ms', '1600ms', ' 3.1s (!)'
  void writeSummaryInformation() {
    stdout.write(elapsedTime.padLeft(5));
    if (seemsSlow)
      stdout.write(' (!)');
  }
}
