...
abstract class ProcessUtils {
  ...
  ///
  /// If [throwOnError] is `true`, and [allowFailures] is supplied,
  /// a [ProcessException] is only thrown on a non-zero exit code if
  /// [allowFailures] returns false when passed the exit code.
  ///
  ...
  Future<RunResult> run(
    ...
    RunResultChecker allowFailures,
...
class _DefaultProcessUtils implements ProcessUtils {
  ...
  Future<RunResult> run(
    ...
    if (timeout == null) {
      ...
      if (throwOnError && runResult.exitCode != 0 &&
          (allowFailures == null || !allowFailures(runResult.exitCode))) {
    ...
    while (true) {
      ...
      if (exitCode != null) {
        _logger.printTrace(runResult.toString());
        if (throwOnError && runResult.exitCode != 0 &&
            (allowFailures == null || !allowFailures(exitCode))) {
          runResult.throwException('Process exited abnormally:\n$runResult');
        }
        return runResult;
      }
  ...
  RunResult runSync(
    ...
    if (allowFailures != null && failedExitCode) {
      failedExitCode = !allowFailures(runResult.exitCode);
    }
