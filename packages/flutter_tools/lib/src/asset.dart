...
class _ManifestAssetBundle implements AssetBundle {
  ...
  Future<int> build({
    ...
    for (final Package package in packageConfig.packages) {
      if (package != null && package.packageUriRoot.scheme == 'file') {
        final String packageManifestPath = globals.fs.path.fromUri(
          package.packageUriRoot.resolve('../pubspec.yaml'));
...
class LicenseCollector {
  ...
  LicenseResult obtainLicenses(
    ...
    for (final Package package in packageConfig.packages) {
      ...
      if (package == null || packageUri.scheme != 'file') {
        continue;
      }
  ...
  for (final Font font in manifest.fonts) {
    ...
    for (final FontAsset fontAsset in font.fontAssets) {
      ...
      if (assetUri.pathSegments.first == 'packages' &&
          !globals.fs.isFileSync(globals.fs.path.fromUri(packageConfig[packageName].packageUriRoot.resolve('../${assetUri.path}')))) {
...
_Asset _resolvePackageAsset(Uri assetUri, PackageConfig packageConfig) {
  ...
  if (assetUri.pathSegments.length > 1) {
    ...
    if (package != null && package.packageUriRoot.scheme == 'file') {
      return _Asset(
        baseDir: globals.fs.path.fromUri(package.packageUriRoot),
        entryUri: assetUri,
