...
class CustomDeviceConfig {
  ...
  factory CustomDeviceConfig.fromJson(dynamic json) {
    ...
    final List<String>? forwardPortCommand = _castStringListOrNull(
        typedMap[_kForwardPortCommand], _kForwardPortCommand, 'null or array of strings with at least one element',
        minLength: 1);

    final RegExp? forwardPortSuccessRegex = _convertToRegexOrNull(
        typedMap[_kForwardPortSuccessRegex], _kForwardPortSuccessRegex, 'null or string-ified regex');

    final String? archString =
        _castStringOrNull(typedMap[_kPlatform], _kPlatform, 'null or one of linux-arm64, linux-x64');
    ...
    return CustomDeviceConfig(
        ...
        pingCommand: _castStringList(
            typedMap[_kPingCommand], _kPingCommand, 'array of strings with at least one element', minLength: 1),
        pingSuccessRegex:
        ...
        installCommand: _castStringList(
            typedMap[_kInstallCommand], _kInstallCommand, 'array of strings with at least one element', minLength: 1),
        uninstallCommand: _castStringList(
            typedMap[_kUninstallCommand], _kUninstallCommand, 'array of strings with at least one element',
            minLength: 1),
        runDebugCommand: _castStringList(
            typedMap[_kRunDebugCommand], _kRunDebugCommand, 'array of strings with at least one element', minLength: 1),
        ...
        screenshotCommand: _castStringListOrNull(
            typedMap[_kScreenshotCommand], _kScreenshotCommand, 'array of strings with at least one element',
            minLength: 1));
  }
  ...
  static final CustomDeviceConfig exampleUnix = exampleWindows.copyWith(
      pingCommand: const <String>['ping', '-w', '1', '-c', '1', 'raspberrypi'], explicitPingSuccessRegex: true);
  ...
  CustomDeviceConfig copyWith(
    ...
    return CustomDeviceConfig(
      ...
      forwardPortCommand:
          explicitForwardPortCommand ? forwardPortCommand : (forwardPortCommand ?? this.forwardPortCommand),
