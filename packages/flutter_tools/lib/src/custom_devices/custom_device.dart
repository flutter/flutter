...
class CustomDeviceLogReader extends DeviceLogReader {
  ...
  void listenToProcessOutput(Process process, {Encoding encoding = systemEncoding}) {
    ...
    subscriptions.add(
      process.stdout.transform<String>(decoder).transform<String>(const LineSplitter()).listen(logLinesController.add),
    );
...
class CustomDevicePortForwarder extends DevicePortForwarder {
  ...
  Future<ForwardedPort?> tryForward(int devicePort, int hostPort) async {
    final List<String> interpolated = interpolateCommand(
        _forwardPortCommand, <String, String>{'devicePort': '$devicePort', 'hostPort': '$hostPort'},
...
class CustomDeviceAppSession {
  ...
  Future<LaunchResult> start(
    ...
    final List<String> interpolated = interpolateCommand(_device._config.runDebugCommand, <String, String>{
      'remotePath': '/tmp/',
      'appName': packageName,
...
class CustomDevice extends Device {
  CustomDevice({
    ...
    required ProcessManager processManager,
        ...
        super(config.id, category: Category.mobile, ephemeral: true, platformType: PlatformType.custom);
  ...
  CustomDeviceAppSession _getOrCreateAppSession(covariant ApplicationPackage app) {
    return _sessions.putIfAbsent(app, () {
      ...
      final CustomDeviceAppSession session = CustomDeviceAppSession(
          name: name, device: this, appPackage: app, logger: _logger, processManager: _processManager);
  ...
  Future<bool> _tryPostBuild(
    ...
    final List<String> interpolated = interpolateCommand(
        _config.postBuildCommand!, <String, String>{'appName': appName, 'localPath': localPath},
  ...
  Future<bool> tryUninstall(
      ...
      Duration? timeout,
      Map<String, String> additionalReplacementValues = const <String, String>{}}) async {
    final List<String> interpolated = interpolateCommand(_config.uninstallCommand, <String, String>{'appName': appName},
        additionalReplacementValues: additionalReplacementValues);
...
class CustomDevices extends PollingDeviceDiscovery {
  ...
  Future<List<Device>> pollingGetDevices({Duration? timeout}) async {
    ...
    final Map<CustomDevice, bool> pingedDevices = Map<CustomDevice, bool>.fromIterables(
        devices, await Future.wait(devices.map((CustomDevice e) => e.tryPing(timeout: timeout))));
