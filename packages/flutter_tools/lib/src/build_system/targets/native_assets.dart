...
class DartBuild extends Target {
  ...
  Future<void> build(Environment environment) async {
    ...
    final File? recordedUsesFileToPass;
    final buildMode = BuildMode.fromCliName(buildModeEnvironment);
    if (featureFlags.isRecordUseEnabled && buildMode.isPrecompiled) {
      if (!recordedUsesFile.existsSync()) {
        throwToolExit('recorded_uses.json was not generated by the compiler.');
      }
      recordedUsesFileToPass = recordedUsesFile;
    } else {
      recordedUsesFileToPass = null;
    }
...
// TODO(dcharkes): Split the target in two. The build hooks target to run
// parallel with dart kernel compilation and the link hooks target to run
// afterwards.
class DartBuildForNative extends DartBuild {
  ...
  @override
  List<Target> get dependencies => <Target>[
    if (featureFlags.isRecordUseEnabled) const KernelSnapshot(),
  ];
}
