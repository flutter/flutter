...
Future<StreamChannel<String>> _defaultOpenChannel(Uri uri,
    {io.CompressionOptions compression = io.CompressionOptions.compressionDefault}) async {
  ...
  Future<void> handleError(dynamic e) async {
    ...
    if (attempts == 10) printStatus('This is taking longer than expected...');
  ...
  VMService(
    ...
    if (reloadSources != null) {
      _peer.registerMethod('reloadSources', (rpc.Parameters params) async {
        ...
        if (force is! bool) throw rpc.RpcException.invalidParams('Invalid \'force\': $force');
        if (pause is! bool) throw rpc.RpcException.invalidParams('Invalid \'pause\': $pause');
        ...
        try {
          ...
          throw rpc.RpcException(rpc_error_code.SERVER_ERROR, 'Error during Sources Reload: $e\n$st');
        }
      });
    ...
    if (restart != null) {
      _peer.registerMethod('hotRestart', (rpc.Parameters params) async {
        ...
        try {
          ...
          throw rpc.RpcException(rpc_error_code.SERVER_ERROR, 'Error during Hot Restart: $e\n$st');
        }
      });
    ...
    if (compileExpression != null) {
      _peer.registerMethod('compileExpression', (rpc.Parameters params) async {
        ...
        if (expression is! String || expression.isEmpty)
          throw rpc.RpcException.invalidParams('Invalid \'expression\': $expression');
        ...
        try {
          final String kernelBytesBase64 =
              await compileExpression(isolateId, expression, definitions, typeDefinitions, libraryUri, klass, isStatic);
          ...
          throw rpc.RpcException(rpc_error_code.SERVER_ERROR, 'Error during expression compilation: $e\n$st');
        }
      });
...
void _upgradeCollection(
  ...
  if (collection is ServiceMap) return;
...
abstract class ServiceObject {
  ...
  factory ServiceObject._fromMap(
    ...
    if (!_isServiceMap(map)) throw VMServiceObjectLoadError('Expected a service map', map);
  ...
  Future<ServiceObject> reload() async {
    ...
    if (!isVM && (!hasId || (immutable && loaded))) return this;
  ...
  void updateFromMap(Map<String, dynamic> map) {
    ...
    if ((_type != null) && (_type != mapType)) {
      throw VMServiceObjectLoadError('ServiceObject types must not change', map);
    }
    _type = mapType;
...
class VM extends ServiceObjectOwner {
  ...
  void _update(Map<String, dynamic> map, bool mapIsRef) {
    ...
    _pid = map['pid'];
    if (map['_heapAllocatedMemoryUsage'] != null) _heapAllocatedMemoryUsage = map['_heapAllocatedMemoryUsage'];
    _maxRSS = map['_maxRSS'];
  ...
  ServiceObject getFromMap(Map<String, dynamic> map) {
    ...
    switch (type) {
      ...
      case 'FlutterView':
        ...
        break;
      default:
        throw VMServiceObjectLoadError('VM.getFromMap called for something other than an isolate', map);
    }
  }
  ...
  Future<ServiceMap> runInView(
    ...
    return invokeRpc<ServiceMap>('_flutter.runInView', params: <String, dynamic>{
      'viewId': viewId,
  ...
  Future<void> refreshViews({bool waitForViews = false}) async {
    ...
    while (true) {
      ...
      if (_viewCache.values.isNotEmpty || !waitForViews) return;
      failCount += 1;
  ...
  List<FlutterView> allViewsWithName(String isolateFilter) {
    ...
    return _viewCache.values.where((FlutterView v) => v.uiIsolate.name.contains(isolateFilter)).toList();
  }
}
...
class HeapSpace extends ServiceObject {
  ...
  Duration get avgCollectionTime {
    final double mcs = _totalCollectionTimeInSeconds * Duration.microsecondsPerSecond / math.max(_collections, 1);
...
class Isolate extends ServiceObjectOwner {
  ...
  ServiceObject getFromMap(Map<String, dynamic> map) {
    ...
    if ((serviceObject != null) && serviceObject.canCache) _cache[mapId] = serviceObject;
    return serviceObject;
  }
  ...
  Future<Map<String, dynamic>> getObject(Map<String, dynamic> objectRef) {
    return invokeRpcRaw('getObject', params: <String, dynamic>{'objectId': objectRef['id']});
  }
  ...
  Future<List<ProgramElement>> getUnusedChangesInLastReload() async {
    final Map<String, dynamic> response = await invokeRpcRaw('_getUnusedChangesInLastReload');
    final List<Future<ProgramElement>> unusedElements = <Future<ProgramElement>>[];
    for (Map<String, dynamic> element in response['unused']) unusedElements.add(_describeElement(element));
  ...
  Future<Map<String, dynamic>> flutterToggleDebugCheckElevationsEnabled() =>
      _flutterToggle('debugCheckElevationsEnabled');
  ...
  Future<List<int>> flutterDebugSaveCompilationTrace() async {
    final Map<String, dynamic> result = await invokeFlutterExtensionRpcRaw('ext.flutter.saveCompilationTrace');
    if (result != null && result['value'] is List<dynamic>) return result['value'].cast<int>();
    return null;
  }
...
class FlutterView extends ServiceObject {
  ...
  Future<void> setAssetDirectory(Uri assetsDirectory) async {
    ...
    await owner.vmService.vm.invokeRpc<ServiceObject>('_flutter.setAssetBundlePath', params: <String, dynamic>{
  ...
  Future<void> flushUIThreadTasks() async {
    await owner.vm.invokeRpcRaw('_flutter.flushUIThreadTasks', params: <String, dynamic>{'isolateId': _uiIsolate.id});
  }
