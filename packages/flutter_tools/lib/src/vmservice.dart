...
Future<io.WebSocket> _defaultOpenChannel(
  ...
  Future<void> handleError(Object? e) async {
    ...
    if (attempts == 10) {
      ...
      logger.printStatus('If you do NOT see the Flutter application running, it might have '
          'crashed. The device logs (e.g. from adb or XCode) might have more '
          'details.');
      logger.printStatus('If you do see the Flutter application running on the device, try '
...
class FlutterVmService {
  ...
  Future<vm_service.Response?> callMethodWrapper(String method, {String? isolateId, Map<String, Object?>? args}) async {
    try {
  ...
  Future<void> setAssetDirectory({
    ...
    await callMethodWrapper(kSetAssetBundlePathMethod, isolateId: uiIsolateId, args: <String, Object?>{
      'viewId': viewId,
  ...
  Future<String> flutterDebugDumpRenderTree({
    ...
    final Map<String, Object?>? response = await invokeFlutterExtensionRpcRaw('ext.flutter.debugDumpRenderTree',
  ...
  Future<Brightness?> flutterBrightnessOverride({
    ...
    final Map<String, Object?>? result = await invokeFlutterExtensionRpcRaw(
      ...
      isolateId: isolateId,
      args: brightness != null ? <String, String>{'value': brightness.toString()} : <String, String>{},
    );
    if (result != null && result['value'] is String) {
  ...
  Future<vm_service.Response?> _checkedCallServiceExtension(
    ...
    try {
      ...
      if ((err.code == RPCErrorCodes.kMethodNotFound) || (err.code == RPCErrorCodes.kServiceDisappeared)) {
        return null;
      }
      rethrow;
    }
  ...
  Future<vm_service.IsolateRef> findExtensionIsolate(String extensionName) async {
    ...
    isolateEvents = service.onIsolateEvent.listen((vm_service.Event event) {
      if (event.kind == vm_service.EventKind.kServiceExtensionAdded && event.extensionRPC == extensionName) {
