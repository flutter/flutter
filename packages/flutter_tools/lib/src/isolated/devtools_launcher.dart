...
class DevtoolsServerLauncher extends DevtoolsLauncher {
  ...
  static final RegExp _activateDevToolsPattern = RegExp(r'Activated devtools (\d+.\d+.\d+)');
  ...
  Future<void> launch(Uri vmServiceUri) async {
    try {
      final Process _devToolsActivateProcess = await _processManager.start(<String>[
        _pubExecutable,
        'global',
        'activate',
        'devtools'
      ]);
      _logger.printStatus('Activating DevTools...');
      bool devtoolsActivated = false;
      _devToolsActivateProcess.stdout
          .transform(utf8.decoder)
          .transform(const LineSplitter())
          .listen(
            (String line) {
              final Match match = _activateDevToolsPattern.firstMatch(line);
              if (match != null) {
