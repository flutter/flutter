  ...
  Future<bool> checkAndroidDynamicFeature(List<DeferredComponent> components) async {
    ...
    for (final DeferredComponent component in components) {
      final _DeferredComponentAndroidFiles androidFiles = _DeferredComponentAndroidFiles(
          name: component.name, projectDir: projectDir, logger: logger, templatesDir: _templatesDir);
...
''');
  ...
  void clearOutputDir() {
    final Directory dir =
        projectDir.childDirectory('build').childDirectory(DeferredComponentsValidator.kDeferredComponentsTempDirectory);
...
class _DeferredComponentAndroidFiles {
  ...
  Future<Map<String, List<File>>> generateFiles(
      {Directory? alternateAndroidDir, bool clearAlternateOutputDir = false}) async {
  ...
  Future<List<File>> _setupComponentFiles(Directory outputDir) async {
    ...
    if (templatesDir != null) {
      final Directory templateComponentDir = templatesDir
          .childDirectory('module${globals.fs.path.separator}android${globals.fs.path.separator}deferred_component');
      template = Template(
    ...
    final File tempAndroidManifest =
        outputDir.childDirectory('src').childDirectory('main').childFile('AndroidManifest.xml');
