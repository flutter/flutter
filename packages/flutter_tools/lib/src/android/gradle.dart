  ...
  Future<void> buildGradleApp({
    ...
    if (_artifacts is LocalEngineArtifacts) {
      final LocalEngineArtifacts localEngineArtifacts =
          _artifacts as LocalEngineArtifacts;
      ...
      command.add(
          '-Ptarget-platform=${_getTargetPlatformByLocalEnginePath(localEngineArtifacts.engineOutPath)}');
    ...
    if (buildInfo.fileSystemRoots != null &&
        buildInfo.fileSystemRoots.isNotEmpty) {
  ...
  if (androidBuildInfo.splitPerAbi) {
    return <String>[
      for (AndroidArch androidArch in androidBuildInfo.targetArchs)
        <String>['app', getNameForAndroidArch(androidArch), ...apkPartialName]
            .join('-')
    ];
  }
  return <String>[
