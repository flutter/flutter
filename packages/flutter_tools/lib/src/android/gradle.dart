  ...
  Future<void> buildGradleApp({
    ...
    if (usesAndroidX) {
      ...
      _analytics.send(Event.flutterBuildInfo(
          label: 'app-using-android-x', buildType: 'gradle'));
    ...
    if (exitCode != 0) {
      if (detectedGradleError == null) {
        BuildEvent('gradle-unknown-failure',
            ...
            .send();
        _analytics.send(Event.flutterBuildInfo(
            label: 'gradle-unknown-failure', buildType: 'gradle'));
      ...
      final String usageLabel =
          'gradle-${detectedGradleError?.eventLabel}-failure';
      BuildEvent(usageLabel, type: 'gradle', flutterUsage: _usage).send();
      _analytics
          .send(Event.flutterBuildInfo(label: usageLabel, buildType: 'gradle'));
    ...
    if (isBuildingBundle) {
      final File bundleFile =
          findBundleFile(project, buildInfo, _logger, _usage, _analytics);
    ...
    final Iterable<String> apkFilesPaths = project.isModule
        ? findApkFilesModule(
            project, androidBuildInfo, _logger, _usage, _analytics)
