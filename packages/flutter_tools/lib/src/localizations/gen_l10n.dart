...
Future<LocalizationsGenerator> generateLocalizations({
  ...
  try {
    generator = LocalizationsGenerator(
      ...
      useNamedArgument: options.useNamedArgument,
    )
      ..loadResources()
...
List<String> generateMethodParameters(Message message, bool useNamedArgument) {
  return message.placeholders.values.map((Placeholder placeholder) {
    return '${useNamedArgument ? 'required ' : ''}${placeholder.type} ${placeholder.name}';
  }).toList();
}
...
String generateBaseClassMethod(Message message, LocaleInfo? templateArbLocale, bool useNamedArgument) {
  ...
  if (message.placeholders.isNotEmpty) {
    return (useNamedArgument ? baseClassMethodWithNamedArgumentTemplate : baseClassMethodTemplate)
      .replaceAll('@(comment)', comment)
      .replaceAll('@(templateLocaleTranslationComment)', templateLocaleTranslationComment)
      .replaceAll('@(name)', message.resourceId)
      .replaceAll('@(parameters)', generateMethodParameters(message, useNamedArgument).join(', '));
  }
...
class LocalizationsGenerator {
  ...
  /// Whether to generate the Dart localization methods with named arguments.
  ///
  ...
  String _generateCode() {
    ...
    return fileTemplate
      .replaceAll('@(header)', header.isEmpty ? '' : '$header\n')
      .replaceAll('@(class)', className)
      .replaceAll('@(methods)', _allMessages.map((Message message) => generateBaseClassMethod(message, _templateArbLocale, useNamedArgument)).join('\n'))
...
[${_inputFileNames[locale]}:${message.resourceId}] ICU Syntax Warning: The plural part specified below is overridden by a later plural part.
    ...
    ${Parser.indentForError(pluralPart.positionInMessage)}''');
      ...
      return (useNamedArgument ? methodWithNamedArgumentTemplate : methodTemplate)
                .replaceAll('@(name)', message.resourceId)
                .replaceAll('@(parameters)', generateMethodParameters(message, useNamedArgument).join(', '))
