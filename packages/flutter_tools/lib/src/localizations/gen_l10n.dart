...
LocalizationsGenerator generateLocalizations({
  ...
  final String inputPathString =
      options.arbDirectory?.path ?? fileSystem.path.join('lib', 'l10n');
  final String templateArbFileName =
      options.templateArbFile?.toFilePath() ?? 'app_en.arb';
  final String outputFileString =
      options.outputLocalizationsFile?.toFilePath() ?? 'app_localizations.dart';
...
/// ```
String _replacePlaceholdersWithVariables(
    String message, Iterable<Placeholder> placeholders,
    [String braces = '{}']) {
  ...
  for (final Placeholder placeholder in placeholders) {
    ...
    messageWithValues = messageWithValues.replaceAll(
        ...
        _needsCurlyBracketStringInterpolation(
                messageWithValues, placeholder.name)
            ? '\${$variable}'
            : '\$$variable');
  }
...
String _generatePluralMethod(Message message, String translationForMessage) {
  ...
  final String easyMessage = _replacePlaceholdersBraces(
      translationForMessage, message.placeholders, '##');
  ...
  for (final String pluralKey in pluralIds.keys) {
    ...
    if (match != null && match.groupCount == 2) {
      final String argValue = _replacePlaceholdersWithVariables(
          generateString(match.group(2)!), message.placeholders, '##');
  ...
  if (translationForMessage.startsWith('{') &&
      translationForMessage.endsWith('}')) {
...
String _replaceWithVariable(String translation, String variable) {
  String prefix =
      generateString(translation.substring(0, translation.indexOf('{')));
  prefix = prefix.substring(0, prefix.length - 1);
  String suffix =
      generateString(translation.substring(translation.lastIndexOf('}') + 1));
    ...
    for (final RegExpMatch patternMatch in patternRE.allMatches(pattern)) {
      if (patternMatch.groupCount == 2) {
        String value = patternMatch
            ...
            .replaceAll("'", r"\'")
            .replaceAll('"', r'\"');
        value = _replacePlaceholdersWithVariables(
...
bool _needsCurlyBracketStringInterpolation(
    String messageString, String placeholder) {
  ...
  final bool isPlaceholderEndOfSubstring =
      placeholderIndex + placeholder.length + 2 == messageString.length;
...
String _generateMethod(Message message, String translationForMessage) {
  String generateMessage() {
    return _replacePlaceholdersWithVariables(
        generateString(translationForMessage), message.placeholders);
  }
...
class LocalizationsGenerator {
  ...
  String _generateBaseClassFile(
    ...
    final Iterable<String> methods = messages.map((Message message) {
      ...
      return _generateMethod(
        message,
        bundle.translationFor(message) ??
            templateBundle.translationFor(message)!,
      );
    });
  ...
  String _generateSubclass(
    ...
    final Iterable<String> methods = messages
        .where((Message message) => bundle.translationFor(message) != null)
        .map((Message message) =>
            _generateMethod(message, bundle.translationFor(message)!));
  ...
  String _generateCode() {
    ...
    return fileTemplate
        ...
        .replaceAll(
            ...
            _allMessages
                .map((Message message) =>
                    generateBaseClassMethod(message, _templateArbLocale))
                .join('\n'))
