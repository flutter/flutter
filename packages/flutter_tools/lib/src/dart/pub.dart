...
class _DefaultPub implements Pub {
  ...
  Future<void> get({
    ...
    final File packageConfigFile =
        _fileSystem.file(_fileSystem.path.join(directory, '.dart_tool', 'package_config.json'));
    final Directory generatedDirectory =
        _fileSystem.directory(_fileSystem.path.join(directory, '.dart_tool', 'flutter_gen'));
    final File lastVersion = _fileSystem.file(_fileSystem.path.join(directory, '.dart_tool', 'version'));
    final File currentVersion = _fileSystem.file(_fileSystem.path.join(Cache.flutterRoot!, 'version'));
    final File pubspecYaml = _fileSystem.file(_fileSystem.path.join(directory, 'pubspec.yaml'));
    final File pubLockFile = _fileSystem.file(_fileSystem.path.join(directory, 'pubspec.lock'));
  ...
  Future<void> interactively(
    ...
    if (touchesPackageConfig) {
      ...
      final File packageConfigFile =
          _fileSystem.file(_fileSystem.path.join(targetDirectory, '.dart_tool', 'package_config.json'));
      final Directory generatedDirectory =
          _fileSystem.directory(_fileSystem.path.join(targetDirectory, '.dart_tool', 'flutter_gen'));
      final File lastVersion = _fileSystem.file(_fileSystem.path.join(targetDirectory, '.dart_tool', 'version'));
      final File currentVersion = _fileSystem.file(_fileSystem.path.join(Cache.flutterRoot!, 'version'));
      lastVersion.writeAsStringSync(currentVersion.readAsStringSync());
      await _updatePackageConfig(
        packageConfigFile,
  ...
  List<String> _pubCommand(List<String> arguments) {
    ...
    if (!_processManager.canRun(sdkPath)) {
      throwToolExit('Your Flutter SDK download may be corrupt or missing permissions to run. '
  ...
  Future<void> _updatePackageConfig(
    ...
    packageConfigFile.parent.childFile('package_config_subset').writeAsStringSync(_computePackageConfigSubset(
          packageConfig,
          _fileSystem,
        ));
    ...
    final Map<String, dynamic> jsonContents = json.decode(packageConfigFile.readAsStringSync()) as Map<String, dynamic>;
