...
class _DefaultPub implements Pub {
  ...
  Future<void> get({
    ...
    final File pubspecYaml =
        _fileSystem.file(_fileSystem.path.join(directory, 'pubspec.yaml'));
    ...
    try {
      ...
      if (exception is io.ProcessException) {
        ...
        final Map<String, String> env =
            await _createPubEnvironment(context, flutterRootOverride);
    ...
    if (!packageConfigFile.existsSync()) {
      throwToolExit(
          '$directory: pub did not create .dart_tools/package_config.json file.');
    }
  ...
  Future<void> interactively(
    ...
    if (touchesPackageConfig) {
      final String targetDirectory =
          directory ?? _fileSystem.currentDirectory.path;
      final File packageConfigFile = _fileSystem.file(_fileSystem.path
          .join(targetDirectory, '.dart_tool', 'package_config.json'));
      final Directory generatedDirectory = _fileSystem.directory(
  ...
  /// [context] provides extra information to package server requests to
  /// understand usage.
  Future<Map<String, String>> _createPubEnvironment(PubContext context,
      [String? flutterRootOverride]) async {
