...
bool _validateMetadataMap(Object? yamlRoot, Map<String, Type> validations, Logger logger) {
  ...
  for (final MapEntry<String, Object> entry in validations.entries) {
    ...
    if (map[entry.key] != null && (map[entry.key] as Object).runtimeType != entry.value) {
      isValid = false;
      logger.printTrace(
          'The value of key `${entry.key}` in .metadata was expected to be ${entry.value} but was ${(map[entry.key] as Object).runtimeType}');
      break;
    }
  }
  return isValid;
...
class FlutterProjectMetadata {
  ...
  FlutterProjectMetadata(File file, Logger logger)
      : _metadataFile = file,
        _logger = logger,
...
${migrateConfig.getOutputFileString()}''', flush: true);
  }
...
class MigrateConfig {
  MigrateConfig(
      {Map<SupportedPlatform, MigratePlatformConfig>? platformConfigs, this.unmanagedFiles = _kDefaultUnmanagedFiles})
  ...
  void populate({
    ...
    final FlutterProject flutterProject =
        projectDirectory == null ? FlutterProject.current() : FlutterProject.fromDirectory(projectDirectory);
      ...
      if (platformConfigs.containsKey(platform)) {
        ...
        if (create) {
          platformConfigs[platform] =
              MigratePlatformConfig(createRevision: createRevision, baseRevision: currentRevision);
        }
      }
    }
  }
      ...
      if (platformsYaml is YamlList && platformsYaml.isNotEmpty) {
        for (final Object? platform in platformsYaml) {
          if (_validateMetadataMap(
            ...
            final SupportedPlatform platformString = SupportedPlatform.values.firstWhere((SupportedPlatform val) =>
