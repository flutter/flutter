...
bool _validateMetadataMap(
    Object? yamlRoot, Map<String, Type> validations, Logger logger) {
  ...
  for (final MapEntry<String, Object> entry in validations.entries) {
    ...
    if (map[entry.key] != null &&
        (map[entry.key] as Object).runtimeType != entry.value) {
      isValid = false;
      logger.printTrace(
...
class FlutterProjectMetadata {
  ...
  FlutterProjectMetadata(File file, Logger logger)
    ...
    if (yamlRoot == null || yamlRoot is! YamlMap) {
      _logger.printTrace(
          '.metadata file at ${_metadataFile.path} was empty or malformed.');
      return;
    }
...
class MigrateConfig {
  ...
  bool get isEmpty =>
      ...
      (unmanagedFiles.isEmpty || unmanagedFiles == _kDefaultUnmanagedFiles);
      ...
      if (platformConfigs.containsKey(platform)) {
        ...
        if (create) {
          platformConfigs[platform] = MigratePlatformConfig(
              createRevision: createRevision, baseRevision: currentRevision);
        }
      }
    }
  }
      ...
      if (platformsYaml is YamlList && platformsYaml.isNotEmpty) {
        for (final Object? platform in platformsYaml) {
          if (_validateMetadataMap(
            ...
            final SupportedPlatform platformString = SupportedPlatform.values
