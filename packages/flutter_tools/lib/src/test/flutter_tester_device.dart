...
class FlutterTesterTestDevice extends TestDevice {
  ...
  Future<StreamChannel<String>> start(String entrypointPath) async {
    ...
    final HttpServer server = await bind(host, /*port*/ 0);
    _server = server;
    logger.printTrace('test $id: test harness socket server is running at port:${server.port}');
    ...
    final Process runningProcess = await processManager.start(command, environment: environment);
    _process = runningProcess;

    // Unawaited to update state.
    unawaited(runningProcess.exitCode.then((int exitCode) {
      logger.printTrace('test $id: flutter_tester process at pid ${runningProcess.pid} exited with code=$exitCode');
      _exitCode.complete(exitCode);
    }));

    logger.printTrace('test $id: Started flutter_tester process at pid ${runningProcess.pid}');
  ...
  String toString() {
    ...
    final String status = process != null
        ? 'pid: ${process.pid}, ${_exitCode.isCompleted ? 'exited' : 'running'}'
        : 'not started';
