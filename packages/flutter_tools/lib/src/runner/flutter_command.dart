...
abstract class _HttpRegex {
  ...
  static final RegExp httpHeader = RegExp(
      '^([$_nonDelimiterVchar]+)' r'\s*=\s*' '([$_vchar$_spaceOrTab]+)' r'$');
}
...
abstract class FlutterCommand extends Command<void> {
  ...
  void usesWebRendererOption() {
    argParser.addOption(
        ...
        help:
            'This option is deprecated and will be removed in a future Flutter '
            'release.\n'
  ...
  BuildMode getBuildMode() {
    ...
    bool argIfDefined(String flagName, bool ifNotDefined) {
      return argParser.options.containsKey(flagName)
          ? boolArg(flagName)
          : ifNotDefined;
    }

    final bool debugResult = !_excludeDebug && argIfDefined('debug', false);
    final bool jitReleaseResult =
        !_excludeRelease && argIfDefined('jit-release', false);
    final bool releaseResult =
        !_excludeRelease && argIfDefined('release', false);
  ...
  Future<BuildInfo> getBuildInfo({
    ...
    final bool androidSkipBuildDependencyValidation = !argParser.options
            .containsKey(
                FlutterOptions.kAndroidSkipBuildDependencyValidation) ||
    ...
    if (globalResults?.wasParsed(FlutterGlobalOptions.kLocalWebSDKOption) ??
        false) {
      useLocalWebSdk =
          stringArg(FlutterGlobalOptions.kLocalWebSDKOption, global: true) !=
              null;
    }
    final bool useLocalCanvasKit =
        forcedUseLocalCanvasKit ?? (!useCdn || useLocalWebSdk);

    final String? defaultFlavor = project.manifest.defaultFlavor;
    final String? cliFlavor =
    ...
    if (flavor != null) {
      ...
      if (dartDefines
          .any((String define) => define.startsWith('FLUTTER_APP_FLAVOR'))) {
        throwToolExit(
  ...
  Future<Event> unifiedAnalyticsUsageValues(String commandPath) async =>
      Event.commandUsageValues(
          workflow: commandPath, commandHasTerminal: hasTerminal);
    ...
    return context.run<void>(
      ...
      body: () async {
        ...
        try {
          ...
          globals.printTrace(globals.userMessages.flutterElapsedTime(
  ...
  Map<String, String> extractWebHeaders() {
    ...
    if (argParser.options.containsKey('web-header')) {
      ...
      for (final String candidate in candidates) {
        final Match? keyValueMatch =
            _HttpRegex.httpHeader.firstMatch(candidate);
        if (keyValueMatch == null) {
...
''');
    ...
    if (shouldRunPub) {
      final Environment environment = Environment(
        ...
        useImplicitPubspecResolution: globalResults!
            .flag(FlutterGlobalOptions.kImplicitPubspecResolution),
      );
      ...
      if (stringArg(FlutterGlobalOptions.kDeviceIdOption, global: true) ==
          'preview') {
      ...
      await project.regeneratePlatformSpecificTooling(
        ...
        useImplicitPubspecResolution: boolArg(
            FlutterGlobalOptions.kImplicitPubspecResolution,
            global: true),
      );
      if (reportNullSafety) {
    ...
    if (commandPath != null) {
      ...
      final List<Object> pairOfUsageValues =
  ...
  Future<void> validateCommand() async {
    ...
    if (_usesTargetOption) {
      ...
      if (!globals.fs.isFileSync(targetPath)) {
        throwToolExit(
            globals.userMessages.flutterTargetFileMissing(targetPath));
      }
    }
  }
