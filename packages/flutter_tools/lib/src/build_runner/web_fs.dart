...
class WebFs {
  ...
  static Future<WebFs> start({
    ...
    final BuildDaemonClient client = await buildDaemonCreator
      .startBuildDaemon(fs.currentDirectory.path, release: buildInfo.isRelease, target: target);
    client.startBuild();
...
class BuildDaemonCreator {
  ...
  Future<BuildDaemonClient> startBuildDaemon(String workingDirectory, {bool release = false, @required String target}) async {
    try {
  ...
  Future<BuildDaemonClient> _connectClient(
    ...
    { bool release, @required String target }
  ) {
    ...
    return BuildDaemonClient.connect(
      ...
      <String>[
        ...
        '--define', 'flutter_tools:entrypoint=targets=$target',
