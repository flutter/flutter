...
BuildApp() {
  ...
  if [[ -e "${project_path}/.macos" ]]; then
    derived_dir="${project_path}/.macos/Flutter"
  fi
  ...
  local framework_path="${FLUTTER_ROOT}/bin/cache/artifacts/engine/darwin-x64"
  ...
  local flutter_framework="${framework_path}/FlutterMacOS.framework"
  ...
  if [[ -n "$LOCAL_ENGINE" ]]; then
    if [[ $(echo "$LOCAL_ENGINE" | tr "[:upper:]" "[:lower:]") != *"$build_mode"* ]]; then
      ...
      EchoError "  flutter build macos --local-engine=macos_${build_mode}"
      EchoError "or"
      EchoError "  flutter build macos --local-engine=macos_${build_mode}_unopt"
      EchoError "========================================================================"
      exit -1
    fi
  ...
  if [[ -e "${project_path}/.macos" ]]; then
    ...
    RunCommand find "${derived_dir}/engine/FlutterMacOS.framework" -type f -exec chmod a-w "{}" \;
  else
  ...
  local plistPath="${project_path}/macos/Flutter/AppFrameworkInfo.plist"
  if [[ -e "${project_path}/.macos" ]]; then
    plistPath="${project_path}/.macos/Flutter/AppFrameworkInfo.plist"
  fi
...
EmbedFlutterFrameworks() {
  ...
  if [[ -n "$identity" && "$identity" != "\"\"" ]]; then
    ...
    RunCommand codesign --force --verbose --sign "${identity}" -- "${xcode_frameworks_dir}/FlutterMacOS.framework/Flutter"
  fi
}
