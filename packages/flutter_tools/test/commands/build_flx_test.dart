...
void main() {
  group('build snapshot', () {
    ...
    testUsingContext('rebuilds if target changes', () async {
      ...
      runSync(<String>['flutter', 'build', 'flx', '--target', 'lib/blue.dart'], workingDirectory: projectDir.path, allowReentrantFlutter: true);
      final DateTime timestampBlue = fs.file(snapshot).statSync().changed;
      runSync(<String>['flutter', 'build', 'flx', '--target', 'lib/red.dart'], workingDirectory: projectDir.path, allowReentrantFlutter: true);
