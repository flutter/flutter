...
Pod::Spec.new do |s|
  ...
  s.platform = :ios, '15.0'
      ...
      expect(
        logger.errorText,
        contains('The plugin "$fakePluginName" requires a higher minimum iOS '
...
Pod::Spec.new do |s|
  ...
  s.ios.deployment_target = '15.0'
    ...
    testUsingContext(
        'throws if plugin requires unknown higher minimum iOS version',
        () async {
      final FlutterProject projectUnderTest = setupProjectUnderTest();
      pretendPodIsInstalled();
      pretendPodVersionIs('100.0.0');
  ...
  spec.platform = :osx, "12.7"
      ...
      fakeProcessManager.addCommand(
        FakeCommand(
          ...
          exitCode: 1,
          stdout:
              _fakeHigherMinimumMacOSVersionPodInstallOutput(fakePluginName),
        ),
      );
      ...
      expect(
        logger.errorText,
        contains('The plugin "$fakePluginName" requires a higher minimum macOS '
            'deployment version than your application is targeting.'),
      );
      // The error should contain specific instructions for fixing the build
      // based on parsing the plugin's podspec.
        ...
        testUsingContext(
          ...
          expect(
              ...
              contains(Event.appleUsageEvent(
                  workflow: 'pod-install-failure', parameter: 'arm-ffi')));
        });
      }
...
String _fakeHigherMinimumIOSVersionPodInstallOutput(String fakePluginName,
    {String subdir = 'ios'}) {
  return '''
Preparing
...
String _fakeHigherMinimumMacOSVersionPodInstallOutput(String fakePluginName,
    {String subdir = 'macos'}) {
  return '''
Preparing
