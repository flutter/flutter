  ...
  group('copyDirectorySync', () {
    ...
    testWithoutContext('test directory copy with followLinks: true', () async {
      ...
      final Directory tempDir = fileSystem.systemTempDirectory
          .createTempSync('flutter_copy_directory.');
      try {
        final String sourcePath =
            io.Platform.isWindows ? r'some\origin' : 'some/origin';
        ...
        sourceDirectory
            .childLink('absolute_linked.txt')
            .createSync(sourceFile1.absolute.path);
        final DateTime writeTime = sourceFile1.lastModifiedSync();
        ...
        sourceSubDirectory
            ...
            .writeAsStringSync('file 2');
        final String subdirectorySourcePath =
            io.Platform.isWindows ? r'dir1\dir2' : 'dir1/dir2';
        sourceDirectory
            .childLink('relative_linked_sub_dir')
            .createSync(subdirectorySourcePath);
        sourceDirectory
            .childDirectory('empty_directory')
            .createSync(recursive: true);
        ...
        expect(targetDirectory.childFile('some_file.txt').readAsStringSync(),
            'file 1');
        expect(
            targetDirectory.childFile('absolute_linked.txt').readAsStringSync(),
            'file 1');
        expect(targetDirectory.childLink('absolute_linked.txt').existsSync(),
            false);
        expect(
            targetDirectory
                .childDirectory('dir1')
        ...
        expect(
            ...
            true);
        expect(
            targetDirectory.childLink('relative_linked_sub_dir').existsSync(),
            false);
        expect(
            targetDirectory
                .childDirectory('relative_linked_sub_dir')
        ...
        expect(
            targetDirectory
                ...
                .readAsStringSync(),
            'file 2');
        expect(targetDirectory.childDirectory('empty_directory').existsSync(),
            true);

        // Assert that the copy operation hasn't modified the original file in some way.
        expect(sourceDirectory.childFile('some_file.txt').lastModifiedSync(),
            writeTime);
