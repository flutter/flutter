...
void main() {
  group('output preferences', () {
    testWithoutContext('can wrap output', () async {
      final BufferLogger bufferLogger = BufferLogger(
        ...
        terminal:
            TestTerminal(platform: FakePlatform()..stdoutSupportsAnsi = true),
      );
  ...
  group('ANSI coloring, bold, and clearing', () {
    ...
    testWithoutContext('clearing lines works', () {
      expect(
        ...
        equals('${AnsiTerminal.cursorBeginningOfLineCode}'
    ...
    testWithoutContext(
        'clearing lines when color is not supported does not work', () {
      terminal = AnsiTerminal(
        stdio: Stdio(), // Danger, using real stdio.
  ...
  testWithoutContext('AnsiTerminal.preferredStyle', () {
    ...
    expect(
        AnsiTerminal(stdio: stdio, platform: const LocalPlatform())
            .preferredStyle,
  ...
  testWithoutContext('set singleCharMode resilient to StdinException',
      () async {
