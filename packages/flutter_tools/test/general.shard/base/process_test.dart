...
void main() {
  ...
  group('run', () {
    ...
    testWithoutContext(' does not throw on failure with allowFailures', () async {
      when(mockProcessManager.run(<String>['kaboom'])).thenAnswer((_) {
        return Future<ProcessResult>.value(ProcessResult(0, 1, '', ''));
      });
      expect(
        (await processUtils.run(
          <String>['kaboom'],
          throwOnError: true,
          allowFailures: (int c) => c == 1,
        )).exitCode,
        1,
      );
    });

    testWithoutContext(' throws on failure when not in allowlist', () async {
      when(mockProcessManager.run(<String>['kaboom'])).thenAnswer((_) {
