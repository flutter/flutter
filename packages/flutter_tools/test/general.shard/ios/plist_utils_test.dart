...
void main() {
  group('PlistUtils', () {
    // Purposely don't use `skip` since that would count as tech debt, and
    // us only testing on MacOS is very much intended here.
    if (!Platform.isMacOS) {
      return;
    }
    ...
    group('getValueFromFile', () {
      ...
      testUsingContext('works with xml file', () async {
        ...
        expect(plistUtils.getValueFromFile(file.path, 'CFBundleIdentifier'), 'io.flutter.flutter.app');
        expect(plistUtils.getValueFromFile(file.absolute.path, 'CFBundleIdentifier'), 'io.flutter.flutter.app');
      ...
      testUsingContext('returns null for non-existent plist file', () async {
        expect(plistUtils.getValueFromFile('missing.plist', 'CFBundleIdentifier'), null);
        expect(testLogger.statusText, isEmpty);
        expect(testLogger.errorText, isEmpty);
      }, overrides: overrides);
      ...
      testUsingContext('returns null for non-existent key within plist', () async {
        file.writeAsBytesSync(base64.decode(base64PlistXml));
        expect(plistUtils.getValueFromFile('missing.plist', 'BadKey'), null);
        expect(testLogger.statusText, isEmpty);
        expect(testLogger.errorText, isEmpty);
      }, overrides: overrides);
      ...
      testUsingContext('returns null for malformed plist file', () async {
        ...
        expect(plistUtils.getValueFromFile(file.path, 'CFBundleIdentifier'), null);
