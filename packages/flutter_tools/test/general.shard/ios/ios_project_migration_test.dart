...
void main() {
  group('iOS migration', () {
    ...
    testWithoutContext('migrators succeed', () async {
      final fakeIOSMigrator = FakeIOSMigrator();
      final migration = ProjectMigration(<ProjectMigrator>[fakeIOSMigrator]);
      await migration.run();
    });
    ...
    group('remove framework linking and embedding migration', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = RemoveFrameworkLinkAndEmbeddingMigration(
          project,
          testLogger,
          fakeAnalytics,
        );
...
''');
    ...
    group('new Xcode build system', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = XcodeBuildSystemMigration(project, testLogger);
        await iosProjectMigration.migrate();
...
<dict>
    ...
    group('Xcode default build location', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = ProjectBuildLocationMigration(project, testLogger);
        await iosProjectMigration.migrate();
...
''');
    ...
    group('remove Runner project base configuration', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = ProjectBaseConfigurationMigration(project, testLogger);
        await iosProjectMigration.migrate();
        expect(xcodeProjectInfoFile.existsSync(), isFalse);
...
''');
    ...
    group('update deployment target version', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = IOSDeploymentTargetMigration(project, testLogger);
        await iosProjectMigration.migrate();
        expect(xcodeProjectInfoFile.existsSync(), isFalse);
      ...
      testWithoutContext('skipped if nothing to upgrade', () async {
        const xcodeProjectInfoFileContents = 'IPHONEOS_DEPLOYMENT_TARGET = 13.0;';
        xcodeProjectInfoFile.writeAsStringSync(xcodeProjectInfoFileContents);

        const appFrameworkInfoPlistContents = '''
...
''';
        ...
        const podfileFileContents = "# platform :ios, '13.0'";
...
''');
    ...
    group('update Xcode project object version', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = XcodeProjectObjectVersionMigration(project, testLogger);
        await iosProjectMigration.migrate();
        expect(xcodeProjectInfoFile.existsSync(), isFalse);
      ...
      testWithoutContext('skipped if nothing to upgrade', () async {
        const xcodeProjectInfoFileContents = '''
	classes = {
	};
	objectVersion = 54;
	objects = {
      ...
      ''';
        ...
        const xcodeProjectSchemeFileContents = '''
...
''');
    ...
    group('update info.plist migration', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = HostAppInfoPlistMigration(project, testLogger);
        await iosProjectMigration.migrate();
...
''';
        ...
        final migration = UISceneDelegateMigration(project, testLogger);
...
'''),
    ...
    group('remove bitcode build setting', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final migration = RemoveBitcodeMigration(project, testLogger);
        await migration.migrate();
        expect(xcodeProjectInfoFile.existsSync(), isFalse);
...
''');
    ...
    group('CocoaPods script readlink', () {
      ...
      testWithoutContext('skipped if files are missing', () async {
        final iosProjectMigration = CocoaPodsScriptReadlink(
          project,
  ...
  if [ -L "${source}" ]; then
    ...
    source="$(readlink "${source}")"
        ...
        final xcode142ProjectInterpreter = XcodeProjectInterpreter.test(
          processManager: processManager,
...
''');
    ...
    group('Cocoapods migrate toolchain directory', () {
      ...
      testWithoutContext('skip if directory is missing', () async {
        final iosProjectMigration = CocoaPodsToolchainDirectoryMigration(
          project,
...
''';
        ...
        final xcode14ProjectInterpreter = XcodeProjectInterpreter.test(
          processManager: processManager,
...
''');
  ...
  group('update Xcode script build phase', () {
    ...
    testWithoutContext('skipped if files are missing', () async {
      final iosProjectMigration = XcodeScriptBuildPhaseMigration(project, testLogger);
      await iosProjectMigration.migrate();
      expect(xcodeProjectInfoFile.existsSync(), isFalse);
...
''');
  ...
  group('update Xcode Thin Binary build phase to have input path', () {
    ...
    testWithoutContext('skipped if files are missing', () async {
      final iosProjectMigration = XcodeThinBinaryBuildPhaseInputPathsMigration(project, testLogger);
      await iosProjectMigration.migrate();
      expect(xcodeProjectInfoFile.existsSync(), isFalse);
    ...
    testWithoutContext('skipped if nothing to upgrade', () async {
      const xcodeProjectInfoFileContents = r'''
/* Begin PBXShellScriptBuildPhase section */
		3B06AD1E1E4923F5004D2608 /* Thin Binary */ = {
			isa = PBXShellScriptBuildPhase;
			alwaysOutOfDate = 1;
...
''');
  ...
  group('migrate @UIApplicationMain attribute to @main', () {
    ...
    testWithoutContext('skipped if files are missing', () async {
      final migration = UIApplicationMainDeprecationMigration(project, testLogger);
      await migration.migrate();
