...
''';
    ...
    group('list devices', () {
      testWithoutContext('Handles FileSystemException deleting temp directory',
          () async {
        final Directory tempDir =
            fileSystem.systemTempDirectory.childDirectory('core_devices.rand0');
        ...
        await deviceControl.getCoreDevices();
        expect(logger.errorText,
            contains('Error executing devicectl: ProcessException'));
        expect(fakeProcessManager, hasNoRemainingExpectations);
      });

      testWithoutContext('Handles json file mysteriously disappearing',
          () async {
        final Directory tempDir =
            fileSystem.systemTempDirectory.childDirectory('core_devices.rand0');
        final File tempFile = tempDir.childFile('core_device_list.json');
        ...
        await expectLater(
          ...
          throwsA(
            isA<StateError>().having(
              ...
              'message',
              contains(
                  'Expected the file ${tempFile.path} to exist but it did not'),
            ),
          ),
        );
        expect(
...
''';
        ...
        expect(devices[0].coreDeviceIdentifier,
