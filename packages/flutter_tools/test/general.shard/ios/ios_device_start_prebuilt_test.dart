...
void main() {
  ...
  group('IOSDevice.startApp for CoreDevice', () {
    group('in debug mode', () {
      ...
      testUsingContext('IOSDevice.startApp fails to find Dart VM in CI',
          () async {
        final FileSystem fileSystem = MemoryFileSystem.test();
        final FakeProcessManager processManager = FakeProcessManager.empty();
        ...
        const String projectLogsPath =
            'Runner-project1/Logs/Launch/Runner.xcresults';
        fileSystem
            .directory(
                '$pathToHome/Library/Developer/Xcode/DerivedData/$projectLogsPath')
        ...
        await FakeAsync().run((FakeAsync time) {
          ...
          futureLaunchResult.then((LaunchResult launchResult) {
            ...
            expect(
                fileSystem
                    .directory(
                        '$pathToFlutterLogs/DerivedDataLogs/$projectLogsPath')
                    .existsSync(),
                true);
            completer.complete();
          });
        ...
        MDnsVmServiceDiscovery: () =>
            FakeMDnsVmServiceDiscovery(returnsNull: true),
      });
    });
  });
}
...
IOSDevice setUpIOSDevice({
  ...
  final FakePlatform macPlatform = platform ??
      FakePlatform(
        operatingSystem: 'macos',
        environment: <String, String>{},
      );
