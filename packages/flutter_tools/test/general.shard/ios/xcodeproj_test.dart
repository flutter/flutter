...
''';
  ...
  testWithoutContext('expected scheme for flavored build is the title-cased flavor', () {
    expect(XcodeProjectInfo.expectedSchemeFor(const BuildInfo(BuildMode.debug, 'hello', fontSubset: false)), 'Hello');
    expect(XcodeProjectInfo.expectedSchemeFor(const BuildInfo(BuildMode.profile, 'HELLO', fontSubset: false)), 'HELLO');
    expect(XcodeProjectInfo.expectedSchemeFor(const BuildInfo(BuildMode.release, 'Hello', fontSubset: false)), 'Hello');
  });
  testWithoutContext('expected build configuration for flavored build is Mode-Flavor', () {
    expect(XcodeProjectInfo.expectedBuildConfigurationFor(const BuildInfo(BuildMode.debug, 'hello', fontSubset: false), 'Hello'), 'Debug-Hello');
    expect(XcodeProjectInfo.expectedBuildConfigurationFor(const BuildInfo(BuildMode.profile, 'HELLO', fontSubset: false), 'Hello'), 'Profile-Hello');
    expect(XcodeProjectInfo.expectedBuildConfigurationFor(const BuildInfo(BuildMode.release, 'Hello', fontSubset: false), 'Hello'), 'Release-Hello');
  });
  ...
  testWithoutContext('scheme for default project is Runner', () {
    ...
    expect(info.schemeFor(const BuildInfo(BuildMode.debug, 'unknown', fontSubset: false)), isNull);
  });
  ...
  testWithoutContext('scheme for project with custom schemes is matched against flavor', () {
    ...
    expect(info.schemeFor(const BuildInfo(BuildMode.debug, 'free', fontSubset: false)), 'Free');
    expect(info.schemeFor(const BuildInfo(BuildMode.profile, 'Free', fontSubset: false)), 'Free');
    expect(info.schemeFor(const BuildInfo(BuildMode.release, 'paid', fontSubset: false)), 'Paid');
    expect(info.schemeFor(const BuildInfo(BuildMode.debug, null, fontSubset: false)), isNull);
    expect(info.schemeFor(const BuildInfo(BuildMode.debug, 'unknown', fontSubset: false)), isNull);
  });
  ...
  testWithoutContext('build configuration for project with custom schemes is matched against BuildMode and flavor', () {
    ...
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.debug, 'free', fontSubset: false), 'Free'), 'debug (free)');
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.debug, 'Paid', fontSubset: false), 'Paid'), 'Debug paid');
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.profile, 'FREE', fontSubset: false), 'Free'), 'profile - Free');
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.release, 'paid', fontSubset: false), 'Paid'), 'Release-Paid');
  });
  ...
  testWithoutContext('build configuration for project with inconsistent naming is null', () {
    ...
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.debug, 'Free', fontSubset: false), 'Free'), null);
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.profile, 'Free', fontSubset: false), 'Free'), null);
    expect(info.buildConfigurationFor(const BuildInfo(BuildMode.release, 'Paid', fontSubset: false), 'Paid'), null);
  });
 group('environmentVariablesAsXcodeBuildSettings', () {
  ...
  group('updateGeneratedXcodeProperties', () {
    ...
    testUsingOsxContext('sets ARCHS=armv7 when armv7 local engine is set', () async {
      ...
      const BuildInfo buildInfo = BuildInfo(BuildMode.debug, null, fontSubset: false);
    ...
    testUsingOsxContext('sets TRACK_WIDGET_CREATION=true when trackWidgetCreation is true', () async {
      when(mockArtifacts.getArtifactPath(Artifact.flutterFramework,
          platform: TargetPlatform.ios, mode: anyNamed('mode'))).thenReturn('engine');
      when(mockArtifacts.engineOutPath).thenReturn(fs.path.join('out', 'ios_profile_arm'));
      const BuildInfo buildInfo = BuildInfo(BuildMode.debug, null, trackWidgetCreation: true, fontSubset: false);
      final FlutterProject project = FlutterProject.fromPath('path/to/project');
      await updateGeneratedXcodeProperties(
        project: project,
        buildInfo: buildInfo,
