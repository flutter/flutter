...
void main() {
  ...
  testWithoutContext('Connects to device VM Service and runs test application', () async {
    ...
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(
        command: <String>['dart-sdk/bin/pub', 'run', 'test', '--enable-experiment=non-nullable', 'foo.test', '-rexpanded'],
        exitCode: 23,
        environment: <String, String>{
          'FOO': 'BAR',
          'VM_SERVICE_URL': 'http://127.0.0.1:1234/' // dds forwarded URI
  ...
  testWithoutContext('Connects to device VM Service and runs test application without dds', () async {
    ...
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(
        command: <String>['dart-sdk/bin/pub', 'run', 'test', 'foo.test', '-rexpanded'],
        exitCode: 11,
        environment: <String, String>{
...
FlutterDriverService setUpDriverService({
  ...
  return FlutterDriverService(
    ...
    dartSdkPath: 'dart',
    pubExecutablePath: 'dart-sdk/bin/pub',
