...
''') as YamlMap;
      ...
      expect(plugin.platforms, contains('foo_platform'));
      final PluginPlatform? customPlatform = plugin.platforms['foo_platform'];
      expect(customPlatform, isNotNull);
      expect(customPlatform, isA<CustomPlatformPlugin>());
      final customPlatformPlugin = customPlatform! as CustomPlatformPlugin;
      expect(customPlatformPlugin.configuration, containsPair('pluginClass', 'CustomPlugin'));
      expect(customPlatformPlugin.configuration, containsPair('someCustomKey', 'customValue'));
    });

    testWithoutContext('resolves custom platform implementations', () {
      final customPlugin = Plugin(
        ...
        pluginDartClassPlatforms: const <String, DartPluginClassAndFilePair>{},
        platforms: const <String, PluginPlatform>{
          'foo_platform': CustomPlatformPlugin(
            configuration: <String, Object?>{'pluginClass': 'CustomPluginClass'},
          ),
        },
        dependencies: const <String>[],
        isDirectDependency: true,
      ...
      final plugins = <Plugin>[customPlugin];

      final List<Plugin> resolved = resolvePluginImplementationsForPlatform(
        plugins,
        'foo_platform',
      );
      expect(resolved, contains(customPlugin));
    });
  });
