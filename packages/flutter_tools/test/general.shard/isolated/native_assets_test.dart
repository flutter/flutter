...
import 'package:flutter_tools/src/isolated/native_assets/linux/native_assets.dart'
    show kArBinary, kClangBinary, kLdBinary;
...
void main() {
  ...
  setUp(() {
    ...
    clang = fileSystem.currentDirectory.childFile('clang')..createSync(recursive: true);
    for (final String file in <String>[kClangBinary, kArBinary, kLdBinary]) {
      fileSystem.currentDirectory.childFile(file).createSync();
    }
    ...
    overrides: <Type, Generator>{
      ...
      ProcessManager:
          () => FakeProcessManager.list(<FakeCommand>[
            FakeCommand(command: const <Pattern>['which', 'clang++'], stdout: clang.path),
          ]),
    },
    () async {
      await io.IOOverrides.runWithIOOverrides(() async {
        ...
        final Uri nonFlutterTesterAssetUri =
            environment.buildDir.childFile('native_assets.json').uri;
