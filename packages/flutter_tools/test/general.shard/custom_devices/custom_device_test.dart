...
void _writeCustomDevicesConfigFile(Directory dir, List<CustomDeviceConfig> configs) {
  ...
  file.writeAsStringSync(jsonEncode(
      <String, dynamic>{'custom-devices': configs.map<dynamic>((CustomDeviceConfig c) => c.toJson()).toList()}));
}
...
void main() {
  testWithoutContext('replacing string interpolation occurrences in custom device commands', () async {
    expect(
        interpolateCommand(<String>['scp', r'${localPath}', r'/tmp/${appName}', 'pi@raspberrypi'],
            <String, String>{'localPath': 'build/flutter_assets', 'appName': 'hello_world'}),
        <String>['scp', 'build/flutter_assets', '/tmp/hello_world', 'pi@raspberrypi']);

    expect(
        interpolateCommand(
            <String>[r'${test1}', r' ${test2}', r'${test3}'], <String, String>{'test1': '_test1', 'test2': '_test2'}),
        <String>['_test1', ' _test2', r'']);

    expect(
        interpolateCommand(
            <String>[r'${test1}', r' ${test2}', r'${test3}'], <String, String>{'test1': '_test1', 'test2': '_test2'},
            additionalReplacementValues: <String, String>{'test2': '_nottest2', 'test3': '_test3'}),
        <String>['_test1', ' _test2', r'_test3']);
  });
  ...
  testUsingContext('CustomDevice defaults', () async {
    final CustomDevice device =
        CustomDevice(config: testConfig, processManager: FakeProcessManager.any(), logger: BufferLogger.test());
  ...
  testWithoutContext('CustomDevices.discoverDevices successfully discovers devices and executes ping command',
      () async {
    final MemoryFileSystem fs = MemoryFileSystem.test();
    final Directory dir = fs.directory('custom_devices_config_dir');
  ...
  testWithoutContext("CustomDevices.discoverDevices doesn't report device when ping command fails", () async {
    ...
    final CustomDevices discovery = CustomDevices(
      ...
      logger: BufferLogger.test(),
      processManager: FakeProcessManager.list(<FakeCommand>[
        FakeCommand(command: testConfig.pingCommand, stdout: testConfigPingSuccessOutput, exitCode: 1),
      ]),
      config: CustomDevicesConfig.test(
        fileSystem: fs,
        directory: dir,
  ...
  testWithoutContext('CustomDevice forwards observatory port correctly when port forwarding is configured', () async {
    ...
    final CustomDeviceAppSession appSession = CustomDeviceAppSession(
      name: 'testname',
      device: CustomDevice(config: testConfig, logger: BufferLogger.test(), processManager: processManager),
    ...
    final LaunchResult launchResult =
        await appSession.start(debuggingOptions: DebuggingOptions.enabled(BuildInfo.debug));
  ...
  testWithoutContext(
      'CustomDeviceAppSession forwards observatory port correctly when port forwarding is not configured', () async {
    final Completer<void> runDebugCompleter = Completer<void>();
    ...
    final CustomDeviceAppSession appSession = CustomDeviceAppSession(
        name: 'testname',
        device:
            CustomDevice(config: testConfigNonForwarding, logger: BufferLogger.test(), processManager: processManager),
        appPackage: PrebuiltLinuxApp(executable: 'testexecutable'),
        logger: BufferLogger.test(),
        processManager: processManager);
  ...
  testUsingContext('custom device end-to-end test', () async {
    ...
    final CustomDevices customDevices = CustomDevices(
        ...
        processManager: processManager,
        logger: BufferLogger.test(),
        config: CustomDevicesConfig.test(fileSystem: fs, directory: configFileDir, logger: BufferLogger.test()));
    ...
    final LaunchResult result = await device.startApp(app,
        debuggingOptions: DebuggingOptions.enabled(BuildInfo.debug), bundleBuilder: FakeBundleBuilder());
  ...
  testWithoutContext('CustomDevice screenshotting', () async {
    ...
    final CustomDevice device =
        CustomDevice(config: testConfig, logger: BufferLogger.test(), processManager: processManager);
  ...
  testWithoutContext('CustomDevice without screenshotting support', () async {
    ...
    final CustomDevice device = CustomDevice(
        config: testConfig.copyWith(explicitScreenshotCommand: true, screenshotCommand: null),
        logger: BufferLogger.test(),
        processManager: processManager);
  ...
  testWithoutContext('CustomDeviceLogReader cancels subscriptions before closing logLines stream', () async {
    ...
    logReader.subscriptions.replaceRange(
      ...
      logReader.subscriptions.map((StreamSubscription<String> e) => MyFakeStreamSubscription<String>(e)),
    );
