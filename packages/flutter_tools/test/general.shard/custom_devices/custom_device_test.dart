...
void main() {
  testWithoutContext(
    ...
    expect(
        interpolateCommand(<String>[r'${test1}', r' ${test2}', r'${test3}'],
            <String, String>{'test1': '_test1', 'test2': '_test2'}),
  ...
  testWithoutContext(
      'CustomDevice forwards observatory port correctly when port forwarding is configured',
      () async {
    ...
    expect(
        launchResult.observatoryUri, Uri.parse('http://127.0.0.1:12345/abcd/'));
  ...
  testWithoutContext(
      'CustomDeviceAppSession forwards observatory port correctly when port forwarding is not configured',
      () async {
    final Completer<void> runDebugCompleter = Completer<void>();
    ...
    expect(launchResult.started, true);
    expect(launchResult.observatoryUri,
  ...
  testWithoutContext('CustomDevice without screenshotting support', () async {
    ...
    final CustomDevice device = CustomDevice(
        config: testConfig.copyWith(
            explicitScreenshotCommand: true, screenshotCommand: null),
        logger: BufferLogger.test(),
        processManager: processManager);
