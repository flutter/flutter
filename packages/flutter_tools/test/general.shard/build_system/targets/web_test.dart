...
''');
  ...
  test(
      ...
      () => testbed.run(() async {
            ...
            await Dart2JSTarget(const JsCompilerConfig(
              csp: true,
              sourceMaps: false,
            )).build(environment);
          }, overrides: <Type, Generator>{
      ...
      for (final bool strip in <bool>[true, false]) {
        for (final List<String> defines in const <List<String>>[
          ...
          for (final String buildMode in const <String>['profile', 'release']) {
            test(
                'Dart2WasmTarget invokes dart2wasm with renderer=$renderer, -O$level, stripping=$strip, defines=$defines, modeMode=$buildMode',
                () => testbed.run(() async {
                      environment.defines[kBuildMode] = buildMode;
                      environment.defines[kDartDefines] =
                          encodeDartDefines(defines);

                      final File depFile =
                          environment.buildDir.childFile('dart2wasm.d');
                      ...
                      await Dart2WasmTarget(WasmCompilerConfig(
  ...
  test(
      ...
      () => testbed.run(() async {
            ...
            await WebServiceWorker(
                    globals.fs, <WebCompilerConfig>[const JsCompilerConfig()])
                .build(environment);
      ...
      () => testbed.run(() async {
            ...
            expect(
                environment.outputDir
                    ...
                    .readAsStringSync(),
                contains(
                    '"assets/index.html": "7fc56270e7a70fa81a5935b72eacbe29"'));
            expect(environment.buildDir.childFile('service_worker.d'), exists);
          }));
