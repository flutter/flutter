...
void main() {
  testWithoutContext('Invokes dart properly', () async {
      ...
      FakeCommand(
        ...
        onRun: (List<String> args) {
          ...
          fileSystem
              ...
              .copySync(parsedArgs['output'] as String);
        },
      ),
    ]);
    ...
    final AssetTransformationFailure? transformationFailure =
    ...
    expect(fileSystem.directory('.tmp_rand0').listSync(), isEmpty,
...
Transformation failed, but I forgot to exit with a non-zero code.''');
  ...
  testWithoutContext(
      'correctly chains transformations when there are multiple of them',
      () async {
    final FileSystem fileSystem = MemoryFileSystem.test();
    final Artifacts artifacts = Artifacts.test();
      ...
      FakeCommand(
        ...
        onRun: (List<String> args) {
          ...
          for (int i = 0; i < inputFileContents.length; i++) {
            outputContents
                .write(inputFileContents.codeUnitAt(i) - 'a'.codeUnits.first);
          }
  ...
  testWithoutContext(
      'prints an error when a transformer in a chain (thats not the first) does not produce an output',
      () async {
