...
void main() {
  ...
  group('Asset transformation', () {
    testWithoutContext(
        'DevFS re-transforms assets with transformers during update', () async {
      final MemoryFileSystem fileSystem = MemoryFileSystem.test();
          ...
          FakeCommand(
            ...
            onRun: (List<String> command) {
              ...
              final File outputFile =
                  fileSystem.file(argParseResults['output']);
    ...
    testWithoutContext('DevFS reports failure when asset transformation fails',
        () async {
      final MemoryFileSystem fileSystem = MemoryFileSystem.test();
      final Artifacts artifacts = Artifacts.test();
      final FakeDevFSWriter devFSWriter = FakeDevFSWriter();
      ...
      expect(report.success, false,
          reason:
              'DevFS update should fail since asset transformation failed.');
      expect(devFSWriter.entries, isNull,
          reason:
              'DevFS should not have written anything since the update failed.');
      expect(
        logger.errorText,
