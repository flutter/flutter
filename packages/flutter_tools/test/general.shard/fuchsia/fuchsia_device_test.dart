...
void main() {
  group('fuchsia device', () {
    ...
    testWithoutContext('stores the requested id and name', () {
      ...
      final FuchsiaDevice device = FuchsiaDevice(deviceId, name: name, platform: null,);
    ...
    testWithoutContext('can parse ffx output for single device', () async {
      ...
      when(fuchsiaSdk.listDevices(disabledFfxDiscovery: '0')).thenAnswer((Invocation invocation) async {
    ...
    testWithoutContext('can parse device-finder output for single device', () async {
      ...
      when(fuchsiaWorkflow.canListDevices).thenReturn(true);
      when(fuchsiaSdk.listDevices(disabledFfxDiscovery: '1')).thenAnswer((Invocation invocation) async {
        return '2001:0db8:85a3:0000:0000:8a2e:0370:7334 paper-pulp-bush-angel';
      });
    ...
    testUsingContext('disposing device disposes the portForwarder', () async {
      ...
      final FuchsiaDevice device = FuchsiaDevice('123', platform: FakePlatform(operatingSystem: 'linux'),);
    ...
    testUsingContext('default capabilities', () async {
      final FuchsiaDevice device = FuchsiaDevice('123', platform: FakePlatform(),);
    ...
    testUsingContext('hostAddress parsing works', () async {
      ...
      final FuchsiaDevice device = FuchsiaDevice('id', platform: FakePlatform(),);
...
[2018-11-09 01:30:12][52580][52983][log] INFO: example_app.cmx(flutter): Did thing this time
  ...
  ''';
      ...
      testUsingContext('can be parsed for an app', () async {
        final FuchsiaDevice device = FuchsiaDevice('id', name: 'tester', platform: FakePlatform(),);
        ...
        FuchsiaArtifacts: () => FuchsiaArtifacts(
            devFinder: devFinder, sshConfig: sshConfig, ffx: ffx),
      });
  ...
  group('screenshot', () {
    ...
    testUsingContext('takeScreenshot throws if screencap failed', () async {
      final FuchsiaDevice device = FuchsiaDevice('0.0.0.0', name: 'tester', platform: FakePlatform(),);
...
class FuchsiaDeviceWithFakeDiscovery extends FuchsiaDevice {
  FuchsiaDeviceWithFakeDiscovery(String id, {String name}) : super(id, name: name, platform: FakePlatform(operatingSystem: 'linux', environment: <String, String>{}),);
