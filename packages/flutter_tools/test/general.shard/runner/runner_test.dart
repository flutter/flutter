  ...
  group('unified_analytics', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          testUsage.events,
          contains(const TestUsageEvent('ga4_and_ga3_status_mismatch', 'opted_out_of_ga4')),
        );
        expect(fakeAnalytics.telemetryEnabled, false);
        expect(testUsage.enabled, true);
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          testUsage.events,
          isNot(contains(const TestUsageEvent('ga4_and_ga3_status_mismatch', 'opted_out_of_ga4'))),
        );
        expect(fakeAnalytics.telemetryEnabled, false);
        expect(testUsage.enabled, false);
        expect(fakeAnalytics.sentEvents, isEmpty);
...
class CrashingUsage implements Usage {
  ...
  }) => _impl.sendEvent(category, parameter, label: label, value: value, parameters: parameters);
