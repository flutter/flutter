...
void main() {
  ...
  group('LocalEngineArtifacts', () {
    ...
    testWithoutContext('Looks up dart.exe on windows platforms', () async {
      ...
      expect(artifacts.getHostArtifact(HostArtifact.engineDartBinary).path,
          contains('.exe'));
    });

    testWithoutContext('Looks up windows UWP artifacts in host engine',
        () async {
      artifacts = LocalEngineArtifacts(
        fileSystem.path.join(
            fileSystem.currentDirectory.path, 'out', 'winuwp_debug_unopt'),
        fileSystem.path.join(
            fileSystem.currentDirectory.path, 'out', 'winuwp_debug_unopt'),
        cache: cache,
        fileSystem: fileSystem,
      ...
      expect(artifacts.getArtifactPath(Artifact.windowsUwpDesktopPath),
