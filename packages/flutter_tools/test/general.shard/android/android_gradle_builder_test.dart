...
void main() {
  group('gradle build', () {
    ...
    testUsingContext('converts unrecognized ProcessException into ToolExit', () async {
      final builder = AndroidGradleBuilder(
        java: FakeJava(),
        logger: logger,
        processManager: processManager,
      ...
      processManager.addCommand(
        const FakeCommand(
          ...
          exitCode: 1,
          exception: ProcessException('', <String>[], 'Unrecognized'),
        ),
      );
      ...
      await expectLater(
        ...
        throwsToolExit(
          message:
              ...
              'Details: ProcessException: Unrecognized',
        ),
      );
      expect(processManager, hasNoRemainingExpectations);
    }, overrides: <Type, Generator>{AndroidStudio: () => FakeAndroidStudio()});
...
''');
    ...
    testUsingContext('build apk handles ProcessException on gradlew startup cleanly', () async {
      final builder = AndroidGradleBuilder(
        java: FakeJava(),
        logger: logger,
        processManager: processManager,
      ...
      processManager.addCommand(
        const FakeCommand(
          ...
          exception: ProcessException('gradlew', <String>[
            'assembleAarRelease',
          ], 'Permission denied'),
        ),
      );

      final File pubspecFile = fileSystem.file('pubspec.yaml');
      ...
      await expectLater(
        ...
        throwsToolExit(
          message:
              'Gradle build failed to start.\n'
              'This can happen if the Gradle wrapper is missing, corrupted, or has incorrect permissions.\n'
              'Details: ProcessException: Permission denied',
        ),
      );
      expect(processManager, hasNoRemainingExpectations);
    }, overrides: <Type, Generator>{AndroidStudio: () => FakeAndroidStudio()});
