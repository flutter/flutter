...
void main() {
  ...
  test('no unauthorized imports of dart:io', () {
    final List<String> whitelistedPaths = <String>[
      ...
      fs.path.join(
          flutterTools, 'lib', 'src', 'build_runner', 'kernel_builder.dart'),
    ];
        ...
        for (String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*dart:io')) &&
            ...
            final String relativePath =
                fs.path.relative(file.path, from: flutterTools);
            fail(
  ...
  test('no unauthorized imports of package:path', () {
    final String whitelistedPath = fs.path.join(
        flutterTools, 'lib', 'src', 'build_runner', 'kernel_builder.dart');
        ...
        for (String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*package:path/path.dart')) &&
            ...
            final String relativePath =
                fs.path.relative(file.path, from: flutterTools);
            fail(
                "$relativePath imports 'package:path/path.dart'; use 'fs.path' instead");
          }
        }
      }
    }
  ...
  test('no unauthorized imports of build_runner', () {
    ...
    bool _isNotWhitelisted(FileSystemEntity entity) =>
        whitelistedPaths.every((String path) => !entity.path.contains(path));
