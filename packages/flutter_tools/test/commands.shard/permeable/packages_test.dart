  ...
  group('packages test/pub', () {
    ...
    testUsingContext('test without bot', () async {
      ...
      processManager.addCommand(
        const FakeCommand(command: <String>['/bin/cache/dart-sdk/bin/dart', '--suppress-analytics', 'pub', 'run', 'test']),
      );
    ...
    testUsingContext('test with bot', () async {
      ...
      globals.fs.file('pubspec.yaml').createSync();
      processManager.addCommand(
        const FakeCommand(command: <String>['/bin/cache/dart-sdk/bin/dart', '--suppress-analytics', 'pub', '--trace', 'run', 'test']),
      );
      await createTestCommandRunner(PackagesCommand()).run(<String>['packages', 'test']);
      ...
      processManager.addCommand(
        FakeCommand(
          command: const <String>[
            '/bin/cache/dart-sdk/bin/dart', '--suppress-analytics', 'pub', 'run', '--foo', 'bar',
          ],
          stdin: stdin,
        ),
      );
    ...
    testUsingContext('token pass arguments through to pub', () async {
      ...
      processManager.addCommand(
        FakeCommand(
          command: const <String>[
            '/bin/cache/dart-sdk/bin/dart', '--suppress-analytics', 'pub', 'token', 'list',
          ],
          stdin: stdin,
        ),
      );
    ...
    testUsingContext('upgrade does not check for pubspec.yaml if -h/--help is passed', () async {
      ...
      processManager.addCommand(
        FakeCommand(
          command: const <String>[
            '/bin/cache/dart-sdk/bin/dart', '--suppress-analytics', 'pub', 'upgrade', '-h',
          ],
