...
final Platform notLinuxPlatform = FakePlatform(operatingSystem: 'macos', environment: <String, String>{
  'FLUTTER_ROOT': _kTestFlutterRoot,
});
...
void main() {
  ...
  testUsingContext('Linux build fails on non-linux platform', () async {
    ...
    expect(createTestCommandRunner(command).run(const <String>['build', 'linux', '--no-pub']),
  ...
  testUsingContext('Linux build does not spew stdout to status logger', () async {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'linux', '--debug', '--no-pub']);
...
''');
  ...
  testUsingContext('Linux verbose build sets VERBOSE_SCRIPT_LOGGING', () async {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'linux', '--debug', '-v', '--no-pub']);
  ...
  testUsingContext('Linux on x64 build --profile passes profile mode to make', () async {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'linux', '--profile', '--no-pub']);
  }, overrides: <Type, Generator>{
    FileSystem: () => fileSystem,
    ProcessManager: () => processManager,
    Platform: () => linuxPlatform,
...
[
  {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'linux', '--no-pub', '--analyze-size']);
