...
void main() {
  ...
  group('doctor', () {
    ...
    testUsingContext('intellij plugins path checking on mac', () async {
      final String pathViaToolbox =
          globals.fs.path.join('test', 'data', 'intellij', 'mac_via_toolbox');
      final String pathNotViaToolbox = globals.fs.path
          .join('test', 'data', 'intellij', 'mac_not_via_toolbox');

      final IntelliJValidatorOnMac validatorViaToolbox =
          IntelliJValidatorOnMac('Test', 'Test', pathViaToolbox);
      expect(validatorViaToolbox.plistFile,
          'test/data/intellij/mac_via_toolbox/Contents/Info.plist');

      final IntelliJValidatorOnMac validatorNotViaToolbox =
          IntelliJValidatorOnMac('Test', 'Test', pathNotViaToolbox);
      expect(validatorNotViaToolbox.plistFile,
  ...
  group('doctor with fake validators', () {
    ...
    testUsingContext('gen_snapshot does not work', () async {
      ...
      for (final String msg
          in userMessages.flutterBinariesDoNotRun.split('\n')) {
    ...
    testUsingContext('version checking does not work', () async {
      memoryFileSystem.file(genSnapshotPath).createSync(recursive: true);
      final VersionCheckError versionCheckError =
          VersionCheckError('version error');
...
class IntelliJValidatorTestTarget extends IntelliJValidator {
  ...
  String get pluginsPath =>
      globals.fs.path.join('test', 'data', 'intellij', 'plugins');
