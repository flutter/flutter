...
''',
  ...
  testUsingContext('ios build fails when there is no ios project', () async {
    ...
    expect(createTestCommandRunner(command).run(const <String>['build', 'ios', '--no-pub']),
  ...
  testUsingContext('ios build fails in debug with code analysis', () async {
    ...
    expect(
        createTestCommandRunner(command).run(const <String>['build', 'ios', '--no-pub', '--debug', '--analyze-size']),
  ...
  testUsingContext('ios build invokes xcode build with device ID', () async {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'ios', '--no-pub', '--device-id', '1234']);
    expect(testLogger.statusText, contains('build/ios/iphoneos/Runner.app'));
  }, overrides: <Type, Generator>{
    FileSystem: () => fileSystem,
    ProcessManager: () => FakeProcessManager.list(<FakeCommand>[
  ...
  testUsingContext('ios simulator build invokes xcode build', () async {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'ios', '--simulator', '--no-pub']);
  }, overrides: <Type, Generator>{
    FileSystem: () => fileSystem,
    ProcessManager: () => FakeProcessManager.list(<FakeCommand>[
          xattrCommand,
  ...
  testUsingContext('Performs code size analysis and sends analytics', () async {
    ...
    await createTestCommandRunner(command).run(const <String>['build', 'ios', '--no-pub', '--analyze-size']);
...
[
  {
    ...
    "s": 2400
          ...
          _setUpRsyncCommand(
              onRun: () => fileSystem.file('build/ios/iphoneos/Runner.app/Frameworks/App.framework/App')
                ..createSync(recursive: true)
