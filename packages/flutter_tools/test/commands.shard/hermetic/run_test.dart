...
void main() {
  ...
  group('run', () {
    ...
    group('run app', () {
      ...
      testUsingContext(
        ...
        final FakeDevice deviceWithoutFlavorSupport =
            FakeDevice(supportsFlavors: false);
        final FakeDevice deviceWithFlavorSupport =
            FakeDevice(supportsFlavors: true);
        testDeviceManager.devices = <Device>[
          deviceWithoutFlavorSupport,
          deviceWithFlavorSupport
        ];

        await createTestCommandRunner(TestRunCommandThatOnlyValidates())
            .run(runCommand);
    ...
    group('--web-header', () {
      ...
      testUsingContext(
        ...
        await expectLater(
          () => command.createDebuggingOptions(true),
          throwsToolExit(
              message: 'Invalid web headers: hurray/headers=flutter'),
        );
      }, overrides: <Type, Generator>{
        FileSystem: () => fileSystem,
        ProcessManager: () => FakeProcessManager.any(),
      ...
      testUsingContext(
        ...
        await expectLater(
            ...
            throwsToolExit(
                message: '--wasm is only supported on the web platform'));
      }, overrides: <Type, Generator>{
        FileSystem: () => fileSystem,
        ProcessManager: () => FakeProcessManager.any(),
        Logger: () => logger,
      ...
      testUsingContext('accepts headers with commas in them', () async {
        ...
        final DebuggingOptions options =
            await command.createDebuggingOptions(true);
        expect(options.webHeaders,
            <String, String>{'hurray': 'flutter,flutter=hurray'});
      }, overrides: <Type, Generator>{
        FileSystem: () => fileSystem,
        ProcessManager: () => FakeProcessManager.any(),
        Logger: () => logger,
  ...
  testUsingContext('usingCISystem can also be set by environment LUCI_CI',
      () async {
    final RunCommand command = RunCommand();
    await expectLater(
        () => createTestCommandRunner(command).run(<String>[
    ...
    ProcessManager: () => FakeProcessManager.any(),
    Platform: () =>
        FakePlatform(environment: <String, String>{'LUCI_CI': 'True'}),
  });
...
class FakeMacDesignedForIpadDevice extends Fake
    implements MacOSDesignedForIPadDevice {
  @override
...
class TestRunCommandForUsageValues extends RunCommand {
  ...
  Future<BuildInfo> getBuildInfo(
      ...
      File? forcedTargetFile}) async {
    return const BuildInfo(BuildMode.debug, null,
