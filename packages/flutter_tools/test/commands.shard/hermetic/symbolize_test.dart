...
void main() {
  ...
  testUsingContext('symbolize exits when --debug-info argument is missing',
      () async {
    ...
    final Future<void> result =
        createTestCommandRunner(command).run(const <String>['symbolize']);
  ...
  testUsingContext('symbolize exits when --debug-info dwarf file is missing',
      () async {
    final SymbolizeCommand command = SymbolizeCommand(
      stdio: stdio,
      fileSystem: fileSystem,
  ...
  testUsingContext('symbolize exits when --debug-info dSYM is missing',
      () async {
    final SymbolizeCommand command = SymbolizeCommand(
      stdio: stdio,
      fileSystem: fileSystem,
  ...
  testUsingContext(
    ...
    expect(fileSystem.file('results/foo.result').readAsBytesSync(),
        <int>[104, 101, 108, 108, 111, 10]); // hello
  }, overrides: <Type, Generator>{
    OutputPreferences: () => OutputPreferences.test(),
  });

  testUsingContext('symbolize throws when DwarfSymbolizationService throws',
      () async {
    final SymbolizeCommand command = SymbolizeCommand(
      stdio: stdio,
      fileSystem: fileSystem,
