...
class FakeTerminal implements Terminal {
  ...
  FakeTerminal._private({this.stdio, Platform platform}) : terminal = AnsiTerminal(stdio: stdio, platform: platform);
  ...
  Future<String> promptForCharInput(List<String> acceptedCharacters,
          {Logger logger, String prompt, int defaultChoiceIndex, bool displayAcceptedCharacters = true}) =>
  ...
  return CustomDevicesCommand.test(
    customDevicesConfig: config != null
        ...
        : CustomDevicesConfig.test(
            platform: platform, fileSystem: fileSystem, directory: fileSystem.directory('/'), logger: logger),
...
CommandRunner<void> createCustomDevicesCommandRunner({
  ...
  return FakeCommandRunner(platform: platform, fileSystem: fileSystem, logger: logger)
    ..addCommand(createCustomDevicesCommand(
        config: config,
        terminal: terminal,
        platform: platform,
        fileSystem: fileSystem,
...
void main() {
  ...
  group('linux', () {
    ...
    testUsingContext('custom-devices command shows config file in help when feature is enabled', () async {
      final BufferLogger logger = BufferLogger.test();

      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          logger: logger, usagePrintFn: (Object o) => logger.printStatus(o.toString()), featureEnabled: true);
      await expectLater(runner.run(const <String>['custom-devices', '--help']), completes);
      expect(logger.statusText, contains('Makes changes to the config file at "/.flutter_custom_devices.json".'));
    });
    ...
    // test add command
    testUsingContext('custom-devices add command correctly adds ssh device config on linux', () async {
      ...
      await expectLater(runner.run(const <String>['custom-devices', 'add', '--no-check']), completes);

      final CustomDevicesConfig config =
          CustomDevicesConfig.test(fileSystem: fs, directory: fs.directory('/'), logger: BufferLogger.test());

      expect(
          config.devices,
          contains(CustomDeviceConfig(
            id: 'testid',
            label: 'testlabel',
            sdkNameAndVersion: 'testsdknameandversion',
            enabled: true,
            ...
            runDebugCommand: const <String>['ssh', '-o', 'BatchMode=yes', 'testuser@testhostname', 'testrundebug'],
            ...
            screenshotCommand: const <String>['ssh', '-o', 'BatchMode=yes', 'testuser@testhostname', 'testscreenshot'],
          )));
    });
    ...
    testUsingContext('custom-devices add command correctly adds ipv4 ssh device config', () async {
      ...
      expect(
          ...
          contains(CustomDeviceConfig(
              ...
              runDebugCommand: const <String>['ssh', '-o', 'BatchMode=yes', 'testuser@192.168.178.1', 'testrundebug'],
              forwardPortCommand: const <String>[
                'ssh',
                '-o',
                'BatchMode=yes',
    ...
    testUsingContext('custom-devices add command correctly adds ipv6 ssh device config', () async {
      ...
      expect(
          ...
          contains(CustomDeviceConfig(
              ...
              runDebugCommand: const <String>['ssh', '-o', 'BatchMode=yes', '-6', 'testuser@[::1]', 'testrundebug'],
              forwardPortCommand: const <String>[
                'ssh',
                '-o',
                'BatchMode=yes',
    ...
    testUsingContext('custom-devices add command correctly adds non-forwarding ssh device config', () async {
      final MemoryFileSystem fs = MemoryFileSystem.test();
      ...
      expect(
          config.devices,
          contains(const CustomDeviceConfig(
              id: 'testid',
              label: 'testlabel',
              sdkNameAndVersion: 'testsdknameandversion',
              enabled: true,
              ...
              runDebugCommand: <String>['ssh', '-o', 'BatchMode=yes', 'testuser@testhostname', 'testrundebug'],
              screenshotCommand: <String>['ssh', '-o', 'BatchMode=yes', 'testuser@testhostname', 'testscreenshot'])));
    });

    testUsingContext('custom-devices add command correctly adds non-screenshotting ssh device config', () async {
      final MemoryFileSystem fs = MemoryFileSystem.test();
    ...
    testUsingContext('custom-devices delete command deletes device and creates backup', () async {
      ...
      final CommandRunner<void> runner =
          createCustomDevicesCommandRunner(config: (_, __) => config, fileSystem: fs, featureEnabled: true);

      final Uint8List contentsBefore = fs.file('.flutter_custom_devices.json').readAsBytesSync();

      await expectLater(runner.run(const <String>['custom-devices', 'delete', '-d', 'testid']), completes);
    ...
    testUsingContext('custom-devices delete command without device argument throws tool exit', () async {
      final MemoryFileSystem fs = MemoryFileSystem.test();
    ...
    testUsingContext('custom-devices delete command throws tool exit with invalid device id', () async {
      ...
      await expectLater(
          runner.run(const <String>['custom-devices', 'delete', '-d', 'testid']),
          throwsToolExit(
              message: 'Couldn\'t find device with id "testid" in config at "/.flutter_custom_devices.json"'));
    });

    testUsingContext('custom-devices list command throws tool exit when config contains errors', () async {
      final MemoryFileSystem fs = MemoryFileSystem.test();
      final BufferLogger logger = BufferLogger.test();
      ...
      final CommandRunner<void> runner =
          createCustomDevicesCommandRunner(fileSystem: fs, logger: logger, featureEnabled: true);

      await expectLater(runner.run(const <String>['custom-devices', 'list']),
    ...
    testUsingContext('custom-devices list command prints message when no devices found', () async {
      ...
      final CommandRunner<void> runner = createCustomDevicesCommandRunner(logger: logger, featureEnabled: true);

      await expectLater(runner.run(const <String>['custom-devices', 'list']), completes);
      expect(logger.statusText, contains('No custom devices found in "/.flutter_custom_devices.json"'));
    });
    ...
    testUsingContext('custom-devices list command lists all devices', () async {
      ...
      CustomDevicesConfig.test(
        ...
        ..add(CustomDeviceConfig.exampleUnix.copyWith(id: 'testid', label: 'testlabel', enabled: true))
        ..add(CustomDeviceConfig.exampleUnix.copyWith(id: 'testid2', label: 'testlabel2', enabled: false));

      final CommandRunner<void> runner =
          createCustomDevicesCommandRunner(logger: logger, fileSystem: fs, featureEnabled: true);

      await expectLater(runner.run(const <String>['custom-devices', 'list']), completes);
      expect(logger.statusText, contains('List of custom devices in "/.flutter_custom_devices.json":'));
    ...
    testUsingContext('custom-devices reset correctly backs up the config file', () async {
      ...
      expect(
          logger.statusText,
          contains('Successfully resetted the custom devices config file and created a '
              'backup at "/.flutter_custom_devices.json.bak".'));

      final Uint8List backupContents = fs.file('.flutter_custom_devices.json.bak').readAsBytesSync();
      expect(contentsBefore, equals(backupContents));
      expect(fs.file('.flutter_custom_devices.json').readAsStringSync(),
          anyOf(equals(defaultConfigLinux1), equals(defaultConfigLinux2)));
    });

    testUsingContext("custom-devices reset outputs correct msg when config file didn't exist", () async {
      final MemoryFileSystem fs = MemoryFileSystem.test();
      final BufferLogger logger = BufferLogger.test();
      ...
      final CommandRunner<void> runner =
          createCustomDevicesCommandRunner(logger: logger, fileSystem: fs, featureEnabled: true);
      await expectLater(runner.run(const <String>['custom-devices', 'reset']), completes);
      expect(logger.statusText, contains('Successfully resetted the custom devices config file.'));
