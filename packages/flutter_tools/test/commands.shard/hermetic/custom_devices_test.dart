...
class FakeTerminal implements Terminal {
  ...
  FakeTerminal._private({this.stdio, Platform platform})
...
class FakeCommandRunner extends FlutterCommandRunner {
  ...
  Future<void> runCommand(ArgResults topLevelResults) async {
    final Logger logger =
        (topLevelResults['verbose'] as bool) ? VerboseLogger(_logger) : _logger;
    ...
    return context.run<void>(
        ...
        body: () {
          ...
          Cache.flutterRoot = _fileSystem.path
              .normalize(_fileSystem.path.absolute(Cache.flutterRoot));
...
CommandRunner<void> createCustomDevicesCommandRunner({
  ...
  return FakeCommandRunner(
      platform: platform, fileSystem: fileSystem, logger: logger)
  ...
  group('linux', () {
    ...
    testUsingContext(
      ...
      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          ...
          featureEnabled: true);
      await expectLater(
          runner.run(const <String>['custom-devices', '--help']), completes);
      expect(
          logger.statusText,
          contains(
              'Makes changes to the config file at "/.flutter_custom_devices.json".'));
    });
    ...
    testUsingContext(
      ...
      await expectLater(
          runner.run(const <String>['custom-devices']), completes);
      expect(
          logger.statusText,
          contains(
              'Makes changes to the config file at "/.flutter_custom_devices.json".'));
    ...
    testUsingContext(
      ...
      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          config: (_, __) => config, fileSystem: fs, featureEnabled: true);

      final Uint8List contentsBefore =
          fs.file('.flutter_custom_devices.json').readAsBytesSync();

      await expectLater(
          runner
              .run(const <String>['custom-devices', 'delete', '-d', 'testid']),
          completes);
    ...
    testUsingContext(
      ...
      final CommandRunner<void> runner =
          createCustomDevicesCommandRunner(featureEnabled: true);
      await expectLater(runner.run(const <String>['custom-devices', 'delete']),
          throwsToolExit());

      final Uint8List contentsAfter =
          fs.file('.flutter_custom_devices.json').readAsBytesSync();
    ...
    testUsingContext(
      ...
      fs
          .file('.flutter_custom_devices.json')
          .writeAsStringSync('{"custom-devices": {}}');

      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          fileSystem: fs, logger: logger, featureEnabled: true);
    ...
    testUsingContext(
      ...
      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          logger: logger, featureEnabled: true);

      await expectLater(
          runner.run(const <String>['custom-devices', 'list']), completes);
      expect(
          logger.statusText,
          contains(
              'No custom devices found in "/.flutter_custom_devices.json"'));
    });
    ...
    testUsingContext('custom-devices list command lists all devices', () async {
      ...
      CustomDevicesConfig.test(
        ...
        ..add(CustomDeviceConfig.exampleUnix
            .copyWith(id: 'testid', label: 'testlabel', enabled: true))
        ..add(CustomDeviceConfig.exampleUnix
            .copyWith(id: 'testid2', label: 'testlabel2', enabled: false));

      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          logger: logger, fileSystem: fs, featureEnabled: true);
      ...
      expect(
          logger.statusText,
          contains(
              'List of custom devices in "/.flutter_custom_devices.json":'));
    ...
    testUsingContext('custom-devices reset correctly backs up the config file',
        () async {
      final MemoryFileSystem fs = MemoryFileSystem.test();
      final BufferLogger logger = BufferLogger.test();
      ...
      final CommandRunner<void> runner = createCustomDevicesCommandRunner(
          logger: logger, fileSystem: fs, featureEnabled: true);
      await expectLater(
          runner.run(const <String>['custom-devices', 'reset']), completes);
      expect(
          logger.statusText,
          contains(
    ...
    testUsingContext(
      ...
      await expectLater(
          runner.run(const <String>['custom-devices', 'reset']), completes);
      expect(logger.statusText,
          contains('Successfully resetted the custom devices config file.'));
