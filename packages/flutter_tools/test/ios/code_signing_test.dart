...
1) 86f7e437faa5a7fce15d1ddcb9eaeaea377667b8 "iPhone Developer: Profile 1 (1111AAAA11)"
    1 valid identities found''',
      ...
      )).thenAnswer((invocation) => new Future<Process>.value(mockProcess));

      when(mockProcess.stdin).thenReturn(mockStdIn);
      when(mockProcess.stdout).thenAnswer((invocation) => new Stream<List<int>>.fromFuture(
        new Future<List<int>>.value(UTF8.encode(
          'subject= /CN=iPhone Developer: Profile 1 (1111AAAA11)/OU=3333CCCC33/O=My Team/C=US'
        ))
      ));
      when(mockProcess.stderr).thenAnswer((invocation) => mockStdErr);
...
3) 5bf1fd927dfb8679496a2e6cf00cbe50c1c87145 "iPhone Developer: Profile 3 (3333CCCC33)"
    3 valid identities found''',
      ...
      when(mockProcessManager.start(
        argThat(contains('openssl')), environment: any, workingDirectory: any,
      )).thenAnswer((invocation) => new Future<Process>.value(mockOpenSslProcess));

      when(mockOpenSslProcess.stdin).thenReturn(mockOpenSslStdIn);
      when(mockOpenSslProcess.stdout).thenAnswer((invocation) => new Stream<List<int>>.fromFuture(
        new Future<List<int>>.value(UTF8.encode(
          'subject= /CN=iPhone Developer: Profile 3 (3333CCCC33)/OU=4444DDDD44/O=My Team/C=US'
        ))
      ));
      when(mockOpenSslProcess.stderr).thenAnswer((invocation) => mockOpenSslStdErr);
