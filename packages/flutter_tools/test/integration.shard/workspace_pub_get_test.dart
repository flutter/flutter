...
void main() {
  ...
  testWithoutContext(
    'flutter tools correctly skip pub get when resolution is up-to-date in a workspace',
    () async {
...
''');
        ...
        expect(
          getResult.exitCode,
          0,
          reason: 'Initial pub get failed:\n${getResult.stderr}\n${getResult.stdout}',
        );

        // Validate workspace output structure
        expect(
          workspaceDir.childDirectory('.dart_tool').childFile('package_config.json').existsSync(),
          true,
        );
        ...
        expect(
          analyzeResult1.stdout.toString(),
          contains('Skipping pub get: resolution up-to-date.'),
        );
...
''');
        ...
        expect(
          analyzeResult2.stdout.toString(),
          isNot(contains('Skipping pub get: resolution up-to-date.')),
        );
