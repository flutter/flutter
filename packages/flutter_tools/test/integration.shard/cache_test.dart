...
class FakeLogger extends Logger {
  ...
  Status startProgress(String message,
      {String progressId, int progressIndicatorPadding = kDefaultStatusPadding}) {
    return SilentStatus(
...
final String dart =
    fileSystem.path.join(getFlutterRoot(), 'bin', platform.isWindows ? 'dart.bat' : 'dart');
    ...
    testWithoutContext('should log a message to stderr when lock is not acquired', () async {
      ...
      try {
        ...
        final File cacheFile = fileSystem
            .file(fileSystem.path.join(Cache.flutterRoot, 'bin', 'cache', 'lockfile'))
          ..createSync(recursive: true);
        final File script = fileSystem
            .file(fileSystem.path.join(Cache.flutterRoot, 'bin', 'cache', 'test_lock.dart'));
...
''');
        final Process process = await const LocalProcessManager()
            .start(<String>[dart, script.absolute.path, cacheFile.absolute.path]);
