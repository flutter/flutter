...
''');
      ...
      expect(logger.warningText, equals('Waiting for another flutter command to release the startup lock...\n'));
  ...
  testWithoutContext('Dart SDK target arch matches host arch', () async {
    ...
    final String unameArch =
        (unameResult.stdout as String).trim().replaceAll('aarch64', 'arm64').replaceAll('x86_64', 'x64');
...
class FakeVersionlessArtifact extends CachedArtifact {
  ...
  Future<void> updateInner(
      ArtifactUpdater artifactUpdater, FileSystem fileSystem, OperatingSystemUtils operatingSystemUtils) async {}
}
