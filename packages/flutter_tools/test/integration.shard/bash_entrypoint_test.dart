...
Future<void> main() async {
  test(
    ...
    final Future<Object?> stdoutFuture =
        process.stdout.transform<String>(utf8.decoder).forEach((String str) {
      stdout += str;
    ...
    expect(stdout, contains('Successfully received SIGTERM!'));
      ...
      skip: platform
          .isWindows); // [intended] Windows does not use the bash entrypoint

  test('shared.sh does not compile flutter tool if PROG_NAME=dart', () async {
    final Directory tempDir =
        fileSystem.systemTempDirectory.createTempSync('bash_entrypoint_test');
    try {
      // bash script checks it is in a git repo
      ProcessResult result = await processManager
          .run(<String>['git', 'init'], workingDirectory: tempDir.path);
      expect(result, const ProcessResultMatcher());
      result = await processManager.run(
          <String>['git', 'commit', '--allow-empty', '-m', 'init commit'],
      ...
      final File fakeSharedSh =
          (tempDir.childDirectory('bin').childDirectory('internal')
...
void makeExecutable(File file) {
  final ProcessResult result =
      processManager.runSync(<String>['chmod', '+x', file.path]);
  expect(result, const ProcessResultMatcher());
}
