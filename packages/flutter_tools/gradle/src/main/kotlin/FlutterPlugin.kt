...
class FlutterPlugin : Plugin<Project> {
    ...
    companion object {
        ...
        private fun addFlutterDeps(
            ...
            // The file is not created by the Flutter toolchain when dart-defines are empty,
            // so a missing file is expected and results in null (falling back to -Pdart-defines).
                ...
                project.findProperty("dart-defines-file")?.toString()?.let { filePath ->
                    ...
                    if (file.exists()) {
                        ...
                        if (content.isNullOrEmpty()) {
                            throw GradleException(
                                "dart-defines-file '$filePath' exists but is empty. " +
                                    "This file is expected to be generated by the Flutter tool with content."
                            )
                        }
                        content
                    } else {
