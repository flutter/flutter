...
//
// Error handling / session disconnection is still WIP.  FakeSession will likely
// generate a CHECK in any place where the real scenic would disconnect the
// session or send a ScenicError.
//
...
//
// Cross-session links are not handled.
class FakeSession : public fuchsia::ui::scenic::testing::Session_TestBase {
 public:
  ...
  // Fire an `OnFramePresented` event.
 ...
 private:
  ...
  std::unordered_map<FakeResourceState*,
      ...
      parents_map_;  // Used to cache parent refs for `AddChildCmd`
