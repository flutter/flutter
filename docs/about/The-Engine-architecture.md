Flutter combines [a Dart framework](https://github.com/flutter/flutter) with a
high-performance [engine](https://github.com/flutter/flutter/tree/main/engine).
...
and
[iOS](https://github.com/flutter/flutter/tree/main/engine/src/flutter/shell/platform/darwin).
We also have an
[embedder API](https://github.com/flutter/flutter/tree/main/engine/src/flutter/shell/platform/embedder)
which allows Flutter's engine to be used as a library (see
[Custom Flutter Engine Embedders](../engine/Custom-Flutter-Engine-Embedders.md)).
...
shell. The shells can also communicate directly to Dart code via
[Platform Channels](https://flutter.io/platform-channels/) which bypass the
engine.
...
thread. For example, this is typically the
[Android Main Thread](https://developer.android.com/guide/components/processes-and-threads.html)
or the
[Main Thread](https://developer.apple.com/documentation/foundation/nsthread/1412704-ismainthread?language=objc)
...
- On vsync, the engine will wake up the Dart code and
  [perform the following](https://api.flutter.dev/flutter/widgets/WidgetsBinding/drawFrame.html):
...
this code be moved into a separate
[Dart isolate](https://api.flutter.dev/flutter/dart-isolate/dart-isolate-library.html)
(e.g. using the
...
If an engine is instantiated directly, it gets its own engine threads and
[dart isolate group](https://dart.dev/language/concurrency#performance-and-isolate-groups).
