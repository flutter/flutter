...
The main reason for this change is that the `flutter/engine` and `flutter/flutter` repos were combined into one "monorepo". In order to preserve the lightweight dev cycle for the framework code, we need to provide pre-built engine artifacts. This way you don't need to compile any C++ code, or install extra tooling (e.g. `depot_tools`), when hacking on the framework alone. The merge queue is what builds those engine binaries.
