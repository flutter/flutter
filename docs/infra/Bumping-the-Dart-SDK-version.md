...
It is **critical** to include the `-0` pre-release suffix (e.g., `^3.13.0-0` instead of `^3.13.0`). 
* **Why:** According to Dart package resolution rules, standard caret constraints (like `^3.13.0`) exclude pre-release versions of that major/minor release (e.g. `3.13.0-dev` or `3.13.0-5.0.pre`).
...
Follow this by running the test suites on your target platforms to ensure no runtime regressions occur:
...
> [!WARNING]
> If your PR breaks Google internal (Google3) integration due to unresolved Dart SDK version mismatches, it may be reverted. Coordinate with the current EngProd sheriff or Dart/Flutter roll managers to ensure alignment.
