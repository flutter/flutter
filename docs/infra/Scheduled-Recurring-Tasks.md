...
| **Dart SDK Constraint Bump** | [quarterly-scheduled-tasks.yml](../../.github/workflows/quarterly-scheduled-tasks.yml) | Prompts team to bump the minimum Dart SDK version constraint across `flutter/flutter` to align with the current stable release, update package checksums, run static analysis and tests, and coordinate with the release team on branch alignment if a large scale change (LSC) is required. | `automated task`, `team-framework` |
