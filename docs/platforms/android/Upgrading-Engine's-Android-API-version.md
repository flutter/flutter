...
2. In each of the engine builders (at the time of writing: `standalone/linux_android_emulator_skia.json` & `standalone/linux_android_emulator.json`), find the `android_virtual_device` and `avd_cipd_version` dependency entries and update them to the versions you desire, e.g.
