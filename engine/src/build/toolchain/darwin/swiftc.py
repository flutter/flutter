...
def invoke_swift_compiler(args, extras_args, build_cache_dir, output_file_map):
    ...
    if args.emit_clang_header_min_access:
        swiftc_args.extend(
            ("-emit-clang-header-min-access", args.emit_clang_header_min_access)
        )
    ...
    testing_plugin_path = os.path.join(
        args.mac_host_toolchain_path, "usr/lib/swift/host/plugins/testing"
    )
...
def main(args):
    ...
    # Required arguments.
    parser.add_argument("-module-name", required=True, help="name of the Swift module")

    parser.add_argument("--src-dir", required=True, help="path to the source directory")
    ...
    parser.add_argument(
        "--depfile-path", required=True, help="path to the output dependency file"
    )
    ...
    # Optional arguments.
    parser.add_argument("--derived-data-dir", help="path to the derived data directory")
    ...
    parser.add_argument(
        "--swift-toolchain-path", default="", help="path to the Swift toolchain to use"
    )

    parser.add_argument(
        "--mac-host-toolchain-path",
        default="",
    ...
    parser.add_argument(
        "-swift-version", default="5", help="version of the Swift language"
    )

    parser.add_argument(
        "-file-prefix-map", help="remap source paths in debug, coverage, and index info"
    )
    ...
    parser.add_argument("sources", nargs="+", help="Swift source files to compile")
