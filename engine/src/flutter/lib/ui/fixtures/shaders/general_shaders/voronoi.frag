...
{
    return coordinate + vec2((rand(coordinate * 400.0) - 0.5) * 0.800000011920928955078125, (rand(coordinate * 400.0) - 0.5) * 0.800000011920928955078125);
}

vec3 getColorForGridPoint(vec2 coordinate)
{
    return vec3(rand(coordinate * 100.0), rand(coordinate * 200.0), rand(coordinate * 300.0));
}
