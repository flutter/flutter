...
<sem>
  ...
  test('preserves host element when changing between generic and scrollable roles', () {
    semantics()
      ..debugOverrideTimestampFunction(() => _testTime)
      ..semanticsEnabled = true;
    ...
    final SemanticsTextEditingStrategy strategy = SemanticsTextEditingStrategy.instance;
    testTextEditing.debugTextEditingStrategyOverride = strategy;
    testTextEditing.configuration = InputConfiguration(viewId: kImplicitViewId);
    EditingState? lastEditingState;
    strategy.enable(
      InputConfiguration(viewId: kImplicitViewId),
      onChange: (EditingState? editingState, TextEditingDeltaState? _) {
    ...
    void pumpSemantics({required bool isScrollable}) {
      tester.updateNode(
        id: 0,
        children: <SemanticsNodeUpdate>[
          tester.updateNode(
            id: 1,
            flags: isScrollable
                ? const ui.SemanticsFlags(hasImplicitScrolling: true)
                : ui.SemanticsFlags.none,
            hasScrollUp: isScrollable,
    ...
    pumpSemantics(isScrollable: false);
    final DomElement hostElement = owner().debugSemanticsTree![1]!.element;
    final textField = owner().debugSemanticsTree![2]!.semanticRole! as SemanticTextField;
    final editableElement = textField.editableElement as DomHTMLInputElement;
    editableElement.focusWithoutScroll();
    expect(domDocument.activeElement, editableElement, reason: 'Initial input focus failed.');
    expect(hostElement.querySelector('flt-semantics-scroll-overflow'), isNull);

    pumpSemantics(isScrollable: true);
    expect(owner().debugSemanticsTree![1]!.element, same(hostElement));
    expect(hostElement.contains(editableElement), isTrue);
    expect(hostElement.getAttribute('role'), 'group');
    expect(hostElement.style.overflowY, 'scroll');
    expect(hostElement.style.scrollbarWidth, 'none');
    expect(hostElement.querySelector('flt-semantics-scroll-overflow'), isNotNull);
    expect(
      domDocument.activeElement,
      editableElement,
      reason: 'Changing from generic to scrollable should not blur focused descendants.',
    );
    ...
    expect(hostElement.contains(editableElement), isTrue);
    expect(hostElement.querySelector('flt-semantics-scroll-overflow'), isNull);
    expect(hostElement.style.overflowY, 'visible');
    expect(hostElement.style.overflowX, 'visible');
    expect(hostElement.style.scrollbarWidth, isEmpty);
    expect(hostElement.style.touchAction, isEmpty);
    expect(hostElement.getAttribute('role'), 'group');
    expect(
      domDocument.activeElement,
      editableElement,
      reason: 'Changing from scrollable to generic should not blur focused descendants.',
    );
    editableElement.value = 'typed';
    editableElement.setSelectionRange(5, 5);
    editableElement.dispatchEvent(createDomEvent('Event', 'input'));
    expect(lastEditingState, EditingState(text: 'typed', baseOffset: 5, extentOffset: 5));
