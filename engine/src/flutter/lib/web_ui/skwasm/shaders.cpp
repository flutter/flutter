...
SKWASM_EXPORT sp_wrapper<DlColorSource>* shader_createRuntimeEffectShader(
  ...
  // TODO(jacksongardner): Don't copy this every time
  auto start = reinterpret_cast<const uint8_t*>(uniforms->data());
  auto end = start + uniforms->size();
  auto uniformData = std::make_shared<std::vector<uint8_t>>(start, end);
  return new sp_wrapper<DlColorSource>(DlColorSource::MakeRuntimeEffect(
      DlRuntimeEffectSkia::Make(sk_ref_sp(runtimeEffect)),
      std::move(childPointers), std::move(uniformData)));
}
