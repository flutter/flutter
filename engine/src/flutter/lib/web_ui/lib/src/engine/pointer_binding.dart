...
///
/// Physical mouse input is exempt from all of the above and is forwarded to
/// the framework directly, because a synthesized `SemanticsAction.tap` loses
/// the pointer position and device kind that a mouse gesture carries. See
/// [_maybeStartDebouncing].
class ClickDebouncer {
  ...
  /// debouncing events, unless the event came from a physical mouse, which is
  /// never debounced.
  ///
  ...
  /// tappable element and did not come from a physical mouse.
  ///
  ...
  void _maybeStartDebouncing(DomEvent event, List<ui.PointerData> data) {
    ...
    // Physical mouse input is never debounced.
    //
    // The debouncer resolves the pointer-events-vs-"click" ambiguity by
    // dropping the pointer events and synthesizing a generic
    // `SemanticsAction.tap` in their place. That trade is right for an
    // assistive technology, which only wants to activate the node, but it is
    // wrong for a real mouse: the synthesized tap carries no position and no
    // device kind, so a `GestureDetector` under the cursor reports
    // `Tap*Details` pointing at the center of the screen with
    // `PointerDeviceKind.unknown`.
    //
    // The trade-off this makes: the forwarded `pointerup` sets
    // `_lastSentPointerUpTimeStamp`, so the "click" that follows is dropped by
    // [_shouldSendClickEventToFramework]. Mouse input therefore no longer
    // reaches `SemanticsAction.tap` by any route. A semantics node that
    // exposes a tap action but has no hit-testable gesture recognizer under
    // the cursor will not respond to a physical mouse click. That is
    // deliberate: coordinate-accurate taps are worth more than the
    // recognizer-less case, which the framework can express with a real
    // gesture detector.
    //
    // Only `mouse` is exempted, not `pen` or `touch`. Those are also physical
    // input and lose the same detail, but they are the input types the
    // debounce window was originally tuned against, and touch is the case the
    // debouncer was written for. Widening the exemption is a larger behavior
    // change than this fix warrants; see the discussion on the issue below.
    //
    // https://github.com/flutter/flutter/issues/188859
    if ((event as DomPointerEvent).pointerType == 'mouse') {
      _sendToFramework(event, data);
      return;
    }
  ...
  void _doStartDebouncing() {
    ...
    _state = (
      ...
      //     screen readers.
      //
      // Note: physical mouse input no longer reaches this timer at all; it is
      // forwarded to the framework from [_maybeStartDebouncing]. The mouse and
      // trackpad cases above are retained as a record of how the window was
      // originally tuned.
