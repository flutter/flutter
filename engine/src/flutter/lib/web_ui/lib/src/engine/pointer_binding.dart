...
///   gesture. Clicks generated by a mouse are an exception: the queued pointer
///   events are flushed and the click is dropped, because pointer events
///   describe the interaction more precisely than a `SemanticsAction.tap` can.
///
...
class ClickDebouncer {
  ...
  /// Whether the pointer events withheld from the framework were generated by
  /// a mouse.
  bool get _isDebouncingMouseEvents {
    final DebounceState? state = _state;
    if (state == null) {
      return false;
    }
    // The queue is seeded with the `pointerdown` that started the debouncing
    // process and is only ever appended to, so it is never empty.
    final List<ui.PointerData> data = state.queue.first.data;
    return data.isNotEmpty && data.first.kind == ui.PointerDeviceKind.mouse;
  }
  ...
  void onClick(DomEvent click, int viewId, int semanticsNodeId, bool isListening) {
    ...
    if (isListening) {
      if (_isDebouncingMouseEvents) {
        // The queued pointer events carry the exact position and device kind of
        // the interaction, while `SemanticsAction.tap` carries neither, forcing
        // the framework to synthesize a tap at the center of the semantics
        // node. Flush the pointer events so that mouse gestures are recognized
        // exactly like they are when semantics is off.
        //
        // Assistive technologies drive taps either through clicks that come
        // with no pointer events at all, or through touch, so they keep using
        // the semantics action below.
        //
        // See https://github.com/flutter/flutter/issues/188859
