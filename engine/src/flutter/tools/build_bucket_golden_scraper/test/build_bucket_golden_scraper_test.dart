...
void main() {
  test('parses command-line arguments', () {
    ...
    final srcDir = io.Directory(p.join(buildRoot.path, 'src'))..createSync();
    io.Directory(p.join(srcDir.path, 'flutter')).createSync();

    // Does not run, but just parses.
    try {
      final scraper = BuildBucketGoldenScraper.fromCommandLine(<String>[
        '--dry-run',
        '--engine-src-path',
        srcDir.path,
  ...
  test('finds diffs', () async {
    ...
    try {
      final logFile = io.File(p.join(buildRoot.path, 'log.txt'))..createSync();
...
''');
      ...
      final scraper = BuildBucketGoldenScraper(
