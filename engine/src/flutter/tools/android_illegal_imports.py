...
def main():
    ...
    for file in args.files:
        if (
            file.endswith(os.path.join("io", "flutter", "Log.java"))
            or file.endswith(os.path.join("io", "flutter", "util", "TraceSection.java"))
            or file.endswith(os.path.join("io", "flutter", "Build.java"))
        ):
            continue
        with open(file) as f:
            contents = f.read()
    ...
    # is used.
    has_bad_files = (
        CheckBadFiles(bad_log_files, ANDROID_LOG_CLASS, FLUTTER_LOG_CLASS)
        or CheckBadFiles(bad_trace_files, "android[x].tracing.Trace", FLUTTER_TRACE_CLASS)
        or CheckBadFiles(
            bad_version_codes_files, "android.os.Build.VERSION_CODES", "io.flutter.Build.API_LEVELS"
        )
    )
