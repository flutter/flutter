...
def GetDartSdkGitRevision(buildroot):
    return subprocess.check_output(
        ["git", "-C", GetDartPath(buildroot), "rev-parse", "HEAD"]
    ).strip()
...
def GetDartSdkSemanticVersion(buildroot):
    ...
    return subprocess.check_output(
        ["git", "-C", GetDartPath(buildroot), "describe", "--abbrev=0"]
    ).strip()
...
def GetFuchsiaSdkVersion(buildroot):
    with open(
        path.join(buildroot, "third_party", "fuchsia-sdk", "sdk", "meta", "manifest.json"), "r"
