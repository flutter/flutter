...
def CopyVulkanDepsToBucket(src, dst, arch):
    ...
    if not os.path.exists(deps_bucket_path):
        FindFileAndCopyTo(
            "VkLayer_khronos_validation.json", "%s/pkg" % (sdk_path), deps_bucket_path
        )
        FindFileAndCopyTo(
...
def main():
    ...
    parser.add_argument(
        "--engine-version", required=False, help="Specifies the flutter engine SHA."
    )
    ...
    parser.add_argument(
        "--targets",
        default="",
        help=("Comma-separated list; adds additional targets to build for " "Fuchsia."),
    )
