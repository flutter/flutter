...
int IOSSurfacesManager::SurfaceCount() const {
  return frame_layers_.size();
}

void IOSSurfacesManager::CreateSurfaceFrameLayer(int64_t view_id) {
  auto *delegate = static_cast<IOSSurfaceMetalImpeller *>(GetSurface(view_id));

  frame_layers_[view_id] = std::make_unique<GPUSurfaceMetalSurfaceFrameLayer>(
                  delegate,
                  aiks_context_);
}

void IOSSurfacesManager::DestroySurfaceFrameLayer(int64_t view_id) {
  frame_layers_.erase(view_id);
}

SurfaceFrameLayer *IOSSurfacesManager::GetSurfaceFrameLayer(int64_t view_id) {
...
std::shared_ptr<ExternalViewEmbedder> PlatformViewIOS::CreateExternalViewEmbedder() {
  ...
  return std::make_shared<IOSExternalViewEmbedder>(
            ...
            ios_context_,
            [surfaces_manager_ptr, ios_context = ios_context_](int64_t view_id) {               // get_gpu_surface_delegate
                // auto *surface = surfaces_manager_ptr->GetSurface(view_id);
                // auto *delegate = static_cast<IOSSurfaceMetalImpeller *>(
                //     surfaces_manager_ptr->GetSurface(view_id));

                // return std::make_unique<GPUSurfaceMetalSurfaceFrameLayer>(
                //           delegate,
                //           ios_context->GetAiksContext());
