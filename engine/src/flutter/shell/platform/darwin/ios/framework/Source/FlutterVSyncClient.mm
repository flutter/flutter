...
- (instancetype)initWithTaskRunner:(FlutterFMLTaskRunner*)taskRunner
  ...
  auto cpp_callback = [runner, callback](std::unique_ptr<flutter::FrameTimingsRecorder> recorder) {
    double target_time_seconds = recorder->GetVsyncTargetTime().ToEpochDelta().ToSecondsF();
    runner->PostTask([callback, target_time_seconds]() { callback(target_time_seconds); });
  };
