...
android_surfaces_.erase(view_id);
  ...
  std::unique_ptr<Surface> AndroidSurfacesManager::CreateGPUSurface(
      GrDirectContext* gr_context) {
  auto iter = android_surfaces_.find(kFlutterImplicitViewId);
  if (iter != android_surfaces_.end()) {
...
PlatformViewAndroid::CreateExternalViewEmbedder() {
  ...
  return std::make_shared<AndroidExternalViewEmbedderWrapper>(
      ...
      [surfaces_manager_ptr,
       android_context = android_context_](int64_t view_id)
          -> std::unique_ptr<SurfaceFrameLayer> {  // get_gpu_surface_delegate
  ...
  //   AndroidSurfacesManager* surfaces_manager_ptr = this;
    if (is_impeller_auto_select || android_context->IsDynamicSelection()) {
      switch (android_rendering_api) {
        case AndroidRenderingAPI::kImpellerVulkan: {
            FML_LOG(ERROR) << "AndroidRenderingAPI::kImpellerVulkan, view_id: " << view_id;
                auto *surface = surfaces_manager_ptr->GetSurface(view_id);
                auto *backed_surface = static_cast<AndroidSurfaceDynamicImpeller*>(surface);
                auto * surface_vk = static_cast<AndroidSurfaceVKImpeller *>(backed_surface->GetBackedSurface());

                return std::make_unique<GPUSurfaceVulkanSurfaceFrameLayer>(
                          nullptr,
                          surface_vk->GetImpellerContext());
        }
        case AndroidRenderingAPI::kImpellerOpenGLES: {
            FML_LOG(ERROR) << "AndroidRenderingAPI::kImpellerOpenGLES, view_id: " << view_id;
                auto *surface = surfaces_manager_ptr->GetSurface(view_id);
                auto *backed_surface = static_cast<AndroidSurfaceDynamicImpeller*>(surface);
                // GPUSurfaceGLDelegate
            ...
            //         surfaces_manager_ptr->GetSurface(view_id));

                return std::make_unique<GPUSurfaceGLSurfaceFrameLayer>(
                          delegate,
                          android_context->GetImpellerContext());
        }
        case AndroidRenderingAPI::kSoftware:
        case AndroidRenderingAPI::kSkiaOpenGLES:
        case AndroidRenderingAPI::kImpellerAutoselect:
    ...
    switch (android_rendering_api) {
      case AndroidRenderingAPI::kSkiaOpenGLES:
          FML_UNREACHABLE();
      case AndroidRenderingAPI::kImpellerVulkan: {
            FML_LOG(ERROR) << "AndroidRenderingAPI::kImpellerVulkan 222, view_id: " << view_id;
      ...
      case AndroidRenderingAPI::kImpellerOpenGLES:
          {
            ...
            FML_LOG(ERROR) << "AndroidRenderingAPI::kImpellerOpenGLES 222, view_id: " << view_id;
            auto *delegate = static_cast<AndroidSurfaceGLImpeller *>(
                    surfaces_manager_ptr->GetSurface(view_id));
