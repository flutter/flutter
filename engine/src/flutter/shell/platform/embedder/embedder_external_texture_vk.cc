...
sk_sp<DlImage> EmbedderExternalTextureVK::ResolveTextureSkia(
  ...
  if (texture->image == 0 || width == 0 || height == 0) {
    if (release_proc && release_context) {
      release_proc(release_context);
    }
    FML_LOG(ERROR) << "Invalid Vulkan external texture parameters (image="
                   << texture->image << ", size=" << width << "x" << height
                   << ")";
    return nullptr;
  }
  ...
  if (texture->ycbcr_conversion_info != nullptr) {
    ...
    if (ycbcr->external_format != 0) {
      image_info.fYcbcrConversionInfo = skgpu::VulkanYcbcrConversionInfo(
          ...
          components,
          static_cast<VkFormatFeatureFlags>(ycbcr->format_features));
    } else {
      image_info.fYcbcrConversionInfo = skgpu::VulkanYcbcrConversionInfo(
  ...
  if (texture->ycbcr_conversion_info == nullptr) {
    if (texture->format == VK_FORMAT_B8G8R8A8_UNORM ||
