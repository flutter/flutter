...
// position callback runs only once, for the initial placement; afterwards the
...
class HostWindowSatellite : public HostWindowSized {
 ...
 private:
  ...
  // to |parent|, by invoking |get_position_callback| in |isolate|. Returns
  // std::nullopt if the callback declines to provide a rectangle.
  static std::optional<WindowRect> ComputePosition(
      GetWindowPositionCallback get_position_callback,
      const Isolate& isolate,
      HWND parent,
