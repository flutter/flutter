...
class WindowManagerTest : public WindowsTest {
 ...
 protected:
  ...
  static bool OffsetPositionCallback(const WindowSize& child_size,
                                     const WindowRect& parent_rect,
                                     const WindowRect& output_rect,
                                     WindowRect* out_rect) {
    out_rect->left = parent_rect.left + 20;
    out_rect->top = parent_rect.top + 30;
    out_rect->width = child_size.width;
    out_rect->height = child_size.height;
    return true;
  }
...
TEST_F(WindowManagerTest, CreateTooltipWindow) {
  ...
  auto position_callback =
      [](const WindowSize& child_size, const WindowRect& parent_rect,
         const WindowRect& output_rect, WindowRect* out_rect) -> bool {
    out_rect->left = parent_rect.left + 10;
    out_rect->top = parent_rect.top + 10;
    out_rect->width = child_size.width;
    out_rect->height = child_size.height;
    return true;
  };
...
TEST_F(WindowManagerTest,
  ...
  auto position_callback =
    ...
    out_rect->left = parent_rect.left + callback_count * 5;
    out_rect->top = parent_rect.top + callback_count * 5;
    out_rect->width = child_size.width;
    out_rect->height = child_size.height;
    return true;
  };
...
TEST_F(WindowManagerTest, PopupWindowHandlesNullPositionCallback) {
  ...
  auto position_callback = [](const WindowSize& child_size,
                              const WindowRect& parent_rect,
                              const WindowRect& output_rect,
                              WindowRect* out_rect) -> bool { return false; };
...
TEST_F(WindowManagerTest, SatelliteWindowIsDecoratedAndNotMinimizable) {
  IsolateScope isolate_scope(isolate());
  ...
  const LONG style = GetWindowLong(satellite_window_handle, GWL_STYLE);
  EXPECT_NE(style & WS_CAPTION, 0L);
  EXPECT_EQ(style & WS_MINIMIZEBOX, 0L);
}

TEST_F(WindowManagerTest, SatelliteWindowResizableHasThickFrame) {
  IsolateScope isolate_scope(isolate());

  const HWND parent_window_handle = CreateParentWindow();

  SatelliteWindowCreationRequest resizable_request =
      SatelliteCreationRequest(parent_window_handle, OffsetPositionCallback);
  resizable_request.resizable = true;
  const HWND resizable_handle =
      InternalFlutterWindows_WindowManager_GetTopLevelWindowHandle(
          engine_id(),
          InternalFlutterWindows_WindowManager_CreateSatelliteWindow(
              engine_id(), &resizable_request));

  SatelliteWindowCreationRequest fixed_request =
      SatelliteCreationRequest(parent_window_handle, OffsetPositionCallback);
  fixed_request.resizable = false;
  const HWND fixed_handle =
      InternalFlutterWindows_WindowManager_GetTopLevelWindowHandle(
          engine_id(),
          InternalFlutterWindows_WindowManager_CreateSatelliteWindow(
              engine_id(), &fixed_request));

  EXPECT_NE(GetWindowLong(resizable_handle, GWL_STYLE) & WS_THICKFRAME, 0L);
  EXPECT_EQ(GetWindowLong(fixed_handle, GWL_STYLE) & WS_THICKFRAME, 0L);
}
...
TEST_F(WindowManagerTest, SatelliteWindowUsesPositionCallbackOnCreation) {
  ...
  // Compare against the window frame rather than the window rectangle: window
  // creation aligns the frame, not the window rectangle (which includes the
...
TEST_F(WindowManagerTest, DestroyedSatelliteWindowNoLongerFollowsParent) {
  IsolateScope isolate_scope(isolate());
  ...
  // A second satellite on the same parent verifies that iteration skips the
  // destroyed satellite rather than aborting the whole propagation.
  SatelliteWindowCreationRequest survivor_request =
      SatelliteCreationRequest(parent_window_handle, OffsetPositionCallback);
  const HWND survivor_handle =
      InternalFlutterWindows_WindowManager_GetTopLevelWindowHandle(
          engine_id(),
          InternalFlutterWindows_WindowManager_CreateSatelliteWindow(
              engine_id(), &survivor_request));

  InternalFlutterWindows_WindowManager_OnDestroyWindow(satellite_handle);
  EXPECT_FALSE(IsWindow(satellite_handle));
  EXPECT_EQ(engine()->GetViewFromTopLevelWindow(satellite_handle), nullptr);

  RECT survivor_rect_before;
  GetWindowRect(survivor_handle, &survivor_rect_before);

  // Moving the parent must not touch the destroyed satellite. This would
  // use-after-free if the satellite were still registered with the manager.
  RECT parent_rect;
  GetWindowRect(parent_window_handle, &parent_rect);
  SetWindowPos(parent_window_handle, nullptr, parent_rect.left + 25,
               parent_rect.top + 25, 0, 0, SWP_NOSIZE | SWP_NOZORDER);

  RECT survivor_rect_after;
  GetWindowRect(survivor_handle, &survivor_rect_after);
  EXPECT_EQ(survivor_rect_after.left, survivor_rect_before.left + 25);
  EXPECT_EQ(survivor_rect_after.top, survivor_rect_before.top + 25);
}
