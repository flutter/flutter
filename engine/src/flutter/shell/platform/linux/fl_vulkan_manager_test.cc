...
TEST(FlVulkanManagerTest, CreateWithoutWindow) {
  ...
  if (manager != nullptr) {
    ...
    // Test proc address lookup. Query an instance-level command;
    // the global commands (vkCreateInstance, vkEnumerateInstance*) return
    // null from vkGetInstanceProcAddr when a non-null instance is passed.
    void* proc = fl_vulkan_manager_get_instance_proc_address(
        manager, fl_vulkan_manager_get_instance(manager), "vkDestroyInstance");
