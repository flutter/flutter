...
static bool initialize_vulkan(FlVulkanManager* self) {
  ...
  if (self->window != nullptr) {
    ...
    if (GDK_IS_WAYLAND_DISPLAY(gdk_window_get_display(self->window))) {
      ...
      // Use synchronized mode so the subsurface's buffer is applied
      // atomically with the parent surface's commit. This keeps the Vulkan
      // content and the window geometry in lockstep during resizes, avoiding
      // the visible tearing/desync a desynchronized subsurface produces when
      // the parent resizes before the child presents its new-size buffer.
