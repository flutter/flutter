...
static FlEngine* fl_engine_new_full(FlDartProject* project,
  ...
  // The Vulkan backend is selected through the same engine switches as the
  // other desktop platforms: --enable-impeller together with
  // --impeller-backend=vulkan. The software renderer remains opt-in through
  // the FLUTTER_LINUX_RENDERER environment variable, and OpenGL is the
  // default.
  bool enable_impeller = false;
  bool vulkan_requested = false;
  for (const auto& engine_switch : flutter::GetSwitchesFromEnvironment()) {
    if (engine_switch == "--enable-impeller" ||
        engine_switch == "--enable-impeller=true") {
      enable_impeller = true;
    } else if (engine_switch == "--enable-impeller=false") {
      enable_impeller = false;
    } else if (engine_switch == "--impeller-backend=vulkan") {
  ...
  if (g_strcmp0(renderer, "software") == 0) {
  ...
  } else if (enable_impeller && vulkan_requested) {
    if (!fl_vulkan_manager_is_available()) {
      g_warning(
          "The Vulkan backend was requested but Vulkan is not available; "
    ...
    g_warning(
        "The Vulkan backend is not selected through FLUTTER_LINUX_RENDERER. "
        "Use the --enable-impeller and --impeller-backend=vulkan engine "
        "switches instead. Falling back to OpenGL.");
    self->renderer_type = kOpenGL;
  } else {
