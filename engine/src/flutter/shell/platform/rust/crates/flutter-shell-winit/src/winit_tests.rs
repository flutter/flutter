...
fn rejects_malformed_text_input_commands_and_utf16_ranges() {
    ...
    assert!(TextInputCommand::decode(br#"{"method":"TextInput.unknown","args":null}"#,).is_none());
...
fn android_text_input_sync_suppresses_stale_native_echo_until_target_arrives() {
    ...
    assert!(
        !session
            .suppress_android_text_input_while_syncing(false, deadline + Duration::from_secs(1),)
    );
    assert!(session.android_text_input_sync_deadline.is_none());
}
...
fn raw_key_message_terminates_the_modern_key_packet() {
    ...
    let message: Value =
        serde_json::from_slice(&keyboard.raw_event_message(&event)).expect("valid raw key JSON");
