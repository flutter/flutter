...
GPUSurfaceMetalSurfaceFrameLayer::GPUSurfaceMetalSurfaceFrameLayer(
...
GPUSurfaceMetalSurfaceFrameLayer::~GPUSurfaceMetalSurfaceFrameLayer() = default;

bool GPUSurfaceMetalSurfaceFrameLayer::IsValid() {
  return !!aiks_context_ && aiks_context_->IsValid();
}

std::unique_ptr<SurfaceFrame> GPUSurfaceMetalSurfaceFrameLayer::MakeSurfaceFrame(const DlISize& frame_size) {
  ...
  const GPUSurfaceMetalDelegate* delegate = delegate_; //get_gpu_surface_metal_delegate_
...
std::unique_ptr<SurfaceFrame> GPUSurfaceMetalSurfaceFrameLayer::AcquireFrameFromCAMetalLayer(
...
std::unique_ptr<SurfaceFrame> GPUSurfaceMetalSurfaceFrameLayer::AcquireFrameFromMTLTexture(
    const GPUSurfaceMetalDelegate* delegate,
    const DlISize& frame_size) {
  // const GPUSurfaceMetalDelegate* delegate = get_gpu_surface_metal_delegate_
  //                                             ? get_gpu_surface_metal_delegate_(view_id)
...
GPUSurfaceMetalImpeller::GPUSurfaceMetalImpeller(
  ...
  if (render_to_surface) {
    surface_frame_layer_ = std::make_unique<GPUSurfaceMetalSurfaceFrameLayer>(delegate, context);
  }
}
...
std::unique_ptr<SurfaceFrame> GPUSurfaceMetalImpeller::AcquireFrame(const DlISize& frame_size) {
  ...
  FML_CHECK(surface_frame_layer_);

  return surface_frame_layer_->MakeSurfaceFrame(frame_size);
