...
class EngineExecutableTask:  # pylint: disable=too-many-instance-attributes
    ...
    ) -> typing.Tuple[typing.Optional[Exception], typing.List[str]]:
...
def gather_dart_test(
    ...
    error_message = (
        f"{kernel_file_output} doesn't exist. " f"Please run the build that populates {build_dir}"
    )
...
def gather_dart_tests(
    ...
    dart_vm_service_tests = (
        [] if "release" in build_dir else glob.glob(f"{dart_tests_dir}/vm_service/*_test.dart")
    )
...
def run_engine_tasks_in_parallel(tasks: typing.List[EngineExecutableTask]) -> bool:
    ...
    try:
        with multiprocessing.Pool(
            max_processes, worker_init, [queue, _logger.getEffectiveLevel()]
        ) as pool:
