...
def launch_package(package_name, activity_name, adb_path="adb"):
    ...
    subprocess.check_output(
        [adb_path, "shell", "am ", "start", "-n", "%s/%s" % (package_name, activity_name)],
        stderr=subprocess.STDOUT,
    )
...
def collect_and_validate_trace(adb_path="adb"):
    ...
    subprocess.check_output(
        [adb_path, "pull", PERFETTO_TRACE_FILE, "trace.pb"], stderr=subprocess.STDOUT
    )
    ...
    traceconv_output = subprocess.check_output(
        [traceconv, "systrace", "trace.pb"], stderr=subprocess.STDOUT, universal_newlines=True
    )
...
def main():
    ...
    android_api_level = subprocess.check_output(
        [args.adb_path, "shell", "getprop", "ro.build.version.sdk"], text=True
    ).strip()
