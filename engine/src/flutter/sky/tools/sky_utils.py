...
def create_fat_macos_framework(args, dst, fat_framework, arm64_framework, x64_framework):
    ...
    lipo(
        [
            get_mac_framework_dylib_path(arm64_framework),
            get_mac_framework_dylib_path(x64_framework),
        ],
        framework_dylib,
    )
...
def _set_framework_permissions(framework_dir):
    ...
    find_subprocess = subprocess.Popen(
        ["find", versions_path, "-perm", "-100", "-print0"], stdout=subprocess.PIPE
    )
    # Add execute permission for other and group for all files that had it for owner.
    xargs_subprocess = subprocess.Popen(
        ["xargs", "-0", "chmod", "og+x"], stdin=find_subprocess.stdout
    )
