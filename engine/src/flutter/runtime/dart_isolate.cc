...
Dart_Isolate DartIsolate::DartCreateAndStartServiceIsolate(
  ...
  bool is_custom_vmservice = !settings.vmservice_kernel_path.empty() || vm_data->GetServiceIsolateSnapshot() != vm_data->GetIsolateSnapshot();
  ...
  if (!DartServiceIsolate::Startup(
          ...
          settings.enable_service_port_fallback,  // enable fallback to port 0
                                                  // when bind fails.
          is_custom_vmservice,                    // use custom vm service
          error                                   // error (out)
          )) {
