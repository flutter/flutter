...
struct DlColor {
 public:
  ...
  static uint8_t toAlpha(DlScalar opacity) { return toC(opacity); }
  ...
  /// gamut colors.
  int getAlpha() const { return toC(alpha_); }
  ///\deprecated Use floating point accessors to avoid data loss when using wide
  /// gamut colors.
  int getRed() const { return toC(red_); }
  ///\deprecated Use floating point accessors to avoid data loss when using wide
  /// gamut colors.
  int getGreen() const { return toC(green_); }
  ///\deprecated Use floating point accessors to avoid data loss when using wide
  /// gamut colors.
  int getBlue() const { return toC(blue_); }
 ...
 private:
  ...
  static uint8_t toC(DlScalar fComp) { return std::round(fComp * 255); }
};
