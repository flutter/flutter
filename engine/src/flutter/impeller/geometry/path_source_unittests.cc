...
TEST(PathSourceTest, RoundSuperellipseSourceTest) {
  Rect rect = Rect::MakeLTRB(10, 15, 40, 60);
  RoundingRadii radii = {
      .top_left = Size(1, 11),
      .top_right = Size(2, 12),
  ...
  RoundSuperellipse rse = RoundSuperellipse::MakeRectRadii(rect, radii);
...

  {
    ...
    // All of the coordinates here are based on calculations that are not
    // exposed so we will simply curate the order of path segments:
    // 1 MoveTo, 16 CubicTo's, 1 LineTo, and a Close (and a PathEnd)
    EXPECT_CALL(receiver, MoveTo(::testing::_, true));
    EXPECT_CALL(receiver, CubicTo(::testing::_, ::testing::_, ::testing::_))
        .Times(16);
    EXPECT_CALL(receiver, LineTo(::testing::_));
    EXPECT_CALL(receiver, Close());
    EXPECT_CALL(receiver, PathEnd());
  }
