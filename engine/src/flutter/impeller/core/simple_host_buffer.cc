...
SimpleHostBuffer::SimpleHostBuffer(
...
BufferView SimpleHostBuffer::Emplace(const void* buffer,
                                     size_t length,
                                     size_t align,
...
BufferView SimpleHostBuffer::Emplace(const void* buffer, size_t length) {
  auto [range, device_buffer, raw_device_buffer] =
...
BufferView SimpleHostBuffer::Emplace(size_t length,
                                     size_t align,
                                     BufferCategory category,
...
SimpleHostBuffer::TestStateQuery SimpleHostBuffer::GetStateForTest(
    BufferCategory category) {
...
bool SimpleHostBuffer::MaybeCreateNewBuffer() {
  current_buffer_++;
...
SimpleHostBuffer::EmplaceInternal(size_t length,
                                  size_t align,
                                  const EmplaceProc& cb) {
  if (!cb) {
    return {};
...
std::tuple<Range, std::shared_ptr<DeviceBuffer>, DeviceBuffer*>
SimpleHostBuffer::EmplaceInternal(const void* buffer, size_t length) {
  // If the requested allocation is bigger than the block size, create a one-off
  // device buffer and write to that.
  if (length > kAllocatorBlockSize) {
    DeviceBufferDescriptor desc;
...
std::tuple<Range, std::shared_ptr<DeviceBuffer>, DeviceBuffer*>
SimpleHostBuffer::EmplaceInternal(const void* buffer,
                                  size_t length,
                                  size_t align) {
...
const std::shared_ptr<DeviceBuffer>& SimpleHostBuffer::GetCurrentBuffer()
    const {
  return device_buffers_[frame_index_][current_buffer_];
}

void SimpleHostBuffer::Reset() {
...
size_t SimpleHostBuffer::GetMinimumUniformAlignment() const {
