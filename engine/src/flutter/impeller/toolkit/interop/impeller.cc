...
using SamplersAndUniforms =
    std::pair<std::vector<std::shared_ptr<flutter::DlColorSource>>,
...
static SamplersAndUniforms ReadSamplersAndUniforms(ImpellerTexture* samplers,
  ...
  result.first = std::vector<std::shared_ptr<flutter::DlColorSource>>{};
  result.second = std::make_shared<std::vector<uint8_t>>();
  if (data_bytes_length > 0) {
    std::copy(data, data + data_bytes_length,
              std::back_inserter(*result.second));
  }
  if (samplers_count > 0) {
    result.first.reserve(samplers_count);
    for (size_t i = 0; i < samplers_count; i++) {
      result.first.emplace_back(flutter::DlColorSource::MakeImage(
...
ImpellerColorSourceCreateFragmentProgramNew(
  ...
  return ColorSource::MakeFragmentProgram(*GetPeer(context),           //
                                          *GetPeer(fragment_program),  //
                                          std::move(bindings.first),   //
                                          std::move(bindings.second)   //
                                          )
      .Leak();
}
