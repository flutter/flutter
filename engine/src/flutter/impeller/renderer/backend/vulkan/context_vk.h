...
class ContextVK final : public Context,
 ...
 public:
  ...
  ///             to a non-recoverable GPU error (e.g. AMD non-conformant
  ///             OOM from vkQueueSubmit). Once lost, all command-buffer
  ///             creation and queue submission silently short-circuit to
  ///             prevent further Vulkan calls on a broken driver state.
 ...
 private:
  ...
  // Set to true on any unrecoverable GPU error (e.g. vkQueueSubmit OOM on
  // AMD). Once true, CreateCommandBuffer() returns nullptr and Submit()
  // returns immediately so that no further Vulkan API calls are made on the
