...
TEST(PathSdfContentsTest, SimpleMake) {
  ...
  PathSegmentReceiver segment_receiver;
  path.Dispatch(segment_receiver);
  auto contents = PathSdfContents::Make(std::move(geometry), Color::Red(), 0.0f,
                                        segment_receiver.GetSegments());
